...
public struct OBB : IEquatable<OBB>
{
	[Preserve]
	[BurstCompile]
	public struct ContainsJob : IJobParallelFor
	{
		[WriteOnly]
		...
		{
			Results[index] = OBBs[index].Contains(Points[index]);
		}
	}
	...
	public Bounds ToBounds()
	{
		...
		BoundsEncapsulate(ref bounds);
		return bounds;
	}

	public void BoundsEncapsulate(ref Bounds bounds)
	{
		Vector3 vector = extents.x * right;
		Vector3 vector2 = extents.y * up;
		Vector3 vector3 = extents.z * forward;
		bounds.Encapsulate(position + vector2 + vector + vector3);
		bounds.Encapsulate(position + vector2 + vector - vector3);
		bounds.Encapsulate(position + vector2 - vector + vector3);
		bounds.Encapsulate(position + vector2 - vector - vector3);
		bounds.Encapsulate(position - vector2 + vector + vector3);
	...
	{
		return GJK.Distance(in this, in other);
	}
	...
	{
		return GJK.Distance2(in this, in other);
	}
	...
	public void DebugDrawGizmos(Color colour)
	{
		Color color = Gizmos.color;
		Gizmos.color = colour;
		Gizmos.DrawLine(GetPoint(-1f, -1f, -1f), GetPoint(1f, -1f, -1f));
		Gizmos.DrawLine(GetPoint(-1f, -1f, 1f), GetPoint(1f, -1f, 1f));
		Gizmos.DrawLine(GetPoint(-1f, -1f, -1f), GetPoint(-1f, -1f, 1f));
		Gizmos.DrawLine(GetPoint(1f, -1f, -1f), GetPoint(1f, -1f, 1f));
		Gizmos.DrawLine(GetPoint(-1f, -1f, -1f), GetPoint(-1f, 1f, -1f));
	...
	{
		if (rotation.Equals(other.rotation) && position.Equals(other.position) && extents.Equals(other.extents) && forward.Equals(other.forward) && right.Equals(other.right) && up.Equals(other.up))
		{
			return reject.Equals(other.reject);
		}
		return false;
	}
	...
	{
		return HashCode.Combine(rotation, position, extents, forward, right, up, reject);
	}
}
