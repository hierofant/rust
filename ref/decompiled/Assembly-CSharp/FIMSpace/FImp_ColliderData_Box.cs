...
public class FImp_ColliderData_Box : FImp_ColliderData_Base
{
	...
	public override void RefreshColliderData()
	{
		...
		{
			bool flag2 = false;
			if (Vector2.Distance((Vector2)base.Transform.position, (Vector2)previousPosition) > Mathf.Epsilon)
			{
				flag2 = true;
			}
			else if (!FEngineering.QIsSame(base.Transform.rotation, previousRotation))
			...
			if (flag2)
			{
				...
				lossyScale.z = 1f;
				scales = Vector3.Scale((Vector3)Box2D.size, lossyScale);
				scales.Normalize();
			}
		}
	...
	public static void PushOutFromBoxCollider(BoxCollider2D box2D, float segmentColliderRadius, ref Vector3 segmentPosition)
	{
		...
		lossyScale.z = 1f;
		((Vector2)Vector3.Scale((Vector3)box2D.size, lossyScale)).Normalize();
	...
	protected static Vector3 GetTargetPlaneNormal(BoxCollider2D boxCollider, Vector2 point, Vector2 right, Vector2 up, Vector2 scales)
	{
		...
		vector.x = Vector3.Dot((Vector3)normalized, (Vector3)right.normalized);
		vector.y = Vector3.Dot((Vector3)normalized, (Vector3)up.normalized);
		vector.x *= scales.y;
		vector.y *= scales.x;
		vector.Normalize();
