...
public class OreResourceEntity : StagedResourceEntity
{
	...
	public override void OnAttacked(HitInfo info)
	{
		if (!info.DidGather && info.gatherScale > 0f)
		{
			if (useHotspotMinigame)
			{
				...
				{
					num = ((_hotSpot != null) ? Vector3.Distance(info.HitPositionWorld, _hotSpot.transform.position) : float.MaxValue);
				}
				if (flag || flag2 || num <= _hotSpot.GetComponent<SphereCollider>().radius * 1.5f)
				{
					bonusesKilled++;
					...
					if (_hotSpot != null)
					{
						...
						ClientRPC(RpcTarget.NetworkGroup("PlayBonusLevelSound"), bonusesKilled, _hotSpot.transform.position);
					}
				}
	...
	private OreHotSpot SpawnBonusSpotOnRay(Ray r)
	{
		...
		if (ResourceMeshColliders[0].Raycast(r, out var hitInfo, 15f))
		{
	...
	public OreHotSpot SpawnBonusSpot(Vector3 lastDirection)
	{
		...
		MeshCollider meshCollider = ResourceMeshColliders[0];
		Vector3 vector = base.transform.InverseTransformPoint(meshCollider.bounds.center);
		...
		else
		{
			...
			float num2 = UnityEngine.Random.Range(0.25f, 0.5f) + (float)stage * 0.25f;
			float num3 = ((UnityEngine.Random.Range(0, 2) == 0) ? (-1f) : 1f);
		...
		bonusesSpawned++;
		Vector3 normalized = (meshCollider.bounds.center - zero).normalized;
		if (meshCollider.Raycast(new Ray(zero, normalized), out var hitInfo, 15f))
		{
