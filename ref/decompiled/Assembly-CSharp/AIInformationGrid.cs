...
public class AIInformationGrid : MonoBehaviour
{
	...
	public void Init()
	{
		...
		AIPoint[] componentsInChildren = GetComponentsInChildren<AIPoint>(includeInactive: true);
		foreach (AIPoint aIPoint in componentsInChildren)
		{
			BoundingBox.Encapsulate(aIPoint.transform.position);
		}
		...
		for (int j = 0; j < zCellCount; j++)
		{
			for (int k = 0; k < xCellCount; k++)
			{
				...
				Cells[GetIndex(k, j)] = new AIInformationCell(bounds, base.gameObject, k, j);
				vector.x += CellSize;
			}
			vector.x = BoundingBox.min.x + (float)CellSize / 2f;
			vector.z += CellSize;
