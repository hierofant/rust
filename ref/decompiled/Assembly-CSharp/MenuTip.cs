...
public class MenuTip : MonoBehaviour
{
	...
	public static Translate.Phrase[] MenuTips = new Translate.Phrase[34]
	{
	...
	{
		disabled = false;
		icon.SetIcon(Icons.InfoCircle);
		currentTipIndex = UnityEngine.Random.Range(0, MenuTips.Length);
	}

	public void Update()
	{
		if (UI_LoadingScreen.isOpen && !disabled && Time.realtimeSinceStartup >= nextTipTime)
		{
			currentTipIndex++;
	...
	public void SetCustomTip(string iconString, string tip)
	{
		...
		icon.SetIcon(result);
		text.SetText(tip.FilterRichText(true, "color", "br", "u", "b", "i").Truncate(400));
		disabled = true;
	}

	public void UpdateTip()
	{
		text.SetPhrase(MenuTips[currentTipIndex]);
	}
}
