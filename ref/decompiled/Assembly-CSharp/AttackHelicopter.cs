...
public class AttackHelicopter : PlayerHelicopter
{
	...
	public override void FilterServerProjectileAmmo(List<Item> ammoList)
	{
		...
		for (int i = 0; i < ammoList.Count; i++)
		{
			...
			if ((item.info == rockets.hvRocketDef && rockets.preferredRocketType != 0) || (item.info == rockets.incendiaryRocketDef && rockets.preferredRocketType != PreferredRocketType.Incendiary))
			{
				ammoList.RemoveAt(i);
				i--;
			}
