...
public class ScrollRectEx : UIBehaviour, IInitializePotentialDragHandler, IEventSystemHandler, IBeginDragHandler, IEndDragHandler, IDragHandler, IScrollHandler, ICanvasElement, ILayoutGroup, ILayoutController
{
	...
	public Rect normalizedViewRect
	{
		get
		{
			...
			return new Rect(size: (Vector2)Vector3.Scale(m_ViewBounds.size, b), position: (Vector2)vector);
		}
	}
	...
	public virtual void OnScroll(PointerEventData data)
	{
		...
		anchoredPosition += scrollDelta / 10f * m_ScrollSensitivity;
	...
	{
		if ((eventData.button != scrollButton && eventData.button != altScrollButton) || !IsActive() || !RectTransformUtility.ScreenPointToLocalPointInRectangle(viewRect, eventData.position, eventData.pressEventCamera, out var localPoint))
		{
			return;
		}
		UpdateBounds();
