...
public class GenerateDungeonGrid : ProceduralComponent
{
	...
	public override void Process(uint seed)
	{
		...
		else
		{
			...
			foreach (DungeonGridInfo item3 in list)
			{
				...
				foreach (TerrainPathConnect terrainPathConnect in componentsInChildren)
				{
					...
					Prefab<DungeonGridCell> prefab2 = ((cellPos.x > num) ? worldSpaceGrid[cellPos.x - 1, cellPos.y] : null);
					Prefab<DungeonGridCell> prefab3 = ((cellPos.x < num2) ? worldSpaceGrid[cellPos.x + 1, cellPos.y] : null);
					Prefab<DungeonGridCell> prefab4 = ((cellPos.y > num) ? worldSpaceGrid[cellPos.x, cellPos.y - 1] : null);
					...
					bool flag = prefab2 != null || cellPos.x <= num;
					bool flag2 = prefab3 != null || cellPos.x >= num2;
					bool flag3 = prefab4 != null || cellPos.y <= num;
				...
				for (int num6 = 0; num6 < worldSpaceGrid.CellCount; num6++)
				{
					for (int num7 = 0; num7 < worldSpaceGrid.CellCount; num7++)
					{
						...
						if (prefab12 != null)
						{
							...
							while (!IsValidPrefabPlacement(prefab12, zero2 + vector2, Quaternion.identity, Vector3.one, EnvironmentType.Underground | EnvironmentType.Building))
							{
								zero2.y -= 3f;
							}
						}
			...
			{
				Vector3 vector3 = item6.upwards.origin.position + item6.upwards.origin.rotation * Vector3.Scale(item6.upwards.origin.upSocket.localPosition, item6.upwards.origin.scale);
				Vector3 vector4 = item6.downwards.origin.position + item6.downwards.origin.rotation * Vector3.Scale(item6.downwards.origin.downSocket.localPosition, item6.downwards.origin.scale) - vector3;
				...
				for (int k = 0; k < array8.Length; k++)
				{
					...
					int num8 = 0;
					int num9 = 0;
					while (vector4.magnitude > 1f && (num8 < 8 || num9 < 8))
					{
						...
						foreach (Prefab<DungeonGridLink> prefab14 in array9)
						{
							...
							Vector3 a2 = vector4 + vector5;
							Vector3 a3 = vector4 + vector8;
							float magnitude = a2.magnitude;
							float magnitude2 = a3.magnitude;
							Vector3 vector9 = Vector3.Scale(a2, b2);
							Vector3 vector10 = Vector3.Scale(a3, b2);
							...
							if (Mathf.Abs(vector10.x) - Mathf.Abs(vector9.x) > 0.01f || (Mathf.Abs(vector10.x) > 0.01f && a2.x * a3.x < 0f) || Mathf.Abs(vector10.y) - Mathf.Abs(vector9.y) > 0.01f || (Mathf.Abs(vector10.y) > 0.01f && a2.y * a3.y < 0f) || Mathf.Abs(vector10.z) - Mathf.Abs(vector9.z) > 0.01f || (Mathf.Abs(vector10.z) > 0.01f && a2.z * a3.z < 0f) || (flag6 && b2.x == 0f && b2.z == 0f && component.DownType == DungeonGridLinkType.Default && ((Mathf.Abs(a3.x) > 0.01f && Mathf.Abs(a3.x) < LinkRadius * 2f - 0.1f) || (Mathf.Abs(a3.z) > 0.01f && Mathf.Abs(a3.z) < LinkRadius * 2f - 0.1f))))
							{
								continue;
							}
							num10 = num12;
							if (b2.x == 0f && b2.z == 0f)
							{
								if (!flag6 && Mathf.Abs(a3.y) < LinkTransition - 0.1f)
								{
									continue;
								}
							}
							else if ((!flag6 && magnitude4 > 0.01f && (Mathf.Abs(a3.x) < LinkRadius * 2f - 0.1f || Mathf.Abs(a3.z) < LinkRadius * 2f - 0.1f)) || (!flag7 && magnitude4 > 0.01f && (Mathf.Abs(a3.x) < LinkRadius * 1f - 0.1f || Mathf.Abs(a3.z) < LinkRadius * 1f - 0.1f)))
							{
								continue;
							}
						...
						if (b2.x > 0f || b2.z > 0f)
						{
							...
							foreach (Prefab<DungeonGridLink> prefab16 in array9)
							{
								...
								Vector3 a4 = vector4 - vector11;
								Vector3 a5 = vector4 - vector14;
								float magnitude5 = a4.magnitude;
								float magnitude6 = a5.magnitude;
								Vector3 vector15 = Vector3.Scale(a4, b2);
								Vector3 vector16 = Vector3.Scale(a5, b2);
								...
								if (Mathf.Abs(vector16.x) - Mathf.Abs(vector15.x) > 0.01f || (Mathf.Abs(vector16.x) > 0.01f && a4.x * a5.x < 0f) || Mathf.Abs(vector16.y) - Mathf.Abs(vector15.y) > 0.01f || (Mathf.Abs(vector16.y) > 0.01f && a4.y * a5.y < 0f) || Mathf.Abs(vector16.z) - Mathf.Abs(vector15.z) > 0.01f || (Mathf.Abs(vector16.z) > 0.01f && a4.z * a5.z < 0f) || (flag6 && b2.x == 0f && b2.z == 0f && component2.UpType == DungeonGridLinkType.Default && ((Mathf.Abs(a5.x) > 0.01f && Mathf.Abs(a5.x) < LinkRadius * 2f - 0.1f) || (Mathf.Abs(a5.z) > 0.01f && Mathf.Abs(a5.z) < LinkRadius * 2f - 0.1f))))
								{
									continue;
								}
								num13 = num15;
								if (b2.x == 0f && b2.z == 0f)
								{
									if (!flag6 && Mathf.Abs(a5.y) < LinkTransition - 0.1f)
									{
										continue;
									}
								}
								else if ((!flag6 && magnitude8 > 0.01f && (Mathf.Abs(a5.x) < LinkRadius * 2f - 0.1f || Mathf.Abs(a5.z) < LinkRadius * 2f - 0.1f)) || (!flag7 && magnitude8 > 0.01f && (Mathf.Abs(a5.x) < LinkRadius * 1f - 0.1f || Mathf.Abs(a5.z) < LinkRadius * 1f - 0.1f)))
								{
									continue;
								}
			...
			if (TerrainMeta.Path.Rails.Count > 0)
			{
				...
				for (int num16 = 0; num16 < worldSpaceGrid.CellCount; num16++)
				{
					for (int num17 = 0; num17 < worldSpaceGrid.CellCount; num17++)
					{
						...
						{
							if (prefab17.Component.North != prefab18.Component.North || prefab17.Component.South != prefab18.Component.South || prefab17.Component.West != prefab18.Component.West || prefab17.Component.East != prefab18.Component.East || !IsValidPrefabPlacement(prefab18, vector17, Quaternion.identity, Vector3.one, EnvironmentType.Underground) || !prefab18.ApplyTerrainChecks(vector17, Quaternion.identity, Vector3.one) || !prefab18.ApplyTerrainFilters(vector17, Quaternion.identity, Vector3.one))
							{
								continue;
							}
			...
			for (int num19 = 0; num19 < worldSpaceGrid.CellCount; num19++)
			{
				for (int num20 = 0; num20 < worldSpaceGrid.CellCount; num20++)
				{
					...
					if (prefab19 != null)
					{
						...
						World.AddPrefab("Dungeon", prefab19, zero2 + vector19, Quaternion.identity, Vector3.one);
					}
				}
			}
				...
				for (int num22 = 0; num22 < worldSpaceGrid.CellCount - 1; num22++)
				{
					...
					if (prefab20 != null && prefab21 != null && prefab20.Component.EastVariant != prefab21.Component.WestVariant)
					{
						...
						foreach (Prefab<DungeonGridCell> prefab23 in array7)
						{
							if (prefab23.Component.West == prefab20.Component.East && prefab23.Component.East == prefab21.Component.West && prefab23.Component.WestVariant == prefab20.Component.EastVariant && prefab23.Component.EastVariant == prefab21.Component.WestVariant)
							{
								...
								World.AddPrefab("Dungeon", prefab23, zero2 + vector20, Quaternion.identity, Vector3.one);
								break;
							}
						}
					}
					...
					foreach (Prefab<DungeonGridCell> prefab24 in array7)
					{
						if (prefab24.Component.South == prefab20.Component.North && prefab24.Component.North == prefab22.Component.South && prefab24.Component.SouthVariant == prefab20.Component.NorthVariant && prefab24.Component.NorthVariant == prefab22.Component.SouthVariant)
						{
							...
							World.AddPrefab("Dungeon", prefab24, zero2 + vector21, Quaternion.identity, Vector3.one);
							break;
						}
					}
				}
		...
		bool IsValidPrefabPlacement(Prefab<DungeonGridCell> prefab, Vector3 pos, Quaternion rot, Vector3 scale, EnvironmentType type)
		{
			...
			if (!prefab.CheckEnvironmentVolumesBelowAltitude(pos + vector22, rot, scale, EnvironmentType.Underground, EnvironmentType.Entrance) && !prefab.CheckEnvironmentVolumesInsideTerrain(pos + vector22, rot, scale, EnvironmentType.TrainTunnels, EnvironmentType.Entrance))
			{
				return false;
			}
			if (prefab.CheckEnvironmentVolumes(pos + vector23, rot, scale, type))
			{
				return false;
			}
