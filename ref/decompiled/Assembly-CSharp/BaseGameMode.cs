...
public class BaseGameMode : BaseEntity
{
	...
	public GameMode.PlayerScore GetPlayerScoreForPlayer(BasePlayer player)
	{
		...
		{
			if (playerScore2.userid == (ulong)player.userID)
			{
	...
	{
		if (SingletonComponent<ServerMgr>.Instance == null || !SingletonComponent<ServerMgr>.Instance.runFrameUpdate)
		{
		...
		{
			if ((monument.IsSafeZone || monument.Type == MonumentType.Town) && !safeZone)
			{
		...
		if (mlrs || !(SingletonComponent<SpawnHandler>.Instance != null))
		{
			return;
		}
	...
	{
		if (Rust.Application.isServerStarted && rustPlus != CompanionServer.Server.IsEnabled)
		{
			if (rustPlus)
			{
		...
		if (wipeBpsOnProtocol)
		{
			...
			BasePlayer[] array = UnityEngine.Object.FindObjectsByType<BasePlayer>(FindObjectsSortMode.None);
			for (int i = 0; i < array.Length; i++)
			{
	...
	public static bool TryGetActiveGameMode(bool serverside, out BaseGameMode gameMode)
	{
		gameMode = GetActiveGameMode(serverside);
	...
	public override void Save(SaveInfo info)
	{
		...
		info.msg.gameMode.scoreColumns = Facepunch.Pool.Get<List<GameMode.ScoreColumn>>();
		info.msg.gameMode.playerScores = Facepunch.Pool.Get<List<GameMode.PlayerScore>>();
		info.msg.gameMode.teams = Facepunch.Pool.Get<List<GameMode.TeamInfo>>();
	...
	protected virtual void OnCreated()
	{
		...
		{
			ApplyConVars();
			gameModeSpawnGroups = UnityEngine.Object.FindObjectsByType<GameModeSpawnGroup>(FindObjectsSortMode.None);
			UnassignAllPlayers();
	...
	public void ApplyConVars()
	{
		foreach (string convar in convars)
		{
			...
			{
				Debug.LogError("Failed to run ConVar '" + convar + "' for gamemode");
	...
	public virtual void LoadoutPlayer(BasePlayer player)
	{
		...
		if (!IsTeamGame())
		{
			playerInventoryProperties = ((!useStaticLoadoutPerPlayer) ? loadouts[UnityEngine.Random.Range(0, loadouts.Length)] : loadouts[SeedRandom.Range((uint)(ulong)player.userID, 0, loadouts.Length)]);
		}
		else
		{
	...
	public virtual PlayerInventory.CanMoveFromResponse CanMoveItemsFrom(PlayerInventory inv, BaseEntity source, Item item)
	{
	...
	public bool IsAllowed(ItemDefinition item, EraRestriction restriction)
	{
		...
		for (int i = 0; i < restrictions.Length; i++)
		{
			...
			if (itemRestriction.Definition == item && restriction != 0 && itemRestriction.Restriction != 0 && (itemRestriction.Restriction & restriction) == 0)
			{
				return false;
			}
		}
