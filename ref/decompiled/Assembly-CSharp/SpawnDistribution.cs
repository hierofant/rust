...
public class SpawnDistribution
{
	...
	public bool Sample(out Vector3 spawnPos, out Quaternion spawnRot, ByteQuadtree.Element node, bool alignToNormal = false, float dithering = 0f, float radius = 0f, SpawnFilter filter = null, float cutoff = 0f)
	{
		...
		for (int i = 0; i < 15; i++)
		{
			...
			if (vector.y <= spawnPos.y || (filter != null && filter.GetFactor(spawnPos) <= cutoff))
			{
				continue;
			}
			if ((int)placementCheckMask != 0)
			{
				bool flag = false;
				if ((radius != 0f) ? Physics.SphereCast(vector + Vector3.up * placementCheckHeight, radius, Vector3.down, out var hitInfo, placementCheckHeight, placementCheckMask) : Physics.Raycast(vector + Vector3.up * placementCheckHeight, Vector3.down, out hitInfo, placementCheckHeight, placementCheckMask))
				{
