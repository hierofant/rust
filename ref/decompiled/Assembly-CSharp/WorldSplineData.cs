...
public class WorldSplineData
{
	...
	public float GetClosestDistance(Vector3 localPoint)
	{
		...
		for (int i = 0; i < LUTValues.Count; i++)
		{
			...
			{
				float sqrMagnitude = (lUTEntry.points[j].pos - localPoint).sqrMagnitude;
				if (sqrMagnitude < num)
				{
					num = sqrMagnitude;
		...
		for (int k = 0; k < 5; k++)
		{
			...
			float sqrMagnitude2 = (GetPointCubicHermite(num4) - localPoint).sqrMagnitude;
			float sqrMagnitude3 = (GetPointCubicHermite(num5) - localPoint).sqrMagnitude;
