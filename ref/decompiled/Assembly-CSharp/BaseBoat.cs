...
public class BaseBoat : BaseVehicle
{
	...
	{
		return UnityEngine.Time.realtimeSinceStartup < driftDelayedUntil;
	}

	public void SetDriftDelayAmount(float delayAmount)
	{
		driftDelayedUntil = UnityEngine.Time.realtimeSinceStartup + delayAmount;
	}
	...
	public override void RunAIDriverTick()
	{
		...
		{
			aiInputProvider.OnTick(this, base.TimeSinceLastAIUpdate, ref steering, ref gasPedal);
		}
	}
	...
	public override void VehicleFixedUpdate()
	{
		using (TimeWarning.New("BaseBoat.VehicleFixedUpdate"))
		{
			...
			if (AnyMounted() && IsFlipped())
			{
				if (debug_eject_ai)
				{
					Debug.LogWarning($"Ejecting players from flipped boat {this}");
					Debug.LogWarning($"Boat was in state / pos {base.transform.position} / rot {base.transform.rotation.eulerAngles}");
				}
				DismountAllPlayers();
			}
		}
	...
	protected void ApplyCorrectionForces()
	{
		using (TimeWarning.New("ApplyCorrectionForces"))
		{
			if (applyCorrectionForces && planeFitPoints != null && planeFitPoints.Length == 3 && EngineOn() && !(buoyancy.submergedFraction < 0.5f))
			{
				...
				Matrix4x4 matrix4x = Matrix4x4.TRS(position, Quaternion.Euler(0f, eulerAngles.y, 0f), Vector3.one);
				for (int i = 0; i < planeFitPoints.Length; i++)
				{
					Vector3 vector = matrix4x.MultiplyPoint(planeFitPoints[i].localPosition);
					vector.y = WaterLevel.GetWaterLevel(vector, waves: true);
				...
				if (y2 > num + y3)
				{
					...
					Vector3 torque = pidController.ComputeRequiredAngularAcceleration(rotation, desiredOrientation, rigidBody.angularVelocity, UnityEngine.Time.fixedDeltaTime);
					rigidBody.AddTorque(torque, ForceMode.Acceleration);
				}
				else if (y2 > num + x)
				{
					Vector3 vector2 = Vector3.Cross(worldAnchors[1] - worldAnchors[0], worldAnchors[2] - worldAnchors[0]);
					Vector3 rhs = worldAnchors[2] - worldAnchors[1];
					Quaternion desiredOrientation2 = Quaternion.LookRotation(Vector3.Cross(vector2, rhs), vector2);
					pidController.Kp = wavePID.x;
					pidController.Ki = wavePID.y;
					pidController.Kd = wavePID.z;
					Vector3 torque2 = pidController.ComputeRequiredAngularAcceleration(rotation, desiredOrientation2, rigidBody.angularVelocity, UnityEngine.Time.fixedDeltaTime);
					torque2.y = 0f;
	...
	private void CheckDriftToShore()
	{
		...
		else if ((float)shoreDriftTimer < (float)secondsUntilShoreDrift - 1f)
		{
		...
		else if (TerrainTexturing.Instance == null)
		{
			lastDriftCheckStatus = "No terrain tex";
			shoreDriftTimer = 0f;
		}
		else if (BaseNetworkable.HasConnections(base.transform.position))
		{
		...
		else
		{
			...
			(Vector3 shoreDir, float shoreDist) coarseVectorToShore = TerrainTexturing.Instance.GetCoarseVectorToShore(base.transform.position);
			...
			Vector3 origin = base.transform.position + bounds.center + item * (num + 1f);
			Vector3 vector = base.transform.position + bounds.center + item * num2;
			Ray ray = new Ray(origin, item);
			...
			if (obj.Count > 0)
			{
				foreach (RaycastHit item3 in obj)
				{
					if (item3.collider != null)
					{
						BaseEntity baseEntity = GameObjectEx.ToBaseEntity(item3.collider);
						if (baseEntity != null && baseEntity.HasEntityInParents(this))
						{
							continue;
						}
					}
					if (item3.distance <= num3)
					{
						lastDriftCheckStatus = string.Format("Was blocked by {0}:{1} at {2}", (item3.transform.parent != null) ? item3.transform.parent.name : "", item3.transform.name, item3.transform.position);
						Facepunch.Pool.FreeUnmanaged(ref obj);
						return;
					}
					vector = item3.point - item * num3;
		...
		for (int j = 0; j < AI.ocean_patrol_path_iterations && flag; j++)
		{
			...
			for (int k = 0; k < num3; k++)
			{
				...
				for (int l = 0; l < num8; l++)
				{
					...
					if (collider != null && !ColliderEx.IsOnLayer(collider, Rust.Layer.Terrain))
					{
