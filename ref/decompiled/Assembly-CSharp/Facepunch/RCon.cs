...
public class RCon
{
	...
	public static void print_rcon_failed_logins(ConsoleSystem.Arg arg)
	{
		...
		{
			textTable.AddRow(item.Key.ToString(), item.Value.Attempts.ToString(), item.Value.IsBanned.ToString(), DateTime.UtcNow.ToString());
		}
		arg.ReplyWith(flag ? textTable.ToJson() : textTable.ToString());
	}
	...
	public static void print_rcon_clients(ConsoleSystem.Arg arg)
	{
		...
		using TextTable textTable = Pool.Get<TextTable>();
		textTable.ShouldPadColumns = !flag;
		textTable.AddColumns("index", "connection_id", "ip", "port", "connected_time", "inbound_messages", "outbound_messages", "broadcast_messages");
		for (int i = 0; i < clientStats.Count; i++)
		{
			...
			textTable.AddRow(i.ToString(), rconClientStats.ConnectionId.ToString(), rconClientStats.IP.ToString(), rconClientStats.Port.ToString(), System.Math.Floor(timeSpan.TotalSeconds).ToString(), rconClientStats.RecievedMessages.ToString(), rconClientStats.SentMessages.ToString(), rconClientStats.BroadcastedMessages.ToString());
		}
		arg.ReplyWith(flag ? textTable.ToJson() : textTable.ToString());
	}
