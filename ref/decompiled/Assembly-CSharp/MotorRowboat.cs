...
public class MotorRowboat : BaseBoat
{
	...
	public override void DoPushAction(BasePlayer player)
	{
		...
		else
		{
			...
			rigidBody.AddForceAtPosition(vector3 * num2 * rigidBody.mass, position, ForceMode.Impulse);
		}
	...
	public override void VehicleFixedUpdate()
	{
		...
		using (TimeWarning.New("MotorRowboat.VehicleFixedUpdate"))
		{
			...
			{
				float num2 = 1f;
				float value = rigidBody.linearVelocity.Magnitude2D();
	...
	public override void SeatClippedWorld(BaseMountable mountable)
	{
		...
		if (!(mounted == null))
		{
			...
			float num = Mathf.InverseLerp(4f, 20f, rigidBody.linearVelocity.magnitude);
			if (num > 0f)
			{
	...
	{
		float value = rigidBody.linearVelocity.SqrMagnitude2D();
		float num = Mathf.InverseLerp(0f, 2f, value);
		float num2 = angularDragBase * (IsOn() ? 1f : engineOffAngularDragMultiplier);
		rigidBody.angularDamping = num2 + angularDragVelocity * num;
		float num3 = (IsOn() ? waterDrag : (waterDrag * engineOffWaterDragMultiplier));
		rigidBody.linearDamping = landDrag + num3 * Mathf.InverseLerp(0f, 1f, buoyancy.submergedFraction);
		if (offAxisDrag > 0f)
		{
			float value2 = Vector3.Dot(base.transform.forward, rigidBody.linearVelocity.normalized);
			float num4 = Mathf.InverseLerp(0.98f, 0.92f, value2);
			rigidBody.linearDamping += num4 * offAxisDrag * buoyancy.submergedFraction;
		}
	}
	...
	public override bool MountEligable(BasePlayer player)
	{
		...
		if (rigidBody.linearVelocity.magnitude >= 5f && HasDriver())
		{
			return false;
		}
