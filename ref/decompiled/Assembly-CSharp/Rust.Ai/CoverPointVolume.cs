...
public class CoverPointVolume : MonoBehaviour, IServerComponent
{
	...
	public void GenerateCoverPoints(Transform coverPointGroup)
	{
		...
		if (CoverPoints.Count != 0 || !RustNavMeshHelpers.SamplePosition(base.transform.position, out var hitWS, base.transform.localScale.y * cover_point_sample_step_height, -1))
		{
			return;
		}
		...
		for (float num = position.x - vector.x + 1f; num < position.x + vector.x - 1f; num += cover_point_sample_step_size)
		{
			for (float num2 = position.z - vector.z + 1f; num2 < position.z + vector.z - 1f; num2 += cover_point_sample_step_size)
			{
				...
				{
					if (!NavMesh.FindClosestEdge(new Vector3(num, num3, num2), out var hit, hitWS.mask))
					{
						continue;
					}
					hit.position = new Vector3(hit.position.x, hit.position.y + 0.5f, hit.position.z);
					bool flag = true;
					foreach (CoverPoint coverPoint2 in CoverPoints)
					{
						if ((coverPoint2.Position - hit.position).sqrMagnitude < cover_point_sample_step_size * cover_point_sample_step_size)
						{
							flag = false;
							break;
						}
					...
					if (flag)
					{
						CoverPoint coverPoint = CalculateCoverPoint(hit);
						if (coverPoint != null)
						{
