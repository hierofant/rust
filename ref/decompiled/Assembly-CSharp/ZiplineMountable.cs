...
public class ZiplineMountable : BaseMountable
{
	...
	public Vector3 GetLineEndPoint(bool applyDismountOffset = false)
	{
		if (applyDismountOffset && linePoints != null)
		{
			...
			Vector3 vector = list[list.Count - 2];
			List<Vector3> list2 = linePoints;
			Vector3 normalized = (vector - list2[list2.Count - 1]).normalized;
	...
	private void Update()
	{
		...
		GamePhysics.CapsuleSweep(position, position2, 0.5f, base.transform.forward, num, obj, 1084293377);
		...
		if (Vector3.Distance(lastSafePosition, base.transform.position) > 0.5f && GetDismountPosition(GetMounted(), out var _, silent: true))
		{
	...
	public override void PlayerServerInput(InputState inputState, BasePlayer player)
	{
		...
		flagsUpdateScope.Set(Flags.Reserved1, additiveValue > 0.5f);
	}
