...
public class MLRS : BaseMountable
{
	...
	public static readonly Translate.Phrase RaidWindowBlockedToast = new Translate.Phrase("mlrs.raidwindowblocked", "The MLRS can only be fired during the raid window");
	...
	{
		using (TimeWarning.New("MLRS.VehicleFixedUpdate"))
		{
			...
			if (IsBroken())
			{
				if (!((float)timeSinceBroken >= brokenDownMinutes * 60f))
				{
					...
					flagsUpdateScope.Set(Flags.Reserved8, TryGetAimingModule(out var _));
					return;
				}
				SetRepaired();
			}
			...
			if (!IsFiringRockets)
			{
				...
				if (IsRealigning)
				{
					...
					Ballistics.TryGetPhysicsProjectileHitPos(firingPoint.position, firingPoint.forward, rocketSpeed, UnityEngine.Physics.gravity.y * CurGravityMultiplier, out var result, 2f, 0.66f, 128f, this);
					TrueHitPos = result;
				}
			}
	...
	{
		if (Softcore.raidwindow_enabled && Softcore.raidwindow_block_mlrs && BaseGameMode.GetActiveGameMode(serverside: true) is GameModeSoftcore && !RaidWindow.IsWindowOpenNow())
		{
			if (owner != null)
			{
				owner.ShowToast(GameTip.Styles.Error, RaidWindowBlockedToast, false);
			}
			return;
		}
		UpdateStorageState();
