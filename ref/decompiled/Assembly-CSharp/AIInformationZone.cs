...
public class AIInformationZone : BaseMonoBehaviour, IServerComponent
{
	...
	[NonSerialized]
	public List<WakeAIZ> wakeZones = new List<WakeAIZ>();
	...
	private static RustNavMeshPath pathCache = new RustNavMeshPath();
	...
	private void SetMovePointsY(int layerMask, float yStartOffset, float yDistance, float localYDefault)
	{
		foreach (AIMovePoint movePoint in movePoints)
		{
			if (!(movePoint == null))
			{
				if (UnityEngine.Physics.Raycast(movePoint.transform.position + Vector3.up * yStartOffset, Vector3.down, out var hitInfo, yDistance, layerMask))
				{
					movePoint.transform.position = hitInfo.point + Vector3.up * 0.1f;
				}
				else
				{
					movePoint.transform.localPosition = new Vector3(movePoint.transform.localPosition.x, localYDefault, movePoint.transform.localPosition.z);
				}
			}
		}
	}
	...
	private bool ProcessDistances()
	{
		...
		while (PassesBudget(realtimeSinceStartup, budgetSeconds))
		{
			...
			foreach (AICoverPoint coverPoint in coverPoints)
			{
				...
				else if (RustNavMeshHelpers.CalculatePath(aIMovePoint.transform.position, coverPoint.transform.position, areaMask, pathCache) && pathCache.status == NavMeshPathStatus.PathComplete)
				{
					int count = pathCache.corners.Count;
					if (count > 1)
					{
						Vector3 a = pathCache.corners[0].Value;
						float num5 = 0f;
						for (int i = 0; i < count; i++)
						{
							Vector3 value = pathCache.corners[i].Value;
							num5 += Vector3.Distance(a, value);
							a = value;
						}
						num4 = num5;
						pathSuccesses++;
	...
	{
		if (targetEntity == null || fromEntity == null)
		{
			return null;
		}
		...
		int pointCount;
		AIPoint[] movePointsInRange = GetMovePointsInRange(targetEntity.transform.position, maxRange, out pointCount);
		...
		for (int i = 0; i < pointCount; i++)
		{
			...
			if (!aIPoint3.transform.parent.gameObject.activeSelf || (!(fromEntity.transform.position.y < WaterSystem.OceanLevel) && aIPoint3.transform.position.y < WaterSystem.OceanLevel))
			{
				continue;
			}
			float num3 = 0f;
			Vector3 position = aIPoint3.transform.position;
			float num4 = Vector3.Distance(targetEntity.transform.position, position);
			if (num4 < num2)
			{
				aIPoint2 = aIPoint3;
				num2 = num4;
			...
			if (checkLOS)
			{
				...
				bool flag;
				if (!UseSimpleLOSCheck && basePlayer != null && humanNPC != null)
				{
					Vector3 fromOffset = position - humanNPC.transform.position;
					flag = humanNPC.CanSeeTarget(basePlayer, fromOffset);
				}
				else
				{
					flag = !UnityEngine.Physics.Linecast(targetEntity.transform.position + Vector3.up, position + Vector3.up, 1218519297, QueryTriggerInteraction.Ignore);
				}
				if (!flag)
				{
					continue;
		...
		if (aIPoint == null && returnRandom)
		{
			return movePointsInRange[UnityEngine.Random.Range(0, movePointsInRange.Length)] as AIMovePoint;
		}
