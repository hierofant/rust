...
public static class FileEx
{
	...
	public static NativeArray<byte> ReadAllBytesNative(string fileName, Allocator allocator)
	{
		...
		{
			throw new Exception($"File is too large! ({length} > 2GB)");
		}
		NativeMemoryStream nativeMemoryStream = new NativeMemoryStream((int)length, allocator, disposeArray: false);
		File.OpenRead(fileName).CopyTo(nativeMemoryStream);
		NativeArray<byte> buffer = nativeMemoryStream.Buffer;
		nativeMemoryStream.Dispose();
		return buffer;
	}
}
