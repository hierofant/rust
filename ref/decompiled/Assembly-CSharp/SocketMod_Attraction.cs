	...
	public override void ModifyPlacement(ref Construction.Placement place)
	{
		...
		foreach (BaseEntity item in obj)
		{
			...
			foreach (AttractionPoint attractionPoint in array2)
			{
				...
				Quaternion b = ((lockRotation && bypassPlayerRotation) ? (item.transform.rotation * attractionPoint.localRotation) : QuaternionEx.LookRotationWithOffset(worldPosition, vector2 - place.position, Vector3.up));
				...
				if (lockRotation)
				{
					...
					{
						place.rotation = item.transform.rotation * attractionPoint.localRotation;
					}
					else
					{
