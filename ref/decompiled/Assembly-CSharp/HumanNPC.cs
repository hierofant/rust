...
public class HumanNPC : NPCPlayer, IAISenses, IAIAttack, IThinker
{
	...
	{
		if (newAim == Vector3.zero || ObjectEx.IsUnityNull(base.eyes) || ObjectEx.IsUnityNull(base.transform))
		{
			return;
		}
	...
	public override void AttackerInfo(PlayerLifeStory.DeathInfo info)
	{
		...
		{
			BaseMountable baseMountable = GetMounted();
			if (baseMountable != null && baseMountable is MountedWeaponSeat { Owner: var owner })
			{
				info.inflictorName = owner.WeaponDef.shortname;
			}
		}
		else
		{
