...
public class PhoneController : EntityComponent<BaseEntity>
{
	...
	private void UpdateServerPlayer(BasePlayer newPlayer)
	{
		if (!(currentPlayer == newPlayer))
		{
			...
			using (BaseEntity.FlagsUpdateScope flagsUpdateScope = base.baseEntity.StartSetFlags(BaseEntity.FlagsUpdateMode.SendNetworkUpdate))
			{
				flagsUpdateScope.Set(BaseEntity.Flags.Busy, currentPlayer != null);
			}
			if (currentPlayer != null)
			{
	...
	{
		serverState = state;
		base.baseEntity.ClientRPC(RpcTarget.NetworkGroup("SetClientState"), (int)serverState, (activeCallTo != null) ? activeCallTo.PhoneNumber : 0);
	...
	{
		base.baseEntity.ClientRPC(RpcTarget.SendInfo("OnReceivedVoice", new SendInfo(BaseNetworkable.GetConnectionsWithin(base.transform.position, 15f))
		{
	...
	private void InitSavedNumbers()
	{
		...
		{
			savedNumbers.entries = Pool.Get<List<PhoneDirectory.DirectoryEntry>>();
		}
	}
	...
	public void Server_RequestPhoneDirectory(BaseEntity.RPCMessage msg)
	{
		...
		base.baseEntity.ClientRPC(RpcTarget.NetworkGroup("ReceivePhoneDirectory"), phoneDirectory);
	}
	...
	public void AddSavedNumber(int number, string phoneName)
	{
		...
		if (IsSavedContactValid(phoneName, number))
		{
			...
			directoryEntry.phoneName = phoneName;
			directoryEntry.phoneNumber = number;
	...
	public void ServerSendVoicemail(BaseEntity.RPCMessage msg)
	{
		if (!(msg.player == null))
		{
			...
			if (!(telephone == null) && !(telephone.cachedCassette == null) && Cassette.IsOggValid(data, telephone.cachedCassette))
			{
	...
	public void SaveVoicemail(byte[] data, string playerName)
	{
		...
		{
			savedVoicemail = Pool.Get<List<ProtoBuf.VoicemailEntry>>();
		}
	...
	public void ServerDeleteVoicemail(BaseEntity.RPCMessage msg)
	{
		...
		for (int i = 0; i < savedVoicemail.Count; i++)
		{
			...
			{
				ProtoBuf.VoicemailEntry voicemailEntry = savedVoicemail[i];
	...
	public void DeleteAllVoicemail()
	{
		...
		Pool.Free(ref savedVoicemail, freeElements: false);
	}
