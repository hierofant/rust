...
public class Item : Facepunch.Pool.IPooled
{
	...
	public int despawnMultiplier
	{
		get
		{
			...
			{
				rarity = info.isRedirectOf.despawnRarity;
			}
			if (rarity == Rarity.None)
			{
				...
				if (info.isRedirectOf != null)
				{
					rarity = info.isRedirectOf.rarity;
				}
			}
			if (!(info != null))
			{
	...
	public virtual void OnBroken()
	{
		...
		else if (parent != null && parent.HasFlag(ItemContainer.Flag.NoBrokenItems))
		{
			...
			else
			{
				...
				if (playerOwner2 != null && !MoveToContainer(playerOwner2.inventory.containerMain))
				{
					Drop(playerOwner2.transform.position, playerOwner2.eyes.BodyForward() * 1.5f);
				}
			}
		}
	...
	public void AddItemOwnership(BasePlayer player, Translate.Phrase reason)
	{
		AddItemOwnership((player != null && !player.IsDestroyed) ? player.displayName : "", reason.token, amount);
	}
	...
	public void SetParent(ItemContainer target)
	{
		...
		if (parent != null)
		{
			...
			do
			{
				...
				if (entityOwner != null && !entityOwner.enableSaving)
				{
					flag = true;
					break;
				}
	...
	public bool CanMoveTo(BasePlayer player, ItemContainer newcontainer, int iTargetPos = -1)
	{
		...
		if (newcontainer.CanAcceptItem(player, this, iTargetPos) != 0)
		{
			return false;
		}
	...
	{
		if (a.instanceData != null && b.instanceData != null && a.info.TryGetComponent<ItemModFoodSpoiling>(out var component))
		{
			bool flag = false;
			...
			if (!flag)
			{
				float num = component.TotalSpoilTimeHours * 60f * 60f;
			...
			if (!flag && b.parent != null && b.parent.entityOwner != null && b.parent.entityOwner is CookingWorkbench)
			{
				flag = true;
			}
			if (!flag)
	...
	public virtual ProtoBuf.Item Save(bool bIncludeContainer = false, bool bIncludeOwners = true)
	{
		...
		if (heldEntity.uid.IsValid)
		{
			...
			{
				item.ammoCount = chainsaw.ammo + 1;
			}
			else if (baseEntity is FlameThrower flameThrower)
			{
				item.ammoCount = flameThrower.ammo + 1;
			}
		}
		if (hasCondition)
		{
	...
	public virtual void Load(ProtoBuf.Item load)
	{
		...
		if (info == null)
		{
			Debug.LogError($"Load invalid item id {load.itemid} from item {load.UID} (no ItemDefinition found)");
			return;
		}
		uid = load.UID;
		name = load.name;
