...
public class ModularCarGarage : ContainerIOEntity
{
	...
	public override bool OnRpcMessage(BasePlayer player, uint rpc, Message msg)
	{
		using (TimeWarning.New("ModularCarGarage.OnRpcMessage"))
		{
			...
			if (rpc == 613695921 && player != null)
			{
				Assert.IsTrue(player.isServer, "SV_RPC Message is using a clientside player!");
				if (Global.developer > 2)
				{
					Debug.Log("SV_RPCMessage: " + player?.ToString() + " - Server_RequestFuelExtract");
				}
	...
	public void RPC_RepairItem(RPCMessage msg)
	{
		...
		if (vehicleItem != null)
		{
			if (!ComponentEx.HasComponent<ItemModVehicleChassis>(vehicleItem.info) && !ComponentEx.HasComponent<ItemModVehicleModule>(vehicleItem.info))
			{
				Debug.Log($"Can't repair {vehicleItem} as it's not a car part");
				return;
			}
