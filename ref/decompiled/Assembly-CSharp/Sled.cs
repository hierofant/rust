...
public class Sled : BaseVehicle, INotifyTrigger
{
	...
	public PhysicsMaterial BrakeMaterial;

	public PhysicsMaterial SnowMaterial;

	public PhysicsMaterial NonSnowMaterial;
	...
	public PhysicsMaterial cachedMaterial;
	...
	{
		using (TimeWarning.New("Sled.VehicleFixedUpdate"))
		{
	...
	public void UpdatePhysicsMaterial()
	{
		...
		using FlagsUpdateScope flagsUpdateScope = StartSetFlags(FlagsUpdateMode.SendNetworkUpdate);
		flagsUpdateScope.Set(Flags.Reserved2, terrainHandler.IsOnSnowOrIce);
		flagsUpdateScope.Set(Flags.Reserved4, terrainHandler.OnSurface == VehicleTerrainHandler.Surface.Sand);
	}
	...
	public void UpdateGroundedFlag()
	{
		...
		using FlagsUpdateScope flagsUpdateScope = StartSetFlags(FlagsUpdateMode.SendNetworkUpdate);
		flagsUpdateScope.Set(Flags.Reserved3, terrainHandler.IsGrounded);
	}
	...
	public void ApplyInitialForce()
	{
		...
		if (initialForceScale >= InitialForceCutoff && (rigidBody.linearVelocity.magnitude > 1f || !terrainHandler.IsOnSnowOrIce))
		{
	...
	public override void PlayerServerInput(InputState inputState, BasePlayer player)
	{
		...
		if (inputState.IsDown(BUTTON.FORWARD) && (float)lastNudge > NudgeCooldown && rigidBody.linearVelocity.magnitude < MaxNudgeVelocity)
		{
			rigidBody.WakeUp();
		...
		if (num != 0f)
		{
			base.transform.Rotate(Vector3.up * num * Time.deltaTime * linearVelocity.magnitude, Space.Self);
		}
		if (terrainHandler.IsGrounded && Vector3.Dot(rigidBody.linearVelocity.normalized, base.transform.forward) >= 0.5f)
		{
			rigidBody.linearVelocity = Vector3.Lerp(rigidBody.linearVelocity, base.transform.forward * linearVelocity.magnitude, Time.deltaTime * DirectionMatchForce);
		}
	}
