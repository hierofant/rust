...
public static class GameObjectEx
{
	...
	{
		return ToBaseEntity(go.transform, allowDestroyed);
	}

	public static BaseEntity ToBaseEntity(this Collider collider, bool allowDestroyed = false)
	{
		return ToBaseEntity(collider.transform, allowDestroyed);
	}
	...
	public static bool IsOnLayers(this GameObject go, int layerMask)
	{
		if (go != null)
		{
	...
	public static GameObject FindInChildren(this GameObject parent, string name)
	{
		...
		{
			GameObject gameObject = FindInChildren(item.gameObject, name);
			if (gameObject != null)
			{
				return gameObject;
			}
