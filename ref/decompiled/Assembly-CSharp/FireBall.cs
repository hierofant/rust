...
public class FireBall : BaseEntity, ISplashable
{
	...
	public void DoRadialDamage()
	{
		...
		Vis.Entities(position, radius, obj, (int)AttackLayers | 0x8000, QueryTriggerInteraction.Ignore);
		...
		{
			if (!(item == null) && item.isServer && item.IsAlive() && (!ignoreNPC || !item.IsNpc) && (!(item is BaseVehicle) || item is BaseSiegeWeapon) && item.IsVisible(position))
			{
