...
public class Kayak : BaseBoat, IPoolVehicle
{
	...
	public override void ServerInit()
	{
		...
		InvokeRandomized(LowPriorityCheck, 0f, 5f, 2.5f);
	}

	public override void OnDeployed(BaseEntity parent, BasePlayer deployedBy, Item fromItem)
	{
		base.OnDeployed(parent, deployedBy, fromItem);
		buoyancy.wavesEffect = ((deployedBy != null && deployedBy.IsInTutorial) ? 0f : 1f);
	}
	...
	public override void DriverInput(InputState inputState, BasePlayer player)
	{
		...
		if (flag2)
		{
			...
			if (!flag)
			{
				...
				rigidBody.linearVelocity = Vector3.Lerp(linearVelocity, vector * linearVelocity.magnitude, 0.4f);
			}
		}
	}
	...
	public override void DoPushAction(BasePlayer player)
	{
		...
		else
		{
			...
			rigidBody.AddForceAtPosition(vector * num * rigidBody.mass, position, ForceMode.Impulse);
		}
		if (IsInWater())
		{
	...
	{
		using (TimeWarning.New("Kayak.VehicleFixedUpdate"))
		{
			...
			if (inCinematic)
			{
				...
				{
					Vector3 linearVelocity = rigidBody.linearVelocity;
					linearVelocity = Vector3.MoveTowards(linearVelocity, linearVelocity.WithX(0f).WithZ(0f), Time.fixedDeltaTime);
					rigidBody.linearVelocity = linearVelocity;
				}
				base.transform.rotation = Quaternion.RotateTowards(base.transform.rotation, cinematicWorldRotation, 180f * Time.fixedDeltaTime);
			}
			rigidBody.linearDamping = fixedDragUpdate.Get(force: false);
		}
	}
