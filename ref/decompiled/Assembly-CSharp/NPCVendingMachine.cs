...
public class NPCVendingMachine : VendingMachine
{
	...
	public override EraRestriction CurrentEraRestriction => EraRestriction.Vending;
	...
	public virtual void InstallFromVendingOrders()
	{
		if (alternativeVendingOrders != null && alternativeVendingOrders.Length != 0)
		{
			...
			foreach (NPCVendingOrder nPCVendingOrder in array)
			{
				...
				{
					if (allowedEras[j] == ConVar.Server.Era)
					{
						vendingOrders = nPCVendingOrder;
						break;
					}
	...
	public void Refill()
	{
		...
		for (int i = 0; i < vendingOrders.orders.Length; i++)
		{
			...
			int num = 0;
			num = ((!entry.sellItemAsBP) ? Mathf.FloorToInt(base.inventory.GetAmount(entry.sellItem.itemid) / entry.sellItemAmount) : Mathf.FloorToInt(base.inventory.GetAmount(base.blueprintBaseDef.itemid, entry.sellItem.itemid) / entry.sellItemAmount));
	...
	public static void RefreshDeepSeaMapMarkers()
	{
		foreach (NPCVendingMachine allNpcVendingMachine in allNpcVendingMachines)
		{
			if (!(allNpcVendingMachine == null) && allNpcVendingMachine.IsInDeepSeaCached)
			{
				allNpcVendingMachine.UpdateMapMarker();
			}
		}
	}
