...
public class BaseAIBrain : EntityComponent<BaseEntity>, IPet, IAISleepable, global::IAIDesign, IAIGroupable, IAIEventListener
{
		...
		using (TimeWarning.New("BaseAIBrain.OnRpcMessage"))
		{
			if (rpc == 66191493 && player != null)
			{
				...
				using (TimeWarning.New("RequestAIDesign"))
				{
					...
					{
						if (!BaseEntity.RPC_Server.CallsPerSecond.Test(66191493u, "RequestAIDesign", GetBaseEntity(), player, 5uL))
						{
							return true;
						}
						if (!BaseEntity.RPC_Server.IsVisible.Test(66191493u, "RequestAIDesign", GetBaseEntity(), player, 3f))
						{
							return true;
						}
						if (!BaseEntity.RPC_Server.MaxDistance.Test(66191493u, "RequestAIDesign", GetBaseEntity(), player, 3f))
						{
							return true;
						}
					}
	...
	public void SetThinkMode(AIThinkMode mode)
	{
		ThinkMode = mode;
	}
	...
	private bool PlayerCanDesignAI(BasePlayer player)
	{
		...
		if (DesigningPlayer != null && DesigningPlayer != player)
		{
			return false;
		}
	...
	{
		if (!UseAIDesign || msg.player == null || !PlayerCanDesignAI(msg.player))
		{
			return;
		}
		using ProtoBuf.AIDesign aIDesign = msg.read.Proto<ProtoBuf.AIDesign>();
	...
	public void SaveDesign()
	{
		...
		case AIDesignScope.Default:
			...
			try
			{
				...
				{
					aIDesign.WriteToStream(stream2);
				}
	...
	private void StartMovementTick()
	{
		...
		InvokeRandomized(TickMovement, MovementTickStartDelay, 0.1f, 0.010000001f);
	}
