...
public class FLookAnimator : MonoBehaviour, IDropHandler, IEventSystemHandler, IFHierarchyIcon, IClientComponent
{
	...
	public static void TickAll()
	{
		for (int num = components.Count - 1; num >= 0; num--)
		{
			...
			if (ObjectEx.IsUnityNull(fLookAnimator))
			{
	...
	public Vector2 LimitAnglesCalculations(Vector2 angles)
	{
		...
		else
		{
			...
			{
				angles = Vector3.MoveTowards((Vector3)angles, (Vector3)Vector2.zero, 2.75f + RotationSpeed);
				return angles;
			}
	...
	public Vector2 AnimateAnglesTowards(Vector2 angles)
	{
		...
		if (!instantRotation)
		{
			...
			case EFAnimationStyle.FastLerp:
			{
				...
				num /= smoothingEffect;
				Vector3 a = Vector3.Lerp(animatedLookAngles, (Vector3)angles, num * delta);
				if (MaxRotationSpeed < 2.5f)
				{
					...
					a = Vector3.Lerp(animatedLookAngles, (Vector3)angles, num * delta);
				}
				animatedLookAngles = a;
				break;
			}
			case EFAnimationStyle.Linear:
				animatedLookAngles = Vector3.MoveTowards(animatedLookAngles, (Vector3)angles, delta * (0.2f + RotationSpeed) * 300f);
				break;
			}
		}
		else
	...
	private void Tick()
	{
		using (TimeWarning.New("FLookAnimator:Tick"))
		{
			if (!initialized)
			{
				if (StartAfterTPose)
	...
	{
		using (TimeWarning.New("FLookAnimator:FixedTick"))
		{
	...
	{
		using (TimeWarning.New("FLookAnimator:LateTick"))
		{
