...
public class Ragdoll : EntityComponent<BaseEntity>, IPrefabPreProcess
{
	...
	private void SetUpPhysics(bool isServer)
	{
		...
		foreach (Rigidbody rigidbody in rigidbodies)
		{
			...
			if (rigidbody.linearVelocity.sqrMagnitude < 0.25f)
			{
				rigidbody.linearVelocity += UnityEngine.Random.onUnitSphere * 2.5f;
			}
			if (rigidbody.angularVelocity.sqrMagnitude < 0.25f)
			{
				rigidbody.angularVelocity += UnityEngine.Random.onUnitSphere * 2.5f;
			}
		}
	}
	...
	public void PreProcess(IPrefabProcessor preProcess, GameObject rootObj, string name, bool serverside, bool clientside, bool bundling)
	{
		...
		foreach (Joint joint in joints)
		{
			UnityEngine.Object.DestroyImmediate(joint, allowDestroyingAssets: true);
		}
		foreach (Rigidbody rigidbody in rigidbodies)
		{
			UnityEngine.Object.DestroyImmediate(rigidbody, allowDestroyingAssets: true);
		}
	}
	...
	public virtual void ServerInit()
	{
		...
		if (!CollectionEx.IsEmpty(rbTransforms))
		{
			...
			for (int i = 0; i < rbTransforms.Count; i++)
			{
				rbTransformHandles[i] = rbTransforms[i].transformHandle;
			}
		}
	}
	...
	private void SyncJointsToClients()
	{
		...
		BaseNetworkable.ThreadSafeTime threadSafeTime = default(BaseNetworkable.ThreadSafeTime);
		threadSafeTime.Time = UnityEngine.Time.time;
		threadSafeTime.FixedTime = UnityEngine.Time.fixedTime;
		BaseNetworkable.ThreadSafeTime time = threadSafeTime;
		SetRagdollMessageVals(ragdoll, in time);
	...
	private void SetRagdollMessageVals(ProtoBuf.Ragdoll ragdollMsg, in BaseNetworkable.ThreadSafeTime time)
	{
