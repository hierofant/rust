...
public class WeaponRack : StorageContainer
{
		...
		using (TimeWarning.New("WeaponRack.OnRpcMessage"))
		{
			if (rpc == 1682065633 && player != null)
			{
				...
				using (TimeWarning.New("LoadWeaponAmmo"))
				{
					...
					{
						if (!RPC_Server.CallsPerSecond.Test(1682065633u, "LoadWeaponAmmo", this, player, 5uL))
						{
							return true;
						}
						if (!RPC_Server.IsVisible.Test(1682065633u, "LoadWeaponAmmo", this, player, 3f))
						{
							return true;
						}
						if (!RPC_Server.MaxDistance.Test(1682065633u, "LoadWeaponAmmo", this, player, 2f))
						{
							return true;
						}
					}
			...
			if (rpc == 879556270 && player != null)
			{
				Assert.IsTrue(player.isServer, "SV_RPC Message is using a clientside player!");
				if (ConVar.Global.developer > 2)
				{
					Debug.Log("SV_RPCMessage: " + player?.ToString() + " - ReqMountBackSlot");
				}
				using (TimeWarning.New("ReqMountBackSlot"))
				{
					using (TimeWarning.New("Conditions"))
					{
						if (!RPC_Server.CallsPerSecond.Test(879556270u, "ReqMountBackSlot", this, player, 5uL))
						{
							return true;
						}
			...
			if (rpc == 2640584497u && player != null)
			{
				...
				using (TimeWarning.New("ReqMountWeapon"))
				{
					using (TimeWarning.New("Conditions"))
					{
						if (!RPC_Server.CallsPerSecond.Test(2640584497u, "ReqMountWeapon", this, player, 5uL))
						{
							return true;
						}
			...
			if (rpc == 2753286621u && player != null)
			{
				...
				using (TimeWarning.New("ReqSwapWeapon"))
				{
					using (TimeWarning.New("Conditions"))
					{
						if (!RPC_Server.CallsPerSecond.Test(2753286621u, "ReqSwapWeapon", this, player, 5uL))
						{
							return true;
						}
			...
			if (rpc == 3761066327u && player != null)
			{
				...
				using (TimeWarning.New("ReqTakeAll"))
				{
					using (TimeWarning.New("Conditions"))
					{
						if (!RPC_Server.CallsPerSecond.Test(3761066327u, "ReqTakeAll", this, player, 5uL))
						{
							return true;
						}
			...
			if (rpc == 1987971716 && player != null)
			{
				...
				using (TimeWarning.New("ReqTakeWeapon"))
				{
					using (TimeWarning.New("Conditions"))
					{
						if (!RPC_Server.CallsPerSecond.Test(1987971716u, "ReqTakeWeapon", this, player, 5uL))
						{
							return true;
						}
			...
			if (rpc == 3314206579u && player != null)
			{
				...
				using (TimeWarning.New("ReqUnloadWeapon"))
				{
					using (TimeWarning.New("Conditions"))
					{
						if (!RPC_Server.CallsPerSecond.Test(3314206579u, "ReqUnloadWeapon", this, player, 5uL))
						{
							return true;
						}
		...
		if (!added)
		{
			...
			foreach (WeaponRackSlot weaponRackSlot in array)
			{
				if (weaponRackSlot != null && weaponRackSlot.Used)
				{
					...
					if (itemDef == null || !itemDef.IsAllowed(EraRestriction.Default))
					{
	...
	private bool InventoryItemFilter(BasePlayer player, Item item, int targetSlot)
	{
		if (item == null)
		{
			return false;
	...
	public override void Save(SaveInfo info)
	{
		...
		info.msg.weaponRack.items = Facepunch.Pool.Get<List<WeaponRackItem>>();
		WeaponRackSlot[] array = gridSlots;
		foreach (WeaponRackSlot weaponRackSlot in array)
		{
	...
	private void MountBackSlot(BasePlayer player, int gridCellIndex, int rotation)
	{
		if (!(player == null))
		{
			...
			if (anyBackpack != null && (!(anyBackpack.info.GetComponent<ItemModBackpack>() != null) || anyBackpack.contents == null || anyBackpack.contents.itemList == null || anyBackpack.contents.itemList.Count <= 0))
			{
	...
	private void LoadWeaponAmmo(RPCMessage msg)
	{
		...
		if (!(component2 == null) && component2.IsAmmo(component.primaryMagazine.definition.ammoTypes))
		{
			...
			{
				player.GiveItem(ItemManager.CreateByItemID(component.primaryMagazine.ammoType.itemid, component.primaryMagazine.contents, 0uL, 0uL));
