...
public class MiningQuarry : BaseResourceExtractor
{
	...
	{
		SetFlagLocal(Flags.On, isOn);
		engineSwitchPrefab.instance.SetFlagLocal(Flags.On, isOn);
		SendNetworkUpdate();
	...
	public override void ServerInit()
	{
		...
		SpawnChildEntities();
		using (FlagsUpdateScope flagsUpdateScope = engineSwitchPrefab.instance.StartSetFlags(FlagsUpdateMode.SendNetworkUpdate))
		{
	...
	public void ProcessResources()
	{
		...
		foreach (ResourceDepositManager.ResourceDeposit.ResourceDepositEntry resource in _linkedDeposit._resources)
		{
			...
			if (num2 != num3)
			{
				...
				{
					num4 = Mathf.Max(1, Mathf.RoundToInt((float)num4 * GameModeSoftcore.gather_rate));
				}
				Item item = ItemManager.Create(resource.type, num4, 0uL, isServerSide: true, 0uL);
