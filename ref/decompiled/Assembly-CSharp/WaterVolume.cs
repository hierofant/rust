...
public class WaterVolume : TriggerBase
{
	...
	{
		WaterVolumeBurstData result = default(WaterVolumeBurstData);
		result.bounds = cachedBounds;
		result.naturalSource = naturalSource;
		result.cutOffPlaneMatrices = new NativeArray<Matrix4x4>(cutOffPlanes.Length, allocator);
		result.cutOffPlanePoses = new NativeArray<Pose>(cutOffPlanes.Length, allocator);
		for (int i = 0; i < cutOffPlanes.Length; i++)
		{
			if (cutOffPlanes[i] != null)
			{
				result.cutOffPlaneMatrices[i] = cutOffPlanes[i].localToWorldMatrix;
				cutOffPlanes[i].GetPositionAndRotation(out var position, out var rotation);
				result.cutOffPlanePoses[i] = new Pose(position, rotation);
			}
			else
			{
	...
	protected override void Awake()
	{
		...
		{
			WaterVolumeBoundsGrid.Data = new NativeGrid<WaterVolumeBurstData>(Allocator.Persistent);
		}
	}
	...
	private void OnEnable()
	{
		...
		instanceBurstData = AllocateBurstData(Allocator.Persistent);
		WaterVolumeBoundsGrid.Data.AddUnique(instanceBurstData.Value, base.transform.position.x, base.transform.position.z);
	}
