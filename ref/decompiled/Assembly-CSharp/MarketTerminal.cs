...
{
	private static readonly Translate.Phrase _cantFitPhrase = new Translate.Phrase("marketterminal.cantfit", "This drone marketplace is full. Can't process sale.");

	private static readonly Translate.Phrase _noPowerPhrase = new Translate.Phrase("marketterminal.nopower", "This drone marketplace is out of power. Can't process sale.");
	...
	private void RegisterOrder(BasePlayer player, VendingMachine vendingMachine)
	{
		...
		{
			pendingOrders = Facepunch.Pool.Get<List<ProtoBuf.MarketTerminal.PendingOrder>>();
		}
	...
	private void CheckForExpiredOrders()
	{
		if (pendingOrders != null && pendingOrders.Count > 0)
		{
			...
			for (int i = 0; i < pendingOrders.Count; i++)
			{
				...
				else if (!num.HasValue || pendingOrder.timeUntilExpiry < num.Value)
				{
	...
	public void Server_TryOpenMarket(RPCMessage msg)
	{
		...
		entityIdList.entityIds = Facepunch.Pool.Get<List<NetworkableId>>();
		GetDeliveryEligibleVendingMachines(entityIdList.entityIds);
		ClientRPC(RpcTarget.Player("Client_OpenMarket", msg.player), entityIdList);
	}
	...
	public override void Save(SaveInfo info)
	{
		...
		info.msg.marketTerminal.orders = Facepunch.Pool.Get<List<ProtoBuf.MarketTerminal.PendingOrder>>();
		if (pendingOrders == null)
		{
			return;
		}
	...
	public bool CanPlayerAffordOrderAndDeliveryFee(BasePlayer player, ProtoBuf.VendingMachine.SellOrder sellOrder, int numberOfTransactions)
	{
		...
		player.inventory.FindItemsByItemID(pooledList, deliveryFeeCurrency.itemid);
		int num = pooledList.Sum((Item i) => i.amount);
