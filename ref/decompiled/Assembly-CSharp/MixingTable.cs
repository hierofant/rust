...
public class MixingTable : StorageContainer
{
	...
	private void TryFillInventoryForRecipe(Recipe recipe, BasePlayer player, int amount)
	{
		...
		for (int i = 0; i < ingredients.Length; i++)
		{
			...
			if (player.inventory.Take(pooledList, recipeIngredient2.Ingredient.itemid, num6) >= num6)
			{
	...
	protected virtual void CreateRecipeItems(Recipe recipe, int quantity)
	{
		...
		for (int i = 0; i < num2; i++)
		{
			...
			if (MixStartingPlayer != null && !MixStartingPlayer.IsDestroyed)
			{
	...
	public override void Save(SaveInfo info)
	{
		...
		else
		{
			...
			info.msg.mixingTable.currentRecipe = ((currentRecipe != null && currentRecipe.ProducedItem != null) ? currentRecipe.ProducedItem.itemid : (-1));
		}
