...
public static class Auth_CentralizedBans
{
	...
	private static bool CheckIfPlayerBanned(ulong steamId, Connection connection, UnityWebRequest request)
	{
		...
		if (request.responseCode == 404)
		{
			...
			if (playerState.chatMuted || playerState.chatMuteExpiryTimestamp != 0.0)
			{
				playerState.chatMuted = false;
		...
		try
		{
			...
			{
				PlayerState playerState2 = SingletonComponent<ServerMgr>.Instance.playerStateManager.Get(steamId);
				playerState2.chatMuted = true;
				if (payloadData.expiryDate > 0)
				{
					playerState2.chatMuteExpiryTimestamp = payloadData.expiryDate;
				}
				else
				{
					playerState2.chatMuteExpiryTimestamp = 0.0;
				}
				return false;
			}
