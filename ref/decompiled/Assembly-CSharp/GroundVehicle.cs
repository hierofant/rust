...
public abstract class GroundVehicle : BaseVehicle, IEngineControllerUser, IEntity, TriggerHurtNotChild.IHurtTriggerUser
{
	...
	public void OnHurtTriggerOccupant(BaseEntity hurtEntity, DamageType damageType, float damageTotal)
	{
		if (!base.isClient && !hurtEntity.IsDestroyed)
		{
			...
			Vector3 vector2 = Mathf.Min(hurtEntity.RealisticMass, RealisticMass) * vector;
			rigidBody.AddForceAtPosition(vector2 * 1.25f, position, ForceMode.Impulse);
			if (secondaryRigidbody != null)
			{
				secondaryRigidbody.AddForceAtPosition(vector2 * 1.25f, position, ForceMode.Impulse);
			}
			QueueCollisionDamage(this, vector2.magnitude * 0.1f / Time.deltaTime);
	...
	protected virtual void ProcessCollision(Collision collision, Rigidbody ourRigidbody)
	{
		...
		if (contact.otherCollider.attachedRigidbody == ourRigidbody)
		{
			baseEntity = GameObjectEx.ToBaseEntity(contact.otherCollider);
		}
		else if (contact.thisCollider.attachedRigidbody == ourRigidbody)
		{
