...
public class GameManifest : ScriptableObject
{
	...
	public static uint CRC
	{
		...
		{
			Load();
			if (loadedManifest?.pooledStrings == null)
			{
				return 0u;
			}
			uint num = 0u;
			for (int i = 0; i < loadedManifest.pooledStrings.Length; i++)
			{
				num ^= loadedManifest.pooledStrings[i].hash;
				num = (num << 13) | (num >> 19);
			}
			return num;
		}
	...
	internal static UnityEngine.Object GUIDToObject(string guid)
	{
		...
		if (@object == null && FileSystem.Backend is AssetBundleBackend assetBundleBackend && assetBundleBackend.IsWaitingForAssetScene(text))
		{
			return null;
		}
	...
	private static string GetAssetStatus()
	{
		...
		if (loadedManifest != null)
		{
			...
			stringBuilder.AppendLine();
			stringBuilder.Append(((Skinnable.All != null) ? Skinnable.All.Length.ToString() : "0") + " skinnable objects");
		}
		else
		{
