...
public class IOEntity : DecayEntity
{
	...
	public class IOSlot
	{
		...
		public void Preserve(ref SlotPreserveInfo preserve)
		{
			preserve.connectedTo = connectedTo.Get();
			preserve.connectedToSlot = connectedToSlot;
			preserve.linePoints = linePoints;
			preserve.slackLevels = slackLevels;
			preserve.lineAnchors = lineAnchors;
			preserve.worldSpaceLineEndRotation = worldSpaceLineEndRotation;
			preserve.originPosition = originPosition;
			preserve.originRotation = originRotation;
			preserve.wireColour = wireColour;
		}

		public void Restore(ref SlotPreserveInfo preserve)
		{
			connectedTo.Set(preserve.connectedTo);
			connectedToSlot = preserve.connectedToSlot;
			linePoints = preserve.linePoints;
			slackLevels = preserve.slackLevels;
			lineAnchors = preserve.lineAnchors;
			worldSpaceLineEndRotation = preserve.worldSpaceLineEndRotation;
			originPosition = preserve.originPosition;
			originRotation = preserve.originRotation;
			wireColour = preserve.wireColour;
		}
	...
	public struct LineAnchor
	{
		...
		{
			WireLineAnchorInfo wireLineAnchorInfo = Facepunch.Pool.Get<WireLineAnchorInfo>();
			wireLineAnchorInfo.parentID = entityRef.Get(serverside: true).net.ID;
			wireLineAnchorInfo.boneName = boneName;
			wireLineAnchorInfo.index = index;
			wireLineAnchorInfo.position = position;
	...
	public struct IOEntityPreserveInfo
	{
		public SlotPreserveInfo[] outputInfos;

		public SlotPreserveInfo[] inputInfos;

		public List<ConnectionPreserveInfo> connectionInfos;
	}

	public struct SlotPreserveInfo
	{
	...
	public struct ConnectionPreserveInfo
	{
		public SlotPreserveInfo slotInfo;
	...
	[ServerVar(Help = "(Generated) Number of IO update passes allowed to back-track and re-process changed entities each tick; higher values improve signal propagation speed")]
	...
	public static bool infiniteIoPower
	{
		...
		set
		{
			...
			{
				if (!(serverEntity is IOEntity iOEntity) || (iOEntity.GetQueueType() != 0 && iOEntity.GetQueueType() != QueueType.Industrial))
				{
					continue;
				}
				if (infiniteIoPower)
	...
	public virtual Translate.Phrase GetDisplayName()
	{
		if (!(sourceItem != null))
		{
	...
	public virtual bool AllowWireConnections()
	{
		if (!allow_on_boats)
		{
			...
			if (BoatBuildingStation.GetStationOverlappingPosition(base.transform.position, base.isServer, 1.5f) != null)
			{
				return false;
			}
		}
	...
	{
		return player.CanBuild(base.transform.position, base.transform.rotation, bounds);
	}
	...
	public bool OutputsConnectedTo(IOEntity entity, int depth, bool defaultReturn = false, int outputSlot = -1)
	{
		if (depth <= 0)
		{
			return defaultReturn;
		}
		int num = ((outputSlot >= 0) ? outputSlot : 0);
		int num2 = ((outputSlot >= 0) ? (outputSlot + 1) : outputs.Length);
		for (int i = num; i < num2; i++)
		{
			IOEntity iOEntity = outputs[i].connectedTo.Get();
			if (iOEntity != null)
			{
				if (iOEntity == entity)
				{
				...
				if (iOEntity.OutputsConnectedTo(entity, depth - 1, defaultReturn))
				{
					return true;
				}
			}
	...
	[ServerVar(ServerAdmin = true, Help = "(Generated) Prints the current state of all IO entity processing queues showing queue type and pending entity counts; admin-only")]
	...
	public override void Save(SaveInfo info)
	{
		...
		foreach (IOSlot iOSlot2 in array)
		{
			...
			{
				iOConnection2.slackLevels = Facepunch.Pool.Get<List<float>>();
				iOConnection2.slackLevels.AddRange(iOSlot2.slackLevels);
			}
	...
	public void NotifyClientsLineChanged()
	{
		ClientRPC(RpcTarget.NetworkGroup("ForceLinesUpdate"));
	}
	...
	public override void Reskin_Preserve(ref SprayCan.ReskinPreserveInfo preserveInfo)
	{
		...
		ref IOEntityPreserveInfo ioEntityPreserve = ref preserveInfo.ioEntityPreserve;
		ioEntityPreserve.connectionInfos = Facepunch.Pool.Get<List<ConnectionPreserveInfo>>();
		ioEntityPreserve.outputInfos = new SlotPreserveInfo[outputs.Length];
		ioEntityPreserve.inputInfos = new SlotPreserveInfo[inputs.Length];
		for (int i = 0; i < outputs.Length; i++)
		{
			IOSlot iOSlot = outputs[i];
			IOEntity iOEntity = iOSlot.connectedTo.Get();
		...
		{
			IOSlot iOSlot2 = inputs[j];
			IOEntity iOEntity2 = iOSlot2.connectedTo.Get();
			if (!(iOEntity2 == null))
			{
	...
	public override void Reskin_Restore(ref SprayCan.ReskinPreserveInfo preserveInfo)
	{
		...
		ref IOEntityPreserveInfo ioEntityPreserve = ref preserveInfo.ioEntityPreserve;
		if (ioEntityPreserve.inputInfos == null || ioEntityPreserve.outputInfos == null)
		{
			return;
		}
		for (int i = 0; i < Mathf.Min(outputs.Length, ioEntityPreserve.outputInfos.Length); i++)
		{
			outputs[i].Restore(ref ioEntityPreserve.outputInfos[i]);
		}
		for (int j = 0; j < Mathf.Min(inputs.Length, ioEntityPreserve.inputInfos.Length); j++)
		{
			if (!(ioEntityPreserve.inputInfos[j].connectedTo == null))
			{
				inputs[j].Restore(ref ioEntityPreserve.inputInfos[j]);
			}
		}
		HashSet<IOEntity> obj = Facepunch.Pool.Get<HashSet<IOEntity>>();
		foreach (ConnectionPreserveInfo connectionInfo in ioEntityPreserve.connectionInfos)
		{
			SlotPreserveInfo preserve = connectionInfo.slotInfo;
			preserve.connectedTo = this;
			if (!(connectionInfo.connectedEntity == null))
			{
				if (connectionInfo.isOutput)
				{
					connectionInfo.connectedEntity.outputs[connectionInfo.index].Restore(ref preserve);
				}
				else
				{
					connectionInfo.connectedEntity.inputs[connectionInfo.index].Restore(ref preserve);
				}
				obj.Add(connectionInfo.connectedEntity);
			}
		}
		SnapLinesToHandlePositions(obj);
		foreach (IOEntity item in obj)
		{
		...
		Facepunch.Pool.FreeUnmanaged(ref obj);
		Facepunch.Pool.FreeUnmanaged(ref ioEntityPreserve.connectionInfos);
	}

	public void SnapLinesToHandlePositions(HashSet<IOEntity> toUpdate)
	{
		IOSlot[] array = outputs;
		foreach (IOSlot iOSlot in array)
		{
			...
			{
				iOSlot.linePoints[^1] = iOSlot.handlePosition;
				toUpdate.Add(this);
			}
		}
		array = inputs;
		foreach (IOSlot iOSlot2 in array)
		{
			if (iOSlot2.IsConnected())
			{
				IOEntity iOEntity = iOSlot2.connectedTo.Get();
				IOSlot obj = iOEntity.outputs[iOSlot2.connectedToSlot];
				Vector3 position = base.transform.TransformPoint(iOSlot2.handlePosition);
				obj.linePoints[0] = iOEntity.transform.InverseTransformPoint(position);
