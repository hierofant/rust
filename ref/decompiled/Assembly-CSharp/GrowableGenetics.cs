...
public static class GrowableGenetics
{
	...
	{
		if (growable.GetPlanter() == null)
		{
			return;
		}
		using PooledList<GrowableEntity> pooledList = Pool.Get<PooledList<GrowableEntity>>();
		foreach (BaseEntity child in growable.GetPlanter().children)
		{
			float num = Vector3.Distance(child.transform.position, growable.transform.position);
			if (child is GrowableEntity item && num <= 1.5f)
			{
				pooledList.Add(item);
			}
		}
		...
		for (int i = 0; i < 6; i++)
		{
			...
			GeneWeighting dominantGeneWeighting = GetDominantGeneWeighting(growable, pooledList, i);
