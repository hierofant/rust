...
public class PlayerEyes : EntityComponent<BasePlayer>
{
	...
	public Vector3 position
	{
		get
		{
			if ((bool)base.baseEntity && base.baseEntity.isMounted)
			{
				...
				return GetMountedPos(base.transform.position, base.transform.rotation);
			}
			return GetUnmountedPos(base.transform.position, base.transform.rotation);
		}
	}
	...
	public Vector3 offset => base.transform.up * (EyeOffset.y + viewOffset.Get().y);
	...
	public Quaternion parentRotation
	{
		get
		{
			if (!base.baseEntity.isMounted)
			{
				...
				{
					return Quaternion.Euler(0f, parent.rotation.eulerAngles.y, 0f);
				}
			}
	...
	{
		if ((bool)base.baseEntity && isMounted)
		{
			Vector3 vector = mounted.EyePositionForPlayer(base.baseEntity, rotation);
			if (vector != Vector3.zero)
			{
				return vector;
			}
	...
	{
		return pos + rot * (EyeOffset + viewOffset) + BodyLeanOffset;
	}
	...
	public Vector3 PositionWithOverride(Vector3 position)
	{
		if ((bool)base.baseEntity && base.baseEntity.isMounted)
		{
			...
			return position + base.transform.up * (EyeOffset.y + viewOffset.Get().y) + BodyLeanOffset;
		}
		return position + base.transform.rotation * (EyeOffset + viewOffset) + BodyLeanOffset;
	}
