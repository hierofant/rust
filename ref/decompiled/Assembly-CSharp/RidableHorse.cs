...
public class RidableHorse : BaseVehicle, IInventoryProvider, IDetector, HitchTrough.IHitchable, TriggerHurtNotChild.IHurtTriggerUser, IAnimalRagdollCollisionReceiver, ITowing, IMedicalToolTarget
{
	...
	public void SetHitch(HitchTrough hitch, HitchTrough.HitchSpot spot)
	{
		...
		using (FlagsUpdateScope flagsUpdateScope = StartSetFlags(FlagsUpdateMode.SendNetworkUpdate))
		{
			flagsUpdateScope.Set(Flags.Reserved3, currentHitch != null);
		}
		if (hitch != null)
		{
	...
	private void MovementsUpdate()
	{
		...
		if (isGrounded || isSwimming)
		{
			...
			if (rigidBody.linearVelocity.magnitude > 0.5f && !isSwimming)
			{
				Vector3 force = -base.transform.up * rigidBody.linearVelocity.magnitude * 20f;
			...
			if (IsSliding)
			{
				...
				float num = Vector3.Dot(rigidBody.linearVelocity, -base.transform.forward);
			...
			Vector3 vector = Vector3.Project(linearVelocity, forward);
			...
			if (!isStanding && !IsSliding && !isSkidding)
			{
				...
				if (throttleInput != 0f && !duckInputDown)
				{
					...
					else if (throttleInput > 0f)
					{
						...
						{
							Vector3 force4 = (forward * currentMaxSpeed - linearVelocity).normalized * (currentAcceleration * 0.2f);
				...
				{
					float num6 = 0.1f;
					if (rigidBody.linearVelocity.sqrMagnitude < num6 * num6)
					{
						rigidBody.linearVelocity = Vector3.zero;
					}
					else
					{
						Vector3 force5 = -linearVelocity.normalized * (rigidBody.mass * (AnyMounted() ? 1.1f : 3f));
			...
			if ((Mathf.Abs(throttleInput) <= 0f && rigidBody.linearVelocity.magnitude < 2.5f && groundAngle < minMaxSlopeAngle.y && !isSwimming && !IsSliding) || (IsLeading && currentSpeed > 6f))
			{
				ApplyHandBrake();
			}
			else
		...
		float num10 = 10000f;
		if (rigidBody.linearVelocity.magnitude > num10)
		{
			rigidBody.linearVelocity = rigidBody.linearVelocity.normalized * num10;
		}
		float num11 = -1.3f;
		float num12 = Vector3.Dot(rigidBody.linearVelocity, base.transform.forward);
		if (num12 <= num11)
		{
	...
	{
		Vector3 force = (Vector3.zero - rigidBody.linearVelocity).normalized * GetCurrentGait().brakingForce * multiplier;
		rigidBody.AddForce(force, ForceMode.Force);
	}
	...
	private bool ShouldSlide()
	{
		...
		{
			return Vector3.Dot(rigidBody.linearVelocity, -base.transform.forward) > 4f;
		}
		return true;
	}
	...
	private void ApplyBuoyancy()
	{
		...
		if (value > 0f)
		{
			...
			Vector3 vector2 = Vector3.Project(rigidBody.linearVelocity, Vector3.up);
			vector2 *= 1f - UnityEngine.Time.fixedDeltaTime * 6f;
			rigidBody.linearVelocity = vector + vector2;
		}
	}
	...
	private void AlignWithNormal(Vector3 normal, bool force = false)
	{
		...
		Quaternion rotation = (force ? quaternion : Quaternion.Slerp(rigidBody.rotation, quaternion, 5f * UnityEngine.Time.fixedDeltaTime));
	...
	public void UpdateGroundNormal()
	{
		using (TimeWarning.New("RidableHorse.UpdateGroundNormal"))
		{
			...
			for (int i = 0; i < groundSampleOffsets.Length; i++)
			{
				...
				if (GamePhysics.Trace(new Ray(origin, Vector3.down), 0f, out var hitInfo, 1.2f, 1503731969, QueryTriggerInteraction.Ignore, this))
				{
	...
	private void AutoAvoidObstacles()
	{
		...
		Vector3 obj = ((rigidBody.linearVelocity.sqrMagnitude > 0.01f) ? rigidBody.linearVelocity.normalized : base.transform.forward);
	...
	public static void Ragdoll(ConsoleSystem.Arg arg)
	{
		...
		List<RidableHorse> obj = Facepunch.Pool.Get<List<RidableHorse>>();
	...
	public void SetLeading(BasePlayer target)
	{
		...
		SetFlagLocal(Flags.Reserved16, target != null);
	...
	internal override void DoServerDestroy()
	{
		...
		ReleaseInventories();
		UnityEngine.Object.Destroy(riderProtection);
	...
	{
		Matrix4x4 localToWorldMatrix = clippingMountCheckCollider.transform.localToWorldMatrix;
		Vector3 vector = localToWorldMatrix.MultiplyPoint3x4(clippingMountCheckCollider.center);
		Vector3 zero = Vector3.zero;
		zero[clippingMountCheckCollider.direction] = 1f;
		zero = localToWorldMatrix.MultiplyVector(zero);
		float num = clippingMountCheckCollider.radius * 0.9f;
		float num2 = 0.5f * clippingMountCheckCollider.height * 0.9f - num;
		Vector3 point = vector + zero * num2;
		Vector3 point2 = vector - zero * num2;
		List<Collider> obj = Facepunch.Pool.Get<List<Collider>>();
		GamePhysics.OverlapCapsule(point, point2, num, obj, 1235583233);
		for (int i = 0; i < obj.Count; i++)
		{
			BaseEntity baseEntity = GameObjectEx.ToBaseEntity(obj[i]);
			if (baseEntity != null && (baseEntity.isClient != base.isClient || baseEntity == this))
			{
				obj.RemoveUnordered(i);
				i--;
			}
		}
	...
	bool IMedicalToolTarget.IsValidMedicalToolItem(ItemDefinition itemDef)
	{
		...
		{
			if (prohibitedMedicalItems[i] == itemDef)
			{
				return false;
			}
		}
	...
	{
		base.Save(info);
		info.msg.horse = Facepunch.Pool.Get<Horse>();
