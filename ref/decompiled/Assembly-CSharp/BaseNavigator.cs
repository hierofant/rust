...
public class BaseNavigator : BaseMonoBehaviour
{
	...
	[InspectorFlags]
	public TerrainBiome.Enum biomePreference = TerrainBiome.Enum.Tundra | TerrainBiome.Enum.Arctic;
	...
	public RustNavMeshAgent Agent { get; private set; }
	...
	public BaseEntity FacingDirectionEntity => facingDirectionEntity;
	...
	public virtual void Init(BaseCombatEntity entity, RustNavMeshAgent agent)
	{
	...
	public void SetNavMeshEnabled(bool flag)
	{
		...
		if (flag)
		{
			Agent.TryEnableInternalUnityAgent();
		}
	...
	public bool PlaceOnNavMesh(float yOffset)
	{
		...
		if (GetNearestNavmeshPosition(base.transform.position + Vector3.up * yOffset, out var position, maxRange))
		{
			flag = Warp(position);
			if (flag)
			{
	...
	public bool SetDestination(Vector3 pos, float speedFraction = 1f, float updateInterval = 0f, float navmeshSampleDistance = 0f)
	{
		...
		default:
		{
			...
			{
				flag = RustNavMeshHelpers.CalculatePath(base.transform.position, Destination, navMeshQueryFilter, path);
				if (flag)
				{
					if (path.status != 0)
					{
