	...
	public static void Save()
	{
		...
		foreach (User item2 in GetAll(UserGroup.Banned))
		{
			if (!(item2.notes == "EAC"))
			{
				...
				sb.Append(' ');
				sb.QuoteSafe(ConsoleSystem.SanitizeConfigValue(item2.username));
				sb.Append(' ');
				sb.QuoteSafe(ConsoleSystem.SanitizeConfigValue(item2.notes));
				sb.Append(' ');
		...
		foreach (User item3 in GetAll(UserGroup.Owner))
		{
			...
			sb.Append(' ');
			sb.QuoteSafe(ConsoleSystem.SanitizeConfigValue(item3.username));
			sb.Append(' ');
			sb.QuoteSafe(ConsoleSystem.SanitizeConfigValue(item3.notes));
			sb.Append("\r\n");
		}
		foreach (User item4 in GetAll(UserGroup.Moderator))
		{
			...
			sb.Append(' ');
			sb.QuoteSafe(ConsoleSystem.SanitizeConfigValue(item4.username));
			sb.Append(' ');
			sb.QuoteSafe(ConsoleSystem.SanitizeConfigValue(item4.notes));
			sb.Append("\r\n");
		}
		foreach (User item5 in GetAll(UserGroup.SkipQueue))
		{
			...
			sb.Append(' ');
			sb.QuoteSafe(ConsoleSystem.SanitizeConfigValue(item5.username));
			sb.Append(' ');
			sb.QuoteSafe(ConsoleSystem.SanitizeConfigValue(item5.notes));
			sb.Append("\r\n");
		}
