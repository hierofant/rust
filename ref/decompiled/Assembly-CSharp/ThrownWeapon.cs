...
public class ThrownWeapon : AttackEntity
{
	...
	public void ServerThrow(Vector3 targetPosition)
	{
		...
		if (ownerPlayer == null || (!canThrowUnderwater && ownerPlayer.IsHeadUnderwater()) || (!canThrowFromHelicopter && ownerPlayer.IsInAHelicopter()))
		{
			return;
		}
	...
	{
		if (eyeDir.IsNaNOrInfinity())
		{
			return false;
		}
		if (throwScale.IsNaNOrInfinity())
		{
			return false;
		}
	...
	public void DoThrowImpl(Vector3 eyePos, Vector3 eyeDir, BasePlayer owningPlayer, out BaseEntity thrownEntity, float throwScale = 1f, Vector3 throwVelocityOverride = default(Vector3), Item ownerItemOverride = null)
	{
		...
		if (!(baseEntity == null))
		{
			...
			{
				baseEntity.gameObject.SendMessage("SetFrequency", item.instanceData.dataInt, SendMessageOptions.DontRequireReceiver);
			}
