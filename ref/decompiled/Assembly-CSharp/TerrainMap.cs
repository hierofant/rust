...
public abstract class TerrainMap<T> : TerrainMap, IDisposable where T : unmanaged
{
	...
	public NativeArray<T>.ReadOnly Data => src.AsReadOnly();

	public void InitArrays(int size)
	{
		src = (dst = new NativeArray<T>(size, Allocator.Persistent));
	}
	...
	public void Push()
	{
		if (!(src != dst))
		{
			...
			dst = new NativeArray<T>(src, Allocator.Persistent);
		}
	}
	...
	public void FromByteArray(byte[] dat)
	{
		...
		int expectedTypeSize = UnsafeUtility.SizeOf<T>();
		NativeArray<byte> array = dst.Reinterpret<byte>(expectedTypeSize);
		if (dat.Length > array.Length)
		{
			throw new ArgumentException("TerrainMap length exceeds expected native capacity - likely a corrupted/tampered map file");
		}
		new NativeSlice<byte>(array, 0, dat.Length).CopyFrom(dat);
	}
