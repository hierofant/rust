...
public class FreeableLootContainer : LootContainer
{
	...
	public override bool OnRpcMessage(BasePlayer player, uint rpc, Message msg)
	{
		using (TimeWarning.New("FreeableLootContainer.OnRpcMessage"))
		{
			...
			if (rpc == 1460413277 && player != null)
			{
				Assert.IsTrue(player.isServer, "SV_RPC Message is using a clientside player!");
				if (Global.developer > 2)
				{
					Debug.Log("SV_RPCMessage: " + player?.ToString() + " - RPC_FreeCrateTimer");
				}
	...
	{
		if (IsTiedDown() && !(Mathf.Abs(startUntieTime + 6f - UnityEngine.Time.realtimeSinceStartup) > ConVar.AntiHack.rpc_timer_forgiveness))
		{
