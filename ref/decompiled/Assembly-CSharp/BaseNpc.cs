...
public class BaseNpc : BaseCombatEntity
{
	...
	public LayerMask movementMask = 1503731969;
	...
	public RustNavMeshAgent GetNavAgent
	{
		get
		{
			if (base.isClient)
	...
	public static Vector3 GetNewNavPosWithVelocity(BaseEntity ent, Vector3 velocity)
	{
		...
		Vector3 targetPositionWS = ent.ServerPosition + velocity * UnityEngine.Time.fixedDeltaTime;
		RustNavMeshHelpers.Raycast(ent.ServerPosition, targetPositionWS, out var hitWS, -1);
		if (!hitWS.position.IsNaNOrInfinity())
		{
	...
	private void TickSpeed()
	{
		...
		float num = 1f - 0.9f * Vector3.Angle(base.transform.forward, (NavAgent.nextPositionWS - ServerPosition).normalized) / 180f * time * time;
		speed *= num;
		NavAgent.speed = Mathf.Lerp(NavAgent.speed, speed, 0.5f);
	...
	protected virtual void TickMetabolism()
	{
		...
		if (NavAgent.desiredVelocityWS.sqrMagnitude > 0.1f)
		{
			num *= 2f;
		}
	...
	private void TickChase()
	{
		...
		if ((NavAgent.destinationWS - position).sqrMagnitude > 0.010000001f)
		{
	...
	private void TickFollowPath(ref Vector3 moveToPosition)
	{
		...
		stepDirection = NavAgent.desiredVelocityWS.normalized;
	}

	private bool ValidateNextPosition(ref Vector3 moveToPosition)
	{
		if (!ValidBounds.Test(this, moveToPosition) && base.transform != null && !base.IsDestroyed)
		{
	...
	public void UpdateAiRotation()
	{
		...
		{
			Vector3 vector = ((ChaseTransform != null) ? (ChaseTransform.localPosition - ServerPosition) : ((!(AttackTarget != null)) ? (NavAgent.destinationWS - ServerPosition) : (AttackTarget.ServerPosition - ServerPosition)));
		...
		else if ((NavAgent.destinationWS - ServerPosition).sqrMagnitude > 1f)
		{
	...
	{
		if (!(target == null) && (!(MinimumTargetHealthFraction > 0f) || !(target.healthFraction < MinimumTargetHealthFraction)))
		{
	...
	public virtual void Eat()
	{
		if ((bool)FoodTarget)
		{
			...
			ClientRPC(RpcTarget.NetworkGroup("Eat"), FoodTarget.transform.position);
		}
	}
	...
	{
		ClientRPC(RpcTarget.NetworkGroup("Startled"), base.transform.position);
	}
