...
public static class EACServer
{
	...
	public static bool ValidInterface => Interface != null;
	...
	public static void DoStartup()
	{
		if (Network.Net.sv.secure && !Application.isEditor)
		{
			...
			if (CanSendAnalytics)
			{
				BasePlayer.EACTickStates = new NativeArray<BasePlayer.EACTickState>(32, Allocator.Persistent);
			}
			if (ValidInterface)
			{
				BasePlayer.ClientHandles = new NativeArray<IntPtr>(32, Allocator.Persistent, NativeArrayOptions.UninitializedMemory);
			}
		}
		else
		{
	...
	public static void OnJoinGame(Connection connection, EAC.SystemConfig requiredSystemConfig)
	{
		if (Network.Net.sv.secure && !Application.isEditor)
		{
			...
			registerClientOptions.UserId = ProductUserId.FromString(connection.anticheatId);
			...
			registerClientOptions.Reserved01 = (int)((!connection.IsDevelopmentBuild()) ? requiredSystemConfig : EAC.SystemConfig.None);
	...
	public static void OnMessageReceived(Message message)
	{
		...
		if (client == IntPtr.Zero)
		{
			Debug.LogWarning("EAC network packet from invalid connection: " + message.connection.userid);
		}
