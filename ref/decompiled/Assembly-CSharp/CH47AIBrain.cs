...
public class CH47AIBrain : BaseAIBrain
{
	public class DropCrate : BasicAIState
	{
		...
		{
			if (AirfieldCallChinookTerminal.isActive && AirfieldCallChinookTerminal.selectedChinook == null)
			{
				return AirfieldCallChinookTerminal.selectedChinookDropZone != null;
			}
			return false;
		}
		...
		{
			if (AirfieldCallChinookTerminal.isActive && AirfieldCallChinookTerminal.selectedChinook == heli)
			{
				return AirfieldCallChinookTerminal.selectedChinookDropZone != null;
			}
			return false;
		...
		public override void StateEnter(BaseAIBrain brain, BaseEntity entity)
		{
			...
			CH47DropZone cH47DropZone = CH47DropZone.GetClosest(cH47HelicopterAIController.transform.position);
			if (ActiveAirfieldCall())
			{
				Debug.Log($"Chinook {cH47HelicopterAIController.net.ID} is responding to active chinook call at drop zone {AirfieldCallChinookTerminal.selectedChinookDropZone.transform.position}");
			...
			if (cH47DropZone == null)
			{
				nextDropTime = Time.time + 60f;
			}
			brain.mainInterestPoint = cH47DropZone.transform.position;
		...
		public override StateStatus StateThink(float delta, BaseAIBrain brain, BaseEntity entity)
		{
			...
			if (ActiveAirfieldCall())
			{
				Debug.Log($"Chinook {cH47HelicopterAIController.net.ID} is rerouting crate drop to active chinook call at drop zone {AirfieldCallChinookTerminal.selectedChinookDropZone.transform.position}");
				CH47DropZone selectedChinookDropZone = AirfieldCallChinookTerminal.selectedChinookDropZone;
				AirfieldCallChinookTerminal.selectedChinook = cH47HelicopterAIController;
				brain.mainInterestPoint = selectedChinookDropZone.transform.position;
				cH47HelicopterAIController.SetMoveTarget(brain.mainInterestPoint);
			}
			if (CanDrop() && Vector3Ex.Distance2D(brain.mainInterestPoint, cH47HelicopterAIController.transform.position) < 5f && cH47HelicopterAIController.rigidBody.linearVelocity.magnitude < 5f)
			{
	...
	public class EgressState : BasicAIState
	{
		...
		public override void StateEnter(BaseAIBrain brain, BaseEntity entity)
		{
			...
			Vector3 vector = Vector3.Cross(Vector3.Cross(rhs: (rigidBody.linearVelocity.magnitude < 0.1f) ? transform.forward : rigidBody.linearVelocity.normalized, lhs: transform.up), Vector3.up);
			if (vector.x <= 0f)
			{
	...
	public class IdleState : BaseIdleState
	{
		...
		{
			CH47HelicopterAIController cH47HelicopterAIController = entity as CH47HelicopterAIController;
			cH47HelicopterAIController.SetMoveTarget(cH47HelicopterAIController.GetPosition() + cH47HelicopterAIController.rigidBody.linearVelocity.normalized * 10f);
			base.StateEnter(brain, entity);
		}
	}
	...
	public class LandState : BasicAIState
	{
		...
		public override StateStatus StateThink(float delta, BaseAIBrain brain, BaseEntity entity)
		{
			...
			float magnitude = cH47HelicopterAIController.rigidBody.linearVelocity.magnitude;
