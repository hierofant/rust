...
public static class PlayerNetworkingProfiler
{
	...
	public static void Serialize(AnalyticsTable table, int frameIndex, DateTime timestamp)
	{
		...
		{
			SerializeRaknet(table, frameIndex, timestamp, server);
		}
	}

	private unsafe static void SerializeRaknet(AnalyticsTable table, int frameIndex, DateTime timestamp, Facepunch.Network.Raknet.Server server)
	{
