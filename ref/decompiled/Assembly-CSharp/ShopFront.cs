...
public class ShopFront : StorageContainer
{
	...
	private static readonly Translate.Phrase TradeLockedError = new Translate.Phrase("error.tradelocked", "Trade Locked!");

	private static readonly Translate.Phrase TradeUnsuccessfulError = new Translate.Phrase("error.tradeunsuccessful", "Trade Unsuccessful!");
	...
	{
		if (vendorPlayer != null && customerPlayer != null && HasFlag(Flags.Reserved1) && HasFlag(Flags.Reserved4))
		{
	...
	public override void ServerInit()
	{
		...
		{
			customerInventory = Facepunch.Pool.Get<ItemContainer>();
	...
	internal override void DoServerDestroy()
	{
		...
		Facepunch.Pool.Free(ref customerInventory);
	}
	...
	{
		ClientRPC(RpcTarget.NetworkGroup("CLIENT_ReceivePlayers"), (vendorPlayer == null) ? default(NetworkableId) : vendorPlayer.net.ID, (customerPlayer == null) ? default(NetworkableId) : customerPlayer.net.ID);
	}
