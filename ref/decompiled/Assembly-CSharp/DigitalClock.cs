...
public class DigitalClock : IOEntity, INotifyLOD
{
	...
	public void RPC_SetAlarms(RPCMessage msg)
	{
		...
		try
		{
			...
			alarms.Clear();
			foreach (DigitalClockAlarm alarm2 in digitalClockMessage.alarms)
			{
	...
	public override void Save(SaveInfo info)
	{
		...
		foreach (Alarm alarm in alarms)
		{
			DigitalClockAlarm digitalClockAlarm = Facepunch.Pool.Get<DigitalClockAlarm>();
			digitalClockAlarm.active = alarm.active;
			digitalClockAlarm.time = DigitalClockEx.ToFloat(alarm.time);
