...
public class GameManager
{
	...
	public const bool InUnitTest = false;
	...
	public GameManager(bool clientside, bool serverside)
	{
		...
		pool = new PrefabPoolCollection(this, clientside);
	}
	...
	public GameObject FindPrefab(string strPrefab)
	{
		...
		{
			Debug.LogError("Failed to preprocess prefab: " + strPrefab);
			throw;
		}
	...
	public GameObject CreatePrefab(string strPrefab, Transform parent, bool active = true, bool resetScale = true)
	{
		...
		if ((bool)gameObject)
		{
			...
			TransformEx.Identity(gameObject, resetScale);
			if (active)
			{
				PoolableEx.AwakeFromInstantiate(gameObject);
			}
	...
	private GameObject Instantiate(string strPrefab, Vector3 pos, Quaternion rot)
	{
		...
		if (!Clientside && Serverside && gameObject2.transform.parent == null && UnityEngine.Application.isPlaying)
		{
			...
			SceneManager.MoveGameObjectToScene(gameObject2, entityScene);
		}
		return gameObject2;
	}
	...
	public void Retire(GameObject instance)
	{
		...
		using (TimeWarning.New("GameManager.Retire"))
		{
			...
			if (!Rust.Application.isUnloadingWorld && LevelManager.isLoaded && ConVar.Pool.enabled && PoolableEx.SupportsPooling(instance))
			{
				pool.Push(instance);
			}
			else
