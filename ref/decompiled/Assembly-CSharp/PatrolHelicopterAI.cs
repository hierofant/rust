...
public class PatrolHelicopterAI : BaseMonoBehaviour
{
	...
	[ServerVar(Help = "(Generated) Fraction of maximum health at which the patrol helicopter will break off its attack and flee; default 0.35 (35%)")]
	public static float flee_damage_percentage = 0.35f;

	[ServerVar(Help = "(Generated) When enabled, the patrol helicopter avoids designated danger zones during its patrol route")]
	public static bool use_danger_zones = true;

	[ServerVar(Help = "(Generated) When enabled, the patrol helicopter prefers to crash into a monument area when destroyed rather than crashing in open terrain")]
	...
	public void UpdateTargetList()
	{
		...
		for (int num2 = _targetList.Count - 1; num2 >= 0; num2--)
		{
			...
			else if (DeepSeaManager.IsInsideDeepSea(targetinfo2.ply))
			{
				RemoveTargetAt(num2);
			}
			else
	...
	{
		using (TimeWarning.New("PatrolHelicoperAI.AddNewTargetsToList"))
		{
			if (AI.ignoreplayers)
			{
				return;
			...
			BaseEntity.Query.Server.GetPlayersInSphere(base.transform.position, 150f, pooledList, BaseEntity.Query.DistanceCheckType.None);
			foreach (BasePlayer item in pooledList)
			{
				if (item == null || SimpleAIMemory.PlayerIgnoreList.Contains(item))
				{
					continue;
				}
				using (TimeWarning.New("PatrolHelicoperAI.SafeZone"))
				{
					if (item.InSafeZone())
					{
						continue;
				...
				if (item.IsInTutorial || DeepSeaManager.IsInsideDeepSea(item))
				{
					continue;
				}
				using (TimeWarning.New("PatrolHelicoperAI.NoGoZone"))
				{
					if (use_danger_zones && IsInNoGoZone(item.transform.position))
					{
						continue;
					}
				}
	...
	{
		using (TimeWarning.New("PatrolHelicoperAI.IsInNoGoZone"))
		{
			bool result = false;
	...
	[ServerVar(Help = "(Generated) Dumps the current patrol helicopter AI state to the console including state machine, targets, orbit state, and retire status")]
	...
	public void SetInitialDestination(Vector3 dest, float mapScaleDistance = 0.25f)
	{
		...
		Vector3 vector = TerrainMeta.RandomPointOffshore(avoidDeepSeaPortal: false, avoidDeepSea: true);
		vector.y = WaterLevel.GetWaterSurface(vector, waves: false, volumes: false);
		if (mapScaleDistance == 0f)
		{
			vector = interestZoneOrigin + new Vector3(0f, 10f, 0f);
		}
	...
	{
		return UnityEngine.Random.Range(35f, 45f);
	}
	...
	public void TerrainPushback()
	{
		if (_currentState != aiState.DEATH || forceTerrainPushback)
		{
			...
			int mask2 = LayerMask.GetMask("Water");
			Ray ray3 = new Ray(vector, Vector3.down);
			float num8 = 10f;
			if (GamePhysics.Trace(ray3, num, out var hitInfo3, num8 - num * 0.5f, mask2))
			{
				float num9 = 1f - hitInfo3.distance / num8;
				float num10 = obstaclePushForce * num9;
				b2 += Vector3.up * num10;
			}
	...
	public void Update()
	{
		...
		AIThink();
		DoMachineGuns();
		if ((float)timeSinceRestrictedPatrolSize > 240f && restrictedPatrolSize)
		{
			restrictedPatrolSize = false;
			timeSinceRestrictedPatrolSize = 0f;
		}
	...
	public void State_Death_Think(float timePassed)
	{
		...
		if (didImpact || UnityEngine.Time.realtimeSinceStartup > deathTimeout)
		{
			KillOfNaturalCauses();
		}
	}
	...
	public void State_Death_Enter()
	{
		...
		if (monumentInfo == null || IsInsideMonument(base.transform.position))
		{
			reachedSpinoutLocation = true;
			StartSpinout();
			return;
	...
	{
		return GetCloseMonument(maxDistance, base.transform.position);
	}
	...
	private static MonumentInfo GetCloseMonument(float maxDistance, Vector3 position)
	{
		...
		if (TerrainMeta.Path != null && TerrainMeta.Path.Monuments != null && TerrainMeta.Path.Monuments.Count > 0)
		{
			...
			foreach (MonumentInfo monument in TerrainMeta.Path.Monuments)
			{
				...
				if (type != MonumentType.Mountain && type != MonumentType.Lighthouse && type != MonumentType.Lake && type != MonumentType.WaterWell && type != 0 && type != MonumentType.Building && monument.Tier != 0 && !(monument.transform.position.y < WaterSystem.OceanLevel) && monument.AllowPatrolHeliCrash)
				{
					float num2 = Vector3Ex.Distance2D(position, monument.transform.position);
	...
	private static bool IsInsideMonument(Vector3 position)
	{
		if (TerrainMeta.Path != null && TerrainMeta.Path.Monuments != null && TerrainMeta.Path.Monuments.Count > 0)
		{
			foreach (MonumentInfo monument in TerrainMeta.Path.Monuments)
			{
				...
				if (type != MonumentType.Mountain && type != MonumentType.Lighthouse && type != MonumentType.Lake && type != MonumentType.WaterWell && type != 0 && type != MonumentType.Building && monument.Tier != 0 && !(monument.transform.position.y < WaterSystem.OceanLevel) && monument.AllowPatrolHeliCrash)
				{
					float num = Vector3Ex.Distance2D(position, monument.transform.position);
	...
	public void State_Flee_Enter()
	{
		...
		Vector3 normalized;
		if (closeMonument != null)
		{
			normalized = (closeMonument.transform.position - base.transform.position).normalized;
			normalized.y = 0f;
		}
		else
		{
			Vector2 insideUnitCircle = UnityEngine.Random.insideUnitCircle;
			normalized = new Vector3(insideUnitCircle.x, 0f, insideUnitCircle.y).normalized;
		}
		Vector3 vector = base.transform.position + normalized * 130f;
		vector.y = GetPlaneHeight();
		TryMove(vector);
		if (helicopterBase != null)
		{
			helicopterBase.CreateFleeMarker(vector);
		}
	}
	...
	public Vector3 GetOrbitPosition(float rate)
	{
		...
		Vector3 vector = new Vector3(x, 0f, z);
		Vector3 result = interestZoneOrigin + vector;
	...
	{
		breakingOrbit = true;
		Vector3 appropriatePosition = GetAppropriatePosition(base.transform.position + base.transform.forward * 75f);
	...
	{
		Vector3 vector = FindValidDestination();
		if (restrictedPatrolSize)
		{
			Vector3 normalized = (vector - base.transform.position).normalized;
			normalized.y = 0f;
			Vector3 result = base.transform.position + normalized * UnityEngine.Random.Range(65f, 130f);
	...
	public void State_Patrol_Think(float timePassed)
	{
		...
		{
			interestZoneOrigin = base.transform.position;
		}
	}
