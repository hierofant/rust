...
public class GodRaysRenderer : PostProcessEffectRenderer<GodRays>
{
	...
	private void DrawBorder(PostProcessRenderContext context, RenderTargetIdentifier buffer1)
	{
		...
		context.command.BlitFullscreenTriangle(BuiltinRenderTextureType.None, buffer1, propertySheet, 0, clear: false, value);
		context.command.BlitFullscreenTriangle(BuiltinRenderTextureType.None, buffer1, propertySheet, 0, clear: false, value2);
		context.command.BlitFullscreenTriangle(BuiltinRenderTextureType.None, buffer1, propertySheet, 0, clear: false, value3);
		context.command.BlitFullscreenTriangle(BuiltinRenderTextureType.None, buffer1, propertySheet, 0, clear: false, value4);
	}
	...
	private int GetSkyMask(PostProcessRenderContext context, ResolutionType resolution, Vector3 lightPos, int blurIterations, float blurRadius, float maxRadius)
	{
		...
		{
			command.BlitFullscreenTriangle(context.source, num, propertySheet, 1);
		}
		else
		{
			command.BlitFullscreenTriangle(context.source, num, propertySheet, 2);
		}
		...
		for (int i = 0; i < blurIterations; i++)
		{
			...
			command.BlitFullscreenTriangle(num, num2, propertySheet, 0);
			command.ReleaseTemporaryRT(num);
			num3 = blurRadius * (((float)i * 2f + 1f) * 6f) / 768f;
			propertySheet.properties.SetVector("_BlurRadius4", new Vector4(num3, num3, 0f, 0f));
			command.GetTemporaryRT(num, width, height, depthBuffer);
			command.BlitFullscreenTriangle(num2, num, propertySheet, 0);
