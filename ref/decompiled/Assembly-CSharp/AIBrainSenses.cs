...
{
	[ServerVar(Help = "(Generated) Interval in seconds between full AI brain senses update ticks; controls how frequently NPCs refresh their awareness of surroundings")]
	public static float UpdateInterval = 0.5f;

	[ServerVar(Help = "(Generated) Interval in seconds between line-of-sight update ticks for human-type NPCs tracking known player positions")]
	public static float HumanKnownPlayersLOSUpdateInterval = 0.2f;

	[ServerVar(Help = "(Generated) Interval in seconds between line-of-sight update ticks for all NPC types tracking known player positions")]
	...
	private void UpdateSenses()
	{
		...
		if (senseTypes != 0)
		{
			...
			{
				SenseBrains();
				if ((senseTypes & EntityType.Player) == EntityType.Player)
				{
					SensePlayers();
				}
			}
	...
	{
		int playersInSphereFast = BaseEntity.Query.Server.GetPlayersInSphereFast(owner.transform.position, maxRange, playerQueryResults, aiCaresAbout);
	...
	{
		int brainsInSphereFast = BaseEntity.Query.Server.GetBrainsInSphereFast(owner.transform.position, maxRange, queryResults, aiCaresAbout);
	...
	private bool IsValidSenseType(BaseEntity ent)
	{
		...
		if (basePlayer != null)
		{
			if (basePlayer.IsNpc)
			{
				...
				if ((senseTypes & EntityType.BasePlayerNPC) == EntityType.BasePlayerNPC)
				{
					return true;
				}
			}
			else if ((senseTypes & EntityType.Player) == EntityType.Player)
			{
				return true;
			}
		}
		if ((senseTypes & EntityType.NPC) == EntityType.NPC && ent is BaseNpc)
		{
			return true;
		}
		if ((senseTypes & EntityType.WorldItem) == EntityType.WorldItem && ent is WorldItem)
		{
			return true;
		}
		if ((senseTypes & EntityType.Corpse) == EntityType.Corpse && ent is BaseCorpse)
		{
			return true;
		}
		if ((senseTypes & EntityType.TimedExplosive) == EntityType.TimedExplosive && ent is TimedExplosive)
		{
			return true;
		}
		if ((senseTypes & EntityType.Chair) == EntityType.Chair && ent is BaseChair)
		{
			return true;
		}
		return false;
