...
public class BatteringRam : BaseSiegeWeapon, IEngineControllerUser, IEntity, VehicleChassisVisuals<BatteringRam>.IClientWheelUser
{
	...
	public float DoorAngle
	{
		get
		{
			if (base.isServer)
			{
				...
				{
					return Facepunch.Extend.TransformEx.Unsafe.GetLocalRotMT(in doorPhysHingeHandle).eulerAngles.x;
				}
	...
	{
		if (Mathf.Abs(doorRigidBody.angularVelocity.magnitude) > 1f || Mathf.Abs(doorRigidBody.linearVelocity.magnitude) > 1f)
		{
			lastDoorMovingTime = 0f;
		}
		using FlagsUpdateScope flagsUpdateScope = StartSetFlags(FlagsUpdateMode.SendNetworkUpdate);
	...
	public override void ServerInit()
	{
		...
		if (doorPhysicsHinge != null)
		{
			doorPhysHingeHandle = doorPhysicsHinge.transformHandle;
		}
	}
	...
	private void FilterEntities(List<BaseEntity> entityList, BasePlayer driver, Vector3 lineOfSightOrigin)
	{
		...
		for (int i = 0; i < entityList.Count; i++)
		{
			...
			if (baseEntity.isServer && !obj.Contains(baseEntity) && (!(driver != null) || !(baseEntity == driver)) && !(baseEntity == this) && !(baseEntity == Head) && CanHeadReach(baseEntity, lineOfSightOrigin))
			{
				obj.Add(baseEntity);
			}
		}
	...
	private void DamageEntities(List<BaseEntity> entityList, BaseEntity attackingPlayer, List<DamageTypeEntry> damages, Vector3 hitPos)
	{
		...
		{
			BaseEntity baseEntity = entityList[i];
			if (baseEntity == null || !baseEntity.IsValid() || (baseEntity is BuildingBlock buildingBlock && (int)buildingBlock.grade > maxBuildingBlockGrade))
			{
				continue;
			}
