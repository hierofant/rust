...
public class ResearchTable : StorageContainer
{
	...
	{
		ItemDefinition itemDefinition = ((item.info.isRedirectOf != null) ? item.info.isRedirectOf : item.info);
		if (!itemDefinition.IsAllowed(EraRestriction.Craft))
		{
			return false;
		}
	...
	public void DoResearch(RPCMessage msg)
	{
		...
		if (targetItem != null && Interface.CallHook("CanResearchItem", player, targetItem) == null && targetItem.amount <= 1 && IsItemResearchable(targetItem) && (!onlyOneUser || !(msg.player.inventory.loot.entitySource != this)))
		{
