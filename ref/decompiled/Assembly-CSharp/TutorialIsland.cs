...
public class TutorialIsland : BaseEntity, IEntityPingSource
{
	...
	public static void GenerateIslandSpawnPoints(bool loadingSave = false)
	{
		...
		TutorialIslandSpawner.GetEdgeSpawnPoints(islandSpawnLocations, vector, vector2 - vector, cellSize, 1, out WorldBoundsMinusTutorialIslands);
		for (int i = 0; i < islandSpawnLocations.Count; i++)
		{
			Bounds bounds = new OBB(islandSpawnLocations[i], Quaternion.identity, new Bounds(new Vector3(0f, 25f, 0f), new Vector3(400f, 80f, 400f))).ToBounds();
			if (DeepSeaManager.IsInsideDeepSea(bounds))
			{
				islandSpawnLocations.RemoveAt(i);
				i--;
			}
			else if (IsOnDeepSeaPortalSide(bounds))
			{
				islandSpawnLocations.RemoveAt(i);
				i--;
			}
	...
	{
		return Network.Net.sv.visibility.Get((uint)(100 + index));
	}
	...
	public void Return()
	{
		...
		{
			if (!(item is BasePlayer) && !(item is TutorialIsland) && !(item is DeepSeaPortal) && !item.isClient && !item.IsDestroyed)
			{
				item.Kill();
			}
		}
		...
		if (basePlayer != null)
		{
			basePlayer.Server_FailActiveTutorialMission();
		}
		ForPlayer.Set(null);
		ReturnIsland(this);
	...
	private void TickPlayerConnectionStatus()
	{
		...
		{
			Item item = ItemManager.Create(rockDefinition, 1, 0uL, isServerSide: true, 0uL);
	...
	private bool SearchPlayerForItem(ItemDefinition targetItem, BasePlayer player)
	{
		...
		{
			if (item.info == targetItem || item.info.isRedirectOf == targetItem)
			{
				return true;
			}
		}
		...
		{
			if (item2.info == targetItem || item2.info.isRedirectOf == targetItem)
			{
				return true;
			}
		}
		...
		{
			if (item3.info == targetItem || item3.info.isRedirectOf == targetItem)
			{
				return true;
			}
		}
	...
	[ServerVar]
	public static void debugIslandPositions(ConsoleSystem.Arg arg)
	{
		...
		{
			UnityEngine.DDraw.Sphere(basePlayer, islandSpawnLocation, 5f, Color.yellow, @float);
			UnityEngine.DDraw.Bounds(basePlayer, new OBB(islandSpawnLocation, Quaternion.identity, new Bounds(new Vector3(0f, 25f, 0f), new Vector3(400f, 80f, 400f))).ToBounds(), Color.yellow, @float);
		}
	}
	...
	private static void AddIslandBounds(OBB worldBounds, uint netId, bool isServer)
	{
		...
		IslandBounds val = islandBounds;
		if (isServer)
		{
			BoundsListServer.TryAdd(val);
		}
	}
