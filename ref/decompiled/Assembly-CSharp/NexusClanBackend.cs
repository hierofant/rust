...
public class NexusClanBackend : IClanBackend, IDisposable
{
	...
	public async ValueTask<ClanValueResult<IClan>> Get(long clanId)
	{
		...
		catch (Exception exception)
		{
			...
			return new ClanValueResult<IClan>(ClanResult.Fail);
		}
	}
	...
	public async ValueTask<ClanValueResult<IClan>> GetByMember(ulong steamId)
	{
		try
		{
			NexusClanResult<NexusClan> nexusClanResult = await _client.GetClanByMember(steamId);
			if (nexusClanResult.IsSuccess && nexusClanResult.TryGetResponse(out var response))
			{
				return (ClanValueResult<IClan>)(IClan)Wrap(response);
			}
	...
	public async ValueTask<ClanValueResult<IClan>> Create(ulong leaderSteamId, string name)
	{
		try
		{
			...
			clanCreateParameters.LeaderPlayerId = leaderSteamId;
	...
	public async ValueTask<ClanValueResult<List<ClanInvitation>>> ListInvitations(ulong steamId)
	{
		try
		{
			NexusClanResult<List<Facepunch.Nexus.Models.ClanInvitation>> nexusClanResult = await _client.ListClanInvitations(steamId);
			if (nexusClanResult.IsSuccess && nexusClanResult.TryGetResponse(out var response))
			{
				List<ClanInvitation> value = response.Select(delegate(Facepunch.Nexus.Models.ClanInvitation i)
				{
					...
					result.Recruiter = i.RecruiterPlayerId;
		...
		{
			Debug.LogException(exception);
			return new ClanValueResult<List<ClanInvitation>>(ClanResult.Fail);
		}
	}
	...
	public async ValueTask<ClanValueResult<List<ClanLeaderboardEntry>>> GetLeaderboard(int limit = 100)
	{
		try
		{
			NexusClanResult<List<Facepunch.Nexus.Models.ClanLeaderboardEntry>> nexusClanResult = await _client.GetClanLeaderboard(limit);
			if (nexusClanResult.IsSuccess && nexusClanResult.TryGetResponse(out var response))
			{
				List<ClanLeaderboardEntry> value = response.Select(delegate(Facepunch.Nexus.Models.ClanLeaderboardEntry c)
				{
					ClanLeaderboardEntry result = default(ClanLeaderboardEntry);
					result.ClanId = c.ClanId;
					result.Name = c.Name;
					result.Score = c.Score;
					return result;
				}).ToList();
				return new ClanValueResult<List<ClanLeaderboardEntry>>(value);
			}
			return NexusClanUtil.ToClanResult(nexusClanResult.ResultCode);
		}
		catch (Exception exception)
		{
			Debug.LogException(exception);
			return new ClanValueResult<List<ClanLeaderboardEntry>>(ClanResult.Fail);
		}
	}
	...
	private IEnumerator BroadcastClanChatBatches()
	{
		...
		{
			List<ClanChatBatchRequest.Message> obj = Facepunch.Pool.Get<List<ClanChatBatchRequest.Message>>();
