...
public abstract class BaseCardGameEntity : BaseVehicle
{
	...
	public BasePlayer IDToPlayer(ulong id)
	{
		...
		{
			if (mountPoint.mountable != null && mountPoint.mountable.GetMounted() != null && (ulong)mountPoint.mountable.GetMounted().userID == id)
			{
	...
	{
		if (player == null || storage == null || !player.isMounted)
		{
			return false;
		}
		...
		if (GetPlayerStorage(mountPointIndex) != storage)
		{
			return false;
		}
	...
	public int GetMountPointIndex(ulong playerID)
	{
		...
		for (int i = 0; i < mountPoints.Count; i++)
		{
			...
			if (mountable != null)
			{
				...
				if (mounted != null && (ulong)mounted.userID == playerID)
				{
					num = i;
				}
			}
	...
	{
		if (!UnityEngine.Application.isEditor || GameController == null)
		{
			return;
		}
