...
public static class TransformUtil
{
	...
	public static bool GetGroundInfo(Vector3 startPos, out Vector3 pos, out Vector3 normal, float range, LayerMask mask, Transform ignoreTransform = null)
	{
		...
		foreach (RaycastHit item in obj)
		{
			...
			if (ignoreTransform != null)
			{
				...
				if (transform == null || transform == ignoreTransform || transform.IsChildOf(ignoreTransform))
				{
					continue;
				}
			}
	...
	{
		return (from x in Object.FindObjectsByType<Transform>(FindObjectsSortMode.None)
			where x.transform == x.transform.root
			select x).ToArray();
	}

	public static Transform[] GetAllRootObjects()
	{
		return (from x in Object.FindObjectsByType<Transform>(FindObjectsInactive.Include, FindObjectsSortMode.None)
			where x.transform == x.transform.root
			select x).ToArray();
	}

	public static Transform[] GetWholeHierarchy(Transform root)
	{
		List<Transform> list = new List<Transform>();
		AddChildrenRecursive(root, list);
		return list.ToArray();
	}
	...
	private static void AddChildrenRecursive(Transform current, List<Transform> list)
	{
		...
		{
			AddChildrenRecursive(item, list);
		}
	}
}
