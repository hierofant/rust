...
public class CH47FlightTest : MonoBehaviour
{
	...
	public HelicopterInputState_t GetHelicopterInputState()
	{
		...
		result.throttle = (Keyboard.current[Key.W].isPressed ? 1f : 0f);
		result.throttle -= (Keyboard.current[Key.S].isPressed ? 1f : 0f);
		Vector2 vector = MouseDelta.Read();
		result.pitch = vector.y;
		result.roll = 0f - vector.x;
		result.yaw = (Keyboard.current[Key.D].isPressed ? 1f : 0f);
		result.yaw -= (Keyboard.current[Key.A].isPressed ? 1f : 0f);
