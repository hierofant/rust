...
public class ModularCar : BaseModularVehicle, TakeCollisionDamage.ICanRestoreVelocity, CarPhysics<ModularCar>.ICar, IHandbrakeCar, IVehicleLockUser, VehicleChassisVisuals<ModularCar>.IClientWheelUser
{
	...
	{
		using (TimeWarning.New("ModularCar.VehicleFixedUpdate"))
		{
	...
	public override bool MountEligable(BasePlayer player)
	{
		...
		if (modularCarSeat != null && modularCarSeat.associatedSeatingModule != null && !modularCarSeat.associatedSeatingModule.DoorsAreLockable)
		{
			return true;
		}
	...
	public void SpawnPreassignedModules()
	{
		...
		for (int i = 0; i < modularCarPresetConfig.socketItemDefs.Length; i++)
		{
			...
			{
				Item item = ItemManager.Create(itemModVehicleModule.GetComponent<ItemDefinition>(), 1, 0uL, isServerSide: true, 0uL);
