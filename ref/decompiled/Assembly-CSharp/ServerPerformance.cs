...
public class ServerPerformance : BaseMonoBehaviour
{
	...
	private void WriteLine()
	{
		...
		int num = UnityEngine.Object.FindObjectsByType<GameObject>(FindObjectsSortMode.None).Length;
		int num2 = 0;
		int num3 = 0;
		int num4 = 0;
		int num5 = 0;
		...
		File.AppendAllText(fileName, monoUsedSize + "," + usedHeapSize + "," + num7 + "," + count + "," + count2 + "," + NetworkSleep.totalCollidersDisabled + "," + NetworkSleep.totalBehavioursDisabled + "," + num + "," + UnityEngine.Object.FindObjectsByType<Collider>(FindObjectsSortMode.None).Length + "," + UnityEngine.Object.FindObjectsByType<Rigidbody>(FindObjectsSortMode.None).Length + "," + UnityEngine.Object.FindObjectsByType<BuildingBlock>(FindObjectsSortMode.None).Length + "," + num2 + "," + num3 + "," + num4 + "," + num5 + "," + num6 + "," + deaths + "," + spawns + "," + position_changes + "\r\n");
		lastFrame = Time.frameCount;
		deaths = 0uL;
		spawns = 0uL;
	...
	public static void DoReport()
	{
		...
		UnityEngine.Object[] objects = UnityEngine.Object.FindObjectsByType<Transform>(FindObjectsSortMode.None);
		ComponentReport(filename, "All Objects", objects);
		string filename2 = text;
		objects = UnityEngine.Object.FindObjectsByType<BaseEntity>(FindObjectsSortMode.None);
		ComponentReport(filename2, "Entities", objects);
		string filename3 = text;
		objects = UnityEngine.Object.FindObjectsByType<Rigidbody>(FindObjectsSortMode.None);
		ComponentReport(filename3, "Rigidbodies", objects);
		string filename4 = text;
		objects = (from x in UnityEngine.Object.FindObjectsByType<Collider>(FindObjectsSortMode.None)
			where !x.enabled
			select x).ToArray();
