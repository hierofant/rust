...
{
	public struct PlanterBoxPreserveInfo
	{
	...
	[ServerVar(Help = "(Generated) Maximum distance in metres used when raycasting to determine whether a planter box is outdoors and receiving natural sunlight")]
	...
	public bool WantsSplash(ItemDefinition splashType, int amount)
	{
		...
		if (!(splashType == WaterTypes.SaltWaterItemDef) && !(splashType == WaterTypes.RadioactiveWaterItemDef))
		{
	...
	{
		if (splashType == WaterTypes.SaltWaterItemDef)
		{
			soilSaturation = 0;
			RefreshGrowables();
	...
	private void KillAllPlants()
	{
		...
		for (int num = children.Count - 1; num >= 0; num--)
		{
			...
			if (!(baseEntity == null) && baseEntity is GrowableEntity growableEntity)
			{
	...
	public void CalculateRainFactor()
	{
		...
		if (rain > 0f && WaterCatcher.TestIsOutside(base.transform, Vector3.up, 0.5f, outsideTestDistance))
		{
	...
	public void OnPlantInserted(GrowableEntity entity, BasePlayer byPlayer)
	{
		...
		List<uint> obj = Facepunch.Pool.Get<List<uint>>();
		foreach (BaseEntity child in children)
		{
	...
	public override void Reskin_Preserve(ref SprayCan.ReskinPreserveInfo preserveInfo)
	{
		...
		preserveInfo.planterBoxPreserve.soilSaturation = soilSaturation;
	}

	public override void Reskin_Restore(ref SprayCan.ReskinPreserveInfo preserveInfo)
	{
		base.Reskin_Restore(ref preserveInfo);
		soilSaturation = preserveInfo.planterBoxPreserve.soilSaturation;
		Sprinkler.SplashableGrid.RegisterEntity(this);
	}
