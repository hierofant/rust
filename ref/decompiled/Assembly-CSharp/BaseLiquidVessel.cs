...
public class BaseLiquidVessel : AttackEntity
{
	...
	public static Translate.Phrase DifferentLiquidType = new Translate.Phrase("fill_different_liquid_type", "You can't mix different liquids");

	public static Translate.Phrase CannotHoldCrude = new Translate.Phrase("fill_cannot_hold_crude", "You can't collect crude oil in this");
	...
	public void FillCheck()
	{
		...
		else if (facingLiquidContainer != null && facingLiquidContainer.HasLiquidItem())
		{
			...
			if (liquidItem.info == LiquidContainerCrude.CrudeItem)
			{
				if ((float)timeSinceLastToast > 5f)
				{
					timeSinceLastToast = 0f;
	...
	public void DoThrow(Vector3 pos, Vector3 velocity)
	{
		...
		if (slot != null && slot.amount > 0)
		{
			...
			float num = 1f;
			if (GamePhysics.Trace(ray, 0f, out var hitInfo, num, 1084293377))
			{
			...
			if ((bool)waterBall)
			{
				...
				waterBall.transform.position = point;
	...
	public bool CanFillFromWorld()
	{
		...
		if (ownerPlayer.IsInWaterVolume(base.transform.position, out var natural) && !natural)
		{
			return false;
		}
	...
	{
		BasePlayer ownerPlayer = GetOwnerPlayer();
		if (!ownerPlayer || ownerPlayer.eyes == null)
		{
			return null;
		}
