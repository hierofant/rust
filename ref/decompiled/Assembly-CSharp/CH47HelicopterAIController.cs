...
public class CH47HelicopterAIController : CH47Helicopter
{
	...
	{
		CH47HelicopterAIController[] array = Object.FindObjectsByType<CH47HelicopterAIController>(FindObjectsInactive.Exclude, FindObjectsSortMode.None);
	...
	public void TriggeredEventSpawn()
	{
		...
		vector.y = WaterLevel.GetWaterSurface(vector, waves: false, volumes: false) + 30f;
	...
	{
		if (BaseNetworkableEx.Is<HumanNPC>(prefab.GetEntity(), out var _))
		{
		...
		{
			if (!BaseNetworkableEx.Is<BaseNPC2>(prefab.GetEntity(), out var _))
			{
				return;
			}
			...
			foreach (MountPointInfo allMountPoint in base.allMountPoints)
			{
				...
				{
					if (Vector3.Distance(parentedNpc.transform.position, allMountPoint.mountable.mountAnchor.position) < 0.5f)
					{
						flag = true;
						break;
					}
			...
			if (baseMountable != null)
			{
				...
				baseEntity2.transform.position = baseMountable.mountAnchor.position;
				baseEntity2.transform.rotation = baseMountable.mountAnchor.rotation;
				baseEntity2.GetComponent<RustNavMeshAgent>().enabled = false;
			}
		}
	}
	...
	public bool TryDismountOne()
	{
		...
		foreach (BaseNPC2 parentedNpc in parentedNpcs)
		{
			...
			foreach (Transform transform2 in array)
			{
				...
				{
					if (Vector3.Distance(parentedNpc2.transform.position, transform2.position) < 0.5f)
					{
						flag = true;
						break;
					}
				...
				if (!flag)
				{
					float num2 = Vector3.Distance(parentedNpc.transform.position, transform2.position);
					if (num2 < num)
					{
						num = num2;
			...
			if (transform != null)
			{
				...
				parentedNpc.GetComponent<RustNavMeshAgent>().enabled = true;
				parentedNpcs.Remove(parentedNpc);
				parentedNpc.transform.position = transform.position;
				parentedNpc.transform.rotation = transform.rotation;
				return true;
			}
		}
		return false;
	...
	{
		if (!(human == null) && human.Brain != null && human.Brain.Senses != null)
		{
	...
	{
		if (ValidBounds.Test(this, base.transform.position))
		{
	...
	public override void DismountAllPlayers()
	{
		foreach (MountPointInfo mountPoint in mountPoints)
		{
			...
			{
				BasePlayer mounted = mountPoint.mountable.GetMounted();
				if ((bool)mounted && mounted.IsAlive())
				{
	...
	public float CalculateOverrideAltitude()
	{
		...
		{
			Vector3 rhs = ((rigidBody.linearVelocity.magnitude < 0.1f) ? base.transform.forward : rigidBody.linearVelocity.normalized);
			...
			{
				num = hitInfo2.point.y + hoverHeight;
			}
		}
		altOverride = num;
		return altOverride;
	...
	{
		Vector3 vector = base.transform.position + rigidBody.linearVelocity;
