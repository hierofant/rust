...
public class ItemModProjectileRadialDamage : ItemModProjectileMod
{
	...
	public override void ServerProjectileHit(HitInfo info)
	{
		...
		Vis.Entities(info.HitPositionWorld, radius, obj3, 1237003025);
		if (damage.type == DamageType.Explosion)
		{
			SeismicSensor.Notify(info.HitPositionWorld, vibrationLevel);
		}
		foreach (BaseEntity item in obj3)
		{
			if (!item.isServer || obj.Contains(item) || (onlyDoors && !(item is Door)))
			{
				continue;
			}
			BaseEntity parentEntity = item.GetParentEntity();
			bool flag = DamageUtil.ShouldSingleHitSharedParentEntity(item, parentEntity);
			if ((flag && obj2.Contains(parentEntity)) || (item == info.HitEntity && ignoreHitObject))
			{
				continue;
			}
			item.CenterPoint();
			...
			if (item.IsVisibleAndCanSeeLegacy(info.HitPositionWorld - info.ProjectileVelocity.normalized * 0.1f) && item.IsVisibleAndCanSeeLegacy(info.HitPositionWorld - (vector - info.HitPositionWorld).normalized * 0.1f))
			{
				...
				if (flag)
				{
					obj2.Add(parentEntity);
				}
				BasePlayer component = item.GetComponent<BasePlayer>();
				if (component != null && component.TryGetActiveShield(out var foundShield) && info.Initiator != null && foundShield.SphereCastAgainstColliders(info.HitPositionWorld, radius))
				{
