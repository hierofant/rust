...
public class AttackEntity : HeldEntity
{
	...
	public virtual int GetImpactEffectNumberValue(HitInfo info)
	{
		return 0;
	}

	public virtual Effect.Type GetEffectType(HitInfo info)
	{
		return Effect.Type.Generic;
	}

	public virtual DamageType GetDamageTypeForEffect(HitInfo info)
	{
		return info.damageTypes.GetMajorityDamageType();
	}
		...
		if (ConVar.AntiHack.eye_protection > 0)
		{
			...
			if (checkLineOfSight)
			{
				...
				if (ConVar.AntiHack.eye_protection >= 4 && !player.HasParent())
				{
					...
					float num8 = Vector3.Distance(position2, vector2);
					Collider col;
