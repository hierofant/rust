...
internal sealed class ScreenSpaceReflectionsRenderer : PostProcessEffectRenderer<ScreenSpaceReflections>
{
	...
	public override void Render(PostProcessRenderContext context)
	{
		...
		propertySheet.properties.SetTexture(UnityEngine.Rendering.PostProcessing.ShaderIDs.Noise, texture2D);
		...
		propertySheet.properties.SetMatrix(UnityEngine.Rendering.PostProcessing.ShaderIDs.InverseViewMatrix, context.camera.worldToCameraMatrix.inverse);
		propertySheet.properties.SetMatrix(UnityEngine.Rendering.PostProcessing.ShaderIDs.InverseProjectionMatrix, gPUProjectionMatrix.inverse);
		propertySheet.properties.SetMatrix(UnityEngine.Rendering.PostProcessing.ShaderIDs.ScreenSpaceProjectionMatrix, value2);
		propertySheet.properties.SetVector(UnityEngine.Rendering.PostProcessing.ShaderIDs.Params, new Vector4(base.settings.vignette.value, base.settings.distanceFade.value, base.settings.maximumMarchDistance.value, a));
		propertySheet.properties.SetVector(UnityEngine.Rendering.PostProcessing.ShaderIDs.Params2, new Vector4((float)context.width / (float)context.height, (float)num / (float)texture2D.width, base.settings.thickness.value, base.settings.maximumIterationCount.value));
		command.GetTemporaryRT(UnityEngine.Rendering.PostProcessing.ShaderIDs.Test, num, num, 0, FilterMode.Point, context.sourceFormat);
		command.BlitFullscreenTriangle(context.source, UnityEngine.Rendering.PostProcessing.ShaderIDs.Test, propertySheet, 0);
		...
		else
		{
			...
			command.GetTemporaryRT(UnityEngine.Rendering.PostProcessing.ShaderIDs.SSRResolveTemp, num, num, 0, FilterMode.Bilinear, context.sourceFormat);
			command.BlitFullscreenTriangle(context.source, UnityEngine.Rendering.PostProcessing.ShaderIDs.SSRResolveTemp, propertySheet, 1);
			propertySheet.properties.SetTexture(UnityEngine.Rendering.PostProcessing.ShaderIDs.History, m_History);
			command.BlitFullscreenTriangle(UnityEngine.Rendering.PostProcessing.ShaderIDs.SSRResolveTemp, m_Resolve, propertySheet, 2);
		...
		propertySheet.properties.SetTexture(UnityEngine.Rendering.PostProcessing.ShaderIDs.Resolve, m_Resolve);
