...
public sealed class PostProcessDebugLayer
{
	...
	internal void RenderMonitors(PostProcessRenderContext context)
	{
		...
		if (flag2)
		{
			command.GetTemporaryRT(UnityEngine.Rendering.PostProcessing.ShaderIDs.HalfResFinalCopy, context.width / 2, context.height / 2, 0, FilterMode.Bilinear, context.sourceFormat);
			command.Blit(context.destination, UnityEngine.Rendering.PostProcessing.ShaderIDs.HalfResFinalCopy);
		}
	...
	internal void RenderSpecialOverlays(PostProcessRenderContext context)
	{
		...
		else if (debugOverlay == DebugOverlay.MotionVectors)
		{
			...
			propertySheet3.properties.SetVector(UnityEngine.Rendering.PostProcessing.ShaderIDs.Params, new Vector4(overlaySettings.motionColorIntensity, overlaySettings.motionGridSize, 0f, 0f));
