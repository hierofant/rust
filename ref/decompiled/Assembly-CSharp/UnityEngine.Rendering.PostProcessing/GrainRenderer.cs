...
internal sealed class GrainRenderer : PostProcessEffectRenderer<Grain>
{
	...
	public override void Render(PostProcessRenderContext context)
	{
		...
		propertySheet.properties.SetVector(UnityEngine.Rendering.PostProcessing.ShaderIDs.GrainNoiseParameters, new Vector3(12.9898f, 78.233f, 43758.547f));
		...
		uberSheet.properties.SetTexture(UnityEngine.Rendering.PostProcessing.ShaderIDs.GrainTex, m_GrainLookupRT);
		uberSheet.properties.SetVector(UnityEngine.Rendering.PostProcessing.ShaderIDs.Grain_Params1, new Vector2(base.settings.lumContrib.value, base.settings.intensity.value * 20f));
		uberSheet.properties.SetVector(UnityEngine.Rendering.PostProcessing.ShaderIDs.Grain_Params2, new Vector4((float)context.width / (float)m_GrainLookupRT.width / base.settings.size.value, (float)context.height / (float)m_GrainLookupRT.height / base.settings.size.value, z, w));
	}
