...
public class NexusFerry : BaseEntity
{
	...
	private class Graph : Pool.IPooled
	{
		...
		public bool TryFindPath(Node start, Node end, List<Vector3> path)
		{
			...
			List<Node> obj = Pool.Get<List<Node>>();
			obj.Add(start);
			while (obj.Count > 0)
			{
	...
	public static readonly Translate.Phrase RetiringPhrase = new Translate.Phrase("ferry.not_in_service", "Not In Service");
	...
	public LayerMask SpawnObstructionLayers = 134359040;
	...
	private bool MoveTowardsPositionAvoidObstacles(Vector3 targetPosition)
	{
		if (!_pathTargetPosition.HasValue || !_pathTargetRotation.HasValue || (float)_sincePathCalculation > CastInterval)
		{
			...
			List<(float, float, float, Vector3, Quaternion)> obj = Pool.Get<List<(float, float, float, Vector3, Quaternion)>>();
			float num5 = 0f;
		...
		{
			List<Vector3> obj2 = Pool.Get<List<Vector3>>();
	...
	public override void ServerInit()
	{
		...
		if (_targetDock != null && _state >= State.Arrival && _state <= State.Departure)
		{
			_targetDock.CurrentFerry = this;
		}
		All.Add(this);
	...
	public override void DestroyShared()
	{
		...
		{
			Pool.FreeUnmanaged(ref _transferredIds);
		}
	}
	...
	public void UpdateSchedule(List<string> schedule)
	{
		...
		{
			Debug.LogWarning("Ignoring an empty ferry schedule update", this);
			return;
		}
		string zoneKey;
		bool flag = TryGetNextZoneFromIndex(_nextScheduleIndex, out zoneKey);
		if (_schedule != null)
		{
			Pool.FreeUnmanaged(ref _schedule);
		}
		_schedule = schedule.ShallowClonePooled();
		int num = _schedule.FindIndex(NexusServer.ZoneKey, StringComparer.InvariantCultureIgnoreCase);
		if (num >= 0)
		{
			_scheduleIndex = num;
		}
		...
		_nextScheduleIndex = (flag ? _schedule.FindIndex(zoneKey, StringComparer.InvariantCultureIgnoreCase) : (-1));
		if (_nextScheduleIndex < 0)
		{
			UpdateNextScheduleIndex(canRetire: false);
		}
	...
	private async void TransferToNextZone()
	{
		...
		try
		{
			...
			Debug.Log("Sending ferry to " + zoneKey);
	...
	private static int? TryGetNextScheduleIndex(IReadOnlyList<string> schedule, int fromIndex, RoutePreference preference)
	{
		...
		{
			if (IsValidNextIndex(schedule, fromIndex, num) && NexusServer.TryGetZoneStatus(schedule[num], out var status) && status.IsOnline && (preference != 0 || !status.IsFull))
			{
				return num;
			}
			num2++;
	...
	private void UpdateNextScheduleIndex(bool canRetire)
	{
		...
		if (num < 0 && canRetire && !_isRetiring && (float)_sinceSpawned >= NoRouteRetireCooldown)
		{
			Retire();
		}
	}
	...
	private void EjectEntity(BaseEntity entity)
	{
		if (!(entity == null))
		{
			if (_targetDock != null && _targetDock.TryFindEjectionPosition(entity, out var position))
			{
	...
	public override void Save(SaveInfo info)
	{
		...
		{
			info.msg.nexusFerry.transferredIds = _transferredIds.ShallowClonePooled() ?? Pool.Get<List<NetworkableId>>();
		}
	}
