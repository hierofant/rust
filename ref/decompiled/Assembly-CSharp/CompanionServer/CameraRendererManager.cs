...
public class CameraRendererManager : SingletonComponent<CameraRendererManager>
{
	...
	{
		List<CameraRenderer> obj = Pool.Get<List<CameraRenderer>>();
		...
		if (obj.Count > 0)
		{
			int maxSampleCount = Math.Max(1, CameraRenderer.maxRaysPerFrame / obj.Count);
			foreach (CameraRenderer item in obj)
			{
				...
				{
					UnityEngine.Debug.LogError($"Exception rendering Rust+ camera, invalidating renderer: {arg}");
