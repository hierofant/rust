...
public class TransferHandler : BaseNexusRequestHandler<TransferRequest>
{
	...
	private static readonly Dictionary<NetworkableId, ProtoBuf.Entity> UidToEntity = new Dictionary<NetworkableId, ProtoBuf.Entity>();
	...
	private void RepositionEntitiesFromTransfer()
	{
		...
		foreach (ProtoBuf.Entity entity2 in base.Request.entities)
		{
			...
			if (entity2.parent != null && entity2.parent.uid.IsValid)
			{
				if (!UidToEntity.TryGetValue(entity2.parent.uid, out var _))
				{
					Debug.LogError($"Transferred entity (ID={entity2.baseNetworkable.uid}) has a parent set but it wasn't found in the transfer! The parent is required to correctly restore this entity's position!");
				}
				if (entity2.baseEntity.pos.magnitude > 100f)
				{
					Debug.LogError($"Transferred entity (ID={entity2.baseNetworkable.uid}) has a valid parent (ID={entity2.parent.uid}) but its position ({entity2.baseEntity.pos}) doesn't seem to be in local space! This will probably not work properly!");
				}
			}
			else
			{
	...
	private void SpawnEntities(Dictionary<ulong, BasePlayer> players)
	{
		...
		try
		{
			...
			foreach (KeyValuePair<BaseEntity, ProtoBuf.Entity> item3 in EntityToSpawn)
			{
