...
public class Bike : GroundVehicle, CarPhysics<Bike>.ICar, TriggerHurtNotChild.IHurtTriggerUser, VehicleChassisVisuals<Bike>.IClientWheelUser, IPrefabPreProcess
{
	...
	public float SidecarAngle
	{
		get
		{
			if (base.isServer)
			{
				...
				{
					return Facepunch.Extend.TransformEx.Unsafe.GetLocalRotMT(in sidecarPhysicsHingeHandle).eulerAngles.z;
				}
	...
	public override void ServerInit()
	{
		...
		if (sidecarPhysicsHinge != null)
		{
			sidecarPhysicsHingeHandle = sidecarPhysicsHinge.transformHandle;
		}
	}
	...
	public override void VehicleFixedUpdate()
	{
		using (TimeWarning.New("Bike.VehicleFixedUpdate"))
		{
			...
			if (carPhysics.IsGrounded() && (float)timeSinceLastBunnyhop >= 0.5f)
			{
	...
	public override void PlayerServerInput(InputState inputState, BasePlayer player)
	{
		...
		if (hasBell && inputState.WasJustPressed(BUTTON.FIRE_PRIMARY) && (float)timeSinceBellDing > 1f)
		{
	...
	{
		if ((float)timeSinceLastBunnyhop > 3f && carPhysics.IsGrounded() && !engineController.IsWaterlogged())
		{
