		...
		for (int i = 0; i < obj.Count; i++)
		{
			...
			if (proximityInfo.connection)
			{
				...
				if (building != null)
				{
					...
					if (dominatingBuildingPrivilege != null)
					{
						...
						{
							Construction.lastPlacementError = ConstructionErrors.CannotAttachToUnauthorized.translated;
			...
			if (proximityInfo.hit)
			{
				...
				if (!(Mathf.Abs(v.y) > 1.49f) && !(v.Magnitude2D() > 1.49f))
				{
					...
					Construction.lastPlacementError = string.Format(ConstructionErrors.TooCloseTo.translated, translated);
