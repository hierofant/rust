...
public class DevDressPlayer : MonoBehaviour
{
	...
	private void ServerInitComponent()
	{
		...
		foreach (ItemAmount item in clothesToWear)
		{
			...
			{
				ItemManager.Create(item.itemDef, 1, 0uL, isServerSide: true, 0uL).MoveToContainer(component.inventory.containerWear);
			}
		}
	}
	...
	private void DoRandomClothes(BasePlayer player)
	{
		...
		{
			ItemManager.Create(item, 1, 0uL, isServerSide: true, 0uL).MoveToContainer(player.inventory.containerWear);
