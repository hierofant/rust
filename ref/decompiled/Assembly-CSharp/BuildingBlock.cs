...
public class BuildingBlock : StabilityEntity
{
	...
	{
		return new EntityBuildCost(currentGrade.CostToBuild());
	}
	...
	public bool CanSeeWallpaperSocket(BasePlayer player, int side = 0)
	{
		...
		if (!UnityEngine.Physics.Raycast(new Ray(player.eyes.HeadRay().origin, vector4.normalized), vector4.magnitude, 136314880))
		{
			flag = true;
		}
		if (!flag && construction.HasAlternativeLOSChecks())
		{
			...
			foreach (Vector3 vector5 in alternativeLOSPositions)
			{
				...
				if (!UnityEngine.Physics.Raycast(center, direction, direction.magnitude, 136314880))
				{
					flag = true;
					break;
				}
