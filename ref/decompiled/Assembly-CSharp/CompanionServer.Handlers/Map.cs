...
public class Map : BasePlayerHandler<AppEmpty>
{
	...
	public override ValueTask Execute()
	{
		...
		AppMap appMap = Facepunch.Pool.Get<AppMap>();
		...
		appMap.monuments = Facepunch.Pool.Get<List<AppMap.Monument>>();
		if (TerrainMeta.Path != null && TerrainMeta.Path.Landmarks != null)
		{
			foreach (LandmarkInfo landmark in TerrainMeta.Path.Landmarks)
			{
				if (landmark.shouldDisplayOnMap)
				{
					...
					AppMap.Monument monument = Facepunch.Pool.Get<AppMap.Monument>();
