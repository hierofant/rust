...
public class Projectile : ListComponent<Projectile>
{
	...
	[NonSerialized]
	public AttackEntity sourceWeapon;
	...
	{
		float a = mod.distanceOffset + mod.distanceScale * damageDistances.x;
		float num = ((mod.farDistanceScale >= 0f) ? mod.farDistanceScale : 1f);
		float b = mod.distanceOffset + mod.distanceScale * num * damageDistances.y;
		float b2 = ((mod.farDamageScale >= 0f) ? mod.farDamageScale : damageMultipliers.y);
		float num2 = Mathf.Lerp(damageMultipliers.x, b2, Mathf.InverseLerp(a, b, info.ProjectileDistance));
		float num3 = scale * (mod.damageOffset + mod.damageScale * num2);
		foreach (DamageTypeEntry damageType in damageTypes)
		{
			info.damageTypes.Add(damageType.type, damageType.amount * num3);
		}
		if (ConVar.Global.developer > 0)
		{
			Debug.Log(" Projectile damage: " + info.damageTypes.Total() + " (scalar=" + num3 + ")");
		}
	}
