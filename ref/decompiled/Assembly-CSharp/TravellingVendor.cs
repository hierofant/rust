...
public class TravellingVendor : BaseEntity, VehicleChassisVisuals<TravellingVendor>.IClientWheelUser, IPathListener
{
	...
	[ServerVar(Help = "(Generated) When enabled, the Travelling Vendor NPC is eligible to spawn on the server ring road; disable to prevent vendor spawning")]
	public static bool should_spawn = true;

	[ServerVar(Help = "(Generated) When enabled, the Travelling Vendor will attempt to stop for players and perform pullover interactions along its route")]
	public static bool attempt_pullovers = true;

	[ServerVar(Help = "(Generated) Maximum lifetime in seconds before the Travelling Vendor despawns; default 1800s (30 minutes)")]
	public static float alive_time_seconds = 1800f;

	[ServerVar(Help = "(Generated) When enabled, the Travelling Vendor will destroy player-placed buildings that block its ring road path")]
	...
	[ServerVar(Name = "spawn", Help = "(Generated) Spawns a Travelling Vendor at the calling player's position on the nearest ring road; registered as travellingvendor.spawn")]
	...
	[ServerVar(Name = "startevent", Help = "(Generated) Spawns a Travelling Vendor event instance; registered as travellingvendor.startevent")]
	...
	private static bool CheckSpawnPosition(Vector3 testPosition)
	{
		...
		UnityEngine.Physics.OverlapSphereNonAlloc(testPosition, 0.3f, spawncheckColliders, obstacleMask | 0x8000000);
		bool flag = false;
		Collider[] array = spawncheckColliders;
		foreach (Collider collider in array)
		{
			if (!(collider == null) && !collider.CompareTag("IgnoreCollider"))
			{
				flag = true;
				break;
			}
	...
	{
		using (TimeWarning.New("NetGroup"))
		{
			...
			foreach (Connection subscriber in net.group.subscribers)
			{
				if (subscriber != null)
				{
					...
					if (!(player == null) && player is BasePlayer basePlayer && basePlayer.Distance(this) <= (float)searchRange)
					{
		...
		foreach (BaseEntity foundEntity in foundEntities)
		{
			...
			{
				VendorTargetInfo value = playerRecords[basePlayer2];
				value.lastSeenTime = UnityEngine.Time.time;
				playerRecords[basePlayer2] = value;
	...
	public override void Save(SaveInfo info)
	{
		...
		if (IsFollowingSpline())
		{
			...
			if (BaseNetworkable.UseParallelSaves)
			{
				...
				vector = Facepunch.Extend.TransformEx.Unsafe.GetLocalRotMT(in handle) * Vector3.forward;
			}
			else
			{
				vector = base.transform.forward;
			}
			info.msg.travellingVendor.velocity = vector * splineTranslator.Speed;
		}
		else
		{
	...
	{
		float num = Mathf.InverseLerp(5f, 1.5f, myRigidbody.linearVelocity.magnitude * Mathf.Abs(Vector3.Dot(myRigidbody.linearVelocity.normalized, base.transform.forward)));
	...
	private void ApplyForceAtWheels()
	{
		...
		bool flag = myRigidbody.linearVelocity.magnitude >= max_speed;
