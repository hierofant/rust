...
public class HotAirBalloon : BaseCombatEntity, VehicleSpawner.IVehicleSpawnUser, SamSite.ISamSiteTarget, SeekerTarget.ISeekerTargetOwner, TriggerHurtNotChild.IHurtTriggerUser
{
	...
	public override void Load(LoadInfo info)
	{
		...
		if (info.msg.hotAirBalloon != null)
		{
			...
			{
				myRigidbody.linearVelocity = info.msg.hotAirBalloon.velocity;
			}
		}
	...
	private void UpdateEquipmentHealthToggles()
	{
		...
		{
			if (child is HotAirBalloonEquipment hotAirBalloonEquipment && hotAirBalloonEquipment.healthThresholdToggle != null)
			{
	...
	{
		bool wants = (flags & Flags.On) == Flags.On || (flags & Flags.Reserved2) == Flags.Reserved2 || (flags & Flags.Reserved1) == Flags.Reserved1 || !grounded;
	...
	protected void FixedUpdate()
	{
		...
		flagsUpdateScope.Set(Flags.Reserved7, grounded);
		flagsUpdateScope.Set(Flags.Reserved8, CanModifyEquipment());
		bool flag = (IsFullyInflated && myRigidbody.linearVelocity.y < 0f) || (myRigidbody.linearVelocity.y > 0f && myRigidbody.linearVelocity.y < 0.75f);
		...
		if (num != inflationLevel)
		{
			...
			flagsUpdateScope.Set(Flags.Reserved1, inflationLevel > 0.3f);
			flagsUpdateScope.Set(Flags.Reserved2, inflationLevel >= 1f);
			if (flags == base.flags)
			{
				SendNetworkUpdate();
			}
	...
	public BasePlayer GetPlayerDamageInitiator()
	{
		...
		if (component == null || component.contents == null || component.contents.Count == 0)
		{
			return null;
		}
		foreach (GameObject content in component.contents)
		{
			...
			if (!(basePlayer2 == null) && !basePlayer2.isClient && !basePlayer2.IsDead())
			{
				if (basePlayer == null)
				{
	...
	{
		if (ConVar.Server.homingMissileTargetsHab && (flags & Flags.Reserved2) == Flags.Reserved2)
		{
			return (float)engineOffTime <= engineOffTimeToPreventHomingTarget;
		}
		return false;
	}
