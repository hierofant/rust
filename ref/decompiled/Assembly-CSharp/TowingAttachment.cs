...
public class TowingAttachment<TEntity> : TowingAttachment where TEntity : BaseEntity, ITowing
{
	...
	public void AttachTo<TTowableEntity>(TTowableEntity other) where TTowableEntity : ITowing
	{
		...
		using (BaseEntity.FlagsUpdateScope flagsUpdateScope = _entity.StartSetFlags(BaseEntity.FlagsUpdateMode.SendNetworkUpdate))
		{
			flagsUpdateScope.Set(BaseEntity.Flags.Reserved14, b: true);
		}
		using (BaseEntity.FlagsUpdateScope flagsUpdateScope2 = towEntity.StartSetFlags(BaseEntity.FlagsUpdateMode.SendNetworkUpdate))
		{
			flagsUpdateScope2.Set(BaseEntity.Flags.Reserved14, b: true);
		}
		_entity.OnTowAttach();
		other.OnTowAttach();
	}
	...
	public void Detach()
	{
		...
		using (BaseEntity.FlagsUpdateScope flagsUpdateScope = _entity.StartSetFlags(BaseEntity.FlagsUpdateMode.SendNetworkUpdate))
		{
			flagsUpdateScope.Set(BaseEntity.Flags.Reserved14, b: false);
		}
		BaseEntity baseEntity = _otherEntity.Get(serverside: true);
		bool flag = baseEntity == null;
		if (!flag)
		{
			...
			flagsUpdateScope2.Set(BaseEntity.Flags.Reserved14, b: false);
		}
		_otherEntity = default(EntityRef<BaseEntity>);
		_entity.OnTowDetach();
		if (!flag && baseEntity is ITowing towing)
		{
			towing.OnTowDetach();
		}
	}
