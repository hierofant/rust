...
public class Planner : HeldEntity
{
	...
	private void DoPlace(RPCMessage msg)
	{
		...
		using CreateBuilding msg2 = msg.read.Proto<CreateBuilding>();
		DoBuild(msg2);
	}
	...
	public virtual void DoBuild(CreateBuilding msg)
	{
		...
		if (msg.entity.IsValid)
		{
			...
			if (msg.isSnapped)
			{
				...
				Quaternion quaternion = Quaternion.Euler(msg.snappedRotation);
		...
		if (gameObject != null)
		{
			...
			if (baseEntity != null && deployable != null)
			{
				...
				if (deployable.copyInventoryFromItem)
				{
					...
					if (component2 == null)
					{
						ContainerIOEntity component3 = baseEntity.GetComponent<ContainerIOEntity>();
						if ((bool)component3)
						{
	...
	public static bool HasLineOfSight(ref Construction.Placement placement, Construction construction, Construction.Target target)
	{
		...
		bool flag = GamePhysics.LineOfSightRadius(origin, p, layerMask, num3, 0f, padding);
		if (!flag && target.socket != null && construction.HasAlternativeLOSChecks())
		{
			...
			foreach (Vector3 vector in alternativeLOSPositions)
			{
				Vector3 direction = placement.position + placement.rotation * vector - origin;
				if (!UnityEngine.Physics.Raycast(origin, direction, direction.magnitude, layerMask))
				{
					return true;
				}
			}
