...
public class SamSite : ContainerIOEntity
{
	...
	public void FixedUpdate()
	{
		...
		if (!ObjectEx.IsUnityNull(currentTarget) && IsPowered())
		{
			...
			if (currentTarget.GetWorldVelocity().magnitude > 0.1f && currentTarget.SAMTargetType.applyAimError)
			{
	...
	private void ReloadVolley()
	{
		...
		reloadVolleyFinishTime = UnityEngine.Time.time + timeBetweenBursts;
		firedCount = 0;
	}
