...
public class ClanChangeTracker : IClanChangeSink
{
	...
	private void HandleClanChanged(in ClanChangedEvent data)
	{
		...
		if ((data.DataSources & ClanDataSource.Members) == ClanDataSource.Members)
		{
	...
	private void HandleMembershipChanged(in MembershipChangedEvent data)
	{
		...
		if (basePlayer != null)
		{
			...
			if (_clanManager.Backend.TryGet(basePlayer.clanId, out var clan))
			{
				basePlayer.serverClan = clan;
			}
			else
			{
