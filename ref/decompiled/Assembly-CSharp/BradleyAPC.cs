...
public class BradleyAPC : BaseCombatEntity, TriggerHurtNotChild.IHurtTriggerUser, IPathListener
{
	...
	private bool CanDeployScientists(BaseEntity attacker, List<GameObjectRef> scientistPrefabs, List<Vector3> spawnPositions)
	{
		...
		while (!flag)
		{
			if (UnityEngine.Physics.Raycast(ScientistSpawnPoints[num2 % ScientistSpawnPoints.Count].transform.position + Vector3.up * 1f, Vector3.down, out var hitInfo, 2f, layerMask) && RustNavMeshHelpers.SamplePosition(hitInfo.point + Vector3.up * 0.3f, out var _, 6f, walkableAreaMask))
			{
	...
	private ScientistNPC SpawnScientist(GameObjectRef scientistPrefab, Vector3 spawnPos, bool roadSpawned)
	{
		...
		RustNavMeshAgent component = scientistNPC.GetComponent<RustNavMeshAgent>();
		if (component != null)
		{
	...
	public void UpdateMovement_Patrol()
	{
		...
		if (Vector3.Distance(finalDestination, closestToPoint.Position) > 2f)
		{
			...
			else if (AStarPath.FindPath(iAIPathNode, closestToPoint, out path, out pathCost))
			{
				currentPath.Clear();
				if (flag)
				{
	...
	public void DoSimpleAI()
	{
		...
		if (!IsOnSpline())
		{
			...
			if (num > stoppingDist)
			{
				...
				float num5 = Vector3.Dot(myRigidBody.linearVelocity, base.transform.forward);
	...
	public void DoPhysicsMove()
	{
		...
		{
			float num5 = num2;
			ReadOnlySpan<IAIPathSpeedZone> speedZones = patrolPath.SpeedZones;
			for (int i = 0; i < speedZones.Length; i++)
			{
				...
				{
					num5 = Mathf.Min(num5, iAIPathSpeedZone.GetMaxSpeed());
				}
			}
		...
		float t = Mathf.InverseLerp(2f, 1f, linearVelocity.magnitude * Mathf.Abs(Vector3.Dot(linearVelocity.normalized, base.transform.forward)));
		float torqueAmount = Mathf.Lerp(moveForceMax, turnForce, t);
		float num7 = Mathf.InverseLerp(5f, 1.5f, linearVelocity.magnitude * Mathf.Abs(Vector3.Dot(linearVelocity.normalized, base.transform.forward)));
		...
		impactDamager.damageEnabled = myRigidBody.linearVelocity.magnitude > 2f;
	}
	...
	public override void OnDied(HitInfo info)
	{
		...
		for (int j = 0; j < maxCratesToSpawn; j++)
		{
			...
			rigidbody.linearVelocity = zero + onUnitSphere2 * UnityEngine.Random.Range(1f, 3f);
	...
	public float GetDamageMultiplier(BaseEntity ent)
	{
		...
		float num2 = Vector3.Dot(myRigidBody.linearVelocity, base.transform.forward);
		if (num2 > 0f)
		{
			num += num2 * 0.5f;
		}
