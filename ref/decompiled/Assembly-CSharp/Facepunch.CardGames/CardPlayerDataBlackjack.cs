...
public class CardPlayerDataBlackjack : CardPlayerData
{
	...
	{
		base.Dispose();
		Pool.FreeUnmanaged(ref SplitCards);
	}
	...
	public override void Save(CardGame syncData)
	{
		...
		blackjackCardPlayer.splitCards = Pool.Get<List<int>>();
		...
		{
			syncData.blackjack.players = Pool.Get<List<CardGame.BlackjackCardPlayer>>();
		}
	...
	{
		List<PlayingCard> obj = Pool.Get<List<PlayingCard>>();
		obj.AddRange(Cards);
		Cards.Clear();
