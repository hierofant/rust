...
public class BlackjackController : CardGameController
{
	...
	{
		List<BlackjackInputOption> obj = Pool.Get<List<BlackjackInputOption>>();
	...
	protected override void SubReceivedInputFromPlayer(CardPlayerData pData, int input, int value, bool countAsAction)
	{
		...
		if (!((!AllBetsPlaced) ? DoBettingPhasePlayerInput(pdBlackjack, value, countAsAction, ref selectedMove, ref selectedMoveValue) : DoInRoundPlayerInput(pdBlackjack, ref selectedMove, ref selectedMoveValue)))
		{
			return;
		}
		pData.CancelTurnTimer();
		LastActionTarget = pData.UserID;
		LastAction = selectedMove;
	...
	{
		if (selectedMove != BlackjackInputOption.Abandon && (((uint)pdBlackjack.availableInputs & (uint)selectedMove) != (uint)selectedMove || selectedMove == BlackjackInputOption.None))
		{
			return false;
		}
		switch (selectedMove)
	...
	private bool DoBettingPhasePlayerInput(CardPlayerDataBlackjack pdBlackjack, int value, bool countAsAction, ref BlackjackInputOption selectedMove, ref int selectedMoveValue)
	{
		if (selectedMove != BlackjackInputOption.Abandon && (((uint)pdBlackjack.availableInputs & (uint)selectedMove) != (uint)selectedMove || selectedMove == BlackjackInputOption.None))
		{
			return false;
		...
		{
			if (value < MinBuyIn || value > BlackjackMachine.maxbet)
			{
				return false;
			}
	...
	protected override void EndCycle()
	{
		...
		{
			obj.CancelTurnTimer();
			obj.SetHasCompletedTurn(hasActed: false);
		}
