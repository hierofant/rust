...
public class Chat : ConsoleSystem
{
	...
	public static void BroadcastPlayerAction(BasePlayer subject, string action)
	{
		...
		{
			if (!(activePlayer == null) && activePlayer.net?.connection != null)
			{
				string text = ((subject != null) ? NameHelper.GetPlayerNameStreamSafe(activePlayer, subject) : "SERVER").EscapeRichText();
				ConsoleNetwork.SendClientCommand(activePlayer.net.connection, "chat.add", 2, 0, "<color=#eee>SERVER</color> " + text + " " + action);
			}
		}
		RecordPlayerAction(((subject != null) ? subject.displayName : "SERVER") + " " + action, subject);
	}
	...
	public static void BroadcastPlayerAction(BasePlayer subjectA, string middle, BasePlayer subjectB, string suffix)
	{
		...
		foreach (BasePlayer activePlayer in BasePlayer.activePlayerList)
		{
			if (!(activePlayer == null) && activePlayer.net?.connection != null)
			{
				string text = ((subjectA != null) ? NameHelper.GetPlayerNameStreamSafe(activePlayer, subjectA) : "SERVER").EscapeRichText();
				string text2 = ((subjectB != null) ? NameHelper.GetPlayerNameStreamSafe(activePlayer, subjectB) : "SERVER").EscapeRichText();
				ConsoleNetwork.SendClientCommand(activePlayer.net.connection, "chat.add", 2, 0, "<color=#eee>SERVER</color> " + text + middle + text2 + suffix);
			}
		}
		string obj = ((subjectA != null) ? subjectA.displayName : "SERVER");
		string text3 = ((subjectB != null) ? subjectB.displayName : "SERVER");
	...
	private static void RecordPlayerAction(string message, BasePlayer subject)
	{
		...
		ce.Message = message;
		ce.UserId = ((subject != null) ? subject.UserIDString : "0");
		ce.Username = ((subject != null) ? subject.displayName : "SERVER");
		ce.Color = "#eee";
