...
public class CameraRenderTask : CustomYieldInstruction, IDisposable
{
	...
	public CameraRenderTask()
	{
		...
		_emptyTopology = new NativeArray<int>(1, Allocator.Persistent);
	}
	...
	public int Start(int width, int height, float verticalFov, float nearPlane, float farPlane, int layerMask, in Matrix4x4 transf, int sampleCount, int sampleOffset, Dictionary<int, (byte MaterialIndex, int Age)> knownColliders)
	{
		...
		raycastRaySetupJob.cameraPos = position;
		raycastRaySetupJob.cameraRot = rotation;
		...
		raycastRayProcessingJob.cameraForward = -vector;
		raycastRayProcessingJob.farPlane = farPlane;
		raycastRayProcessingJob.oceanEnabled = flag;
		raycastRayProcessingJob.oceanLevel = oceanLevel;
		raycastRayProcessingJob.oceanTopologyMask = 384;
		raycastRayProcessingJob.topologyRes = ((!flag) ? 1 : topologyQueryStructure.res);
		raycastRayProcessingJob.topologyOrigin = new float2(TerrainMeta.Position.x, TerrainMeta.Position.z);
		raycastRayProcessingJob.topologyOneOverSize = new float2(TerrainMeta.OneOverSize.x, TerrainMeta.OneOverSize.z);
		raycastRayProcessingJob.topology = (flag ? topologyQueryStructure.source : _emptyTopology.AsReadOnly());
		raycastRayProcessingJob.raycastCommands = _raycastCommands.GetSubArray(0, sampleCount);
		...
		JobHandle job2 = RaycastCommand.ScheduleBatch(dependsOn: IJobParallelForExtensions.Schedule(jobData2, sampleCount, 100), commands: _raycastCommands.GetSubArray(0, sampleCount), results: _raycastHits.GetSubArray(0, sampleCount), minCommandsPerJob: 100);
		JobHandle dependsOn2 = IJobParallelForExtensions.Schedule(jobData3, sampleCount, 100, JobHandle.CombineDependencies(job, job2));
