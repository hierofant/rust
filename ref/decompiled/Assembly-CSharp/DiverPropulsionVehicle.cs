...
public class DiverPropulsionVehicle : BaseMountable, IEngineControllerUser, IEntity
{
	...
	{
		using FlagsUpdateScope flagsUpdateScope = StartSetFlags(FlagsUpdateMode.SendNetworkUpdate);
		flagsUpdateScope.Set(Flags.Reserved6, rigidBody.IsSleeping() && !AnyMounted());
	}
	...
	public override void VehicleFixedUpdate()
	{
		using (TimeWarning.New("DPV.VehicleFixedUpdate"))
		{
			...
			UpdateWaterInfo();
			rigidBody.linearDamping = (IsMounted() ? normalDrag : highDrag);
			...
			if (WaterIsDeepEnough(updateWaterInfo: false))
			{
				...
				float num3 = rigidBody.linearVelocity.y - targetClimbSpeed;
				...
				if (IsOn())
				{
					...
					if (Mathf.Abs(num) > 1f)
					{
						...
						rigidBody.AddForce(-normalized * (num4 * (0.08f * engineKW) * rigidBody.mass * rigidBody.linearDamping));
					}
					float num5 = turnPower * rigidBody.mass * rigidBody.angularDamping;
