...
public class SlotMachine : BaseMountable, INotifyLOD
{
	...
	private void CheckPayout()
	{
		...
		if (PayoutSettings != null)
		{
			if (CalculatePayout(out var info, out var bonus))
			{
				...
				if (baseEntity != null && baseEntity is SlotMachineStorage slotMachineStorage)
				{
					...
					{
						ItemManager.Create(info.Item.itemDef, num, 0uL, isServerSide: true, 0uL).MoveToContainer(slotMachineStorage.inventory, 1);
					}
				}
				if (CurrentSpinPlayer.IsValid() && CurrentSpinPlayer == GetMounted())
				{
					CurrentSpinPlayer.ChatMessage($"You received {num}x {info.Item.itemDef.displayName.english} for slots payout!");
				}
				Analytics.Azure.OnGamblingResult(CurrentSpinPlayer, this, (int)PayoutSettings.SpinCost.amount * CurrentMultiplier, num);
		...
		{
			Debug.LogError($"Failed to process spin results: PayoutSettings != null {PayoutSettings != null} CurrentSpinPlayer.IsValid {CurrentSpinPlayer.IsValid()} CurrentSpinPlayer == mounted {CurrentSpinPlayer == GetMounted()}");
		}
		if (!flag)
		{
			DelayedSpinningReset();
		}
		else
		{
	...
	{
		if (!(msg.player != GetMounted()) && !HasFlag(Flags.Reserved2))
		{
	...
	{
		using FlagsUpdateScope flagsUpdateScope = StartSetFlags(FlagsUpdateMode.SendNetworkUpdate);
		flagsUpdateScope.Set(Flags.Reserved1, (float)amount >= PayoutSettings.SpinCost.amount * (float)CurrentMultiplier);
	}
