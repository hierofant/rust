...
public class RelationshipManager : BaseEntity
{
	...
	public class PlayerRelationships : Facepunch.Pool.IPooled
	{
		...
		public void EnterPool()
		{
			...
			if (relations != null)
			{
				Facepunch.Pool.Free(ref relations, freeElements: true);
			}
		}
		...
		public void ClearRelations()
		{
			...
			{
				var (_, obj) = (KeyValuePair<ulong, PlayerRelationshipInfo>)(ref relation);
	...
	private static Translate.Phrase RemoteInvitesBlocked = new Translate.Phrase("remote.invites.blocked", "That player has remote invites turned off");
	...
	public override bool OnRpcMessage(BasePlayer player, uint rpc, Message msg)
	{
		using (TimeWarning.New("RelationshipManager.OnRpcMessage"))
		{
			...
			if (rpc == 2865758366u && player != null)
			{
				Assert.IsTrue(player.isServer, "SV_RPC Message is using a clientside player!");
				if (Global.developer > 2)
				{
					Debug.Log("SV_RPCMessage: " + player?.ToString() + " - TryCreateTeam");
				}
	...
	{
		if (ArgEx.Player(arg) == null || ServerInstance == null)
		{
			return;
		}
	...
	public void UpdateAcquaintancesFor(BasePlayer player, float deltaSeconds)
	{
		...
		{
			if (item == player || !item.IsAlive() || item.IsSleeping() || item.isInvisible || item.limitNetworking)
			{
				continue;
			}
			...
			if (relations.type != 0 && relations.type != RelationshipType.Acquaintance)
			{
				continue;
			}
			bool flag = false;
	...
	public void SetRelationship(BasePlayer player, BasePlayer otherPlayer, RelationshipType type, int weight = 1, bool sendImmediate = false)
	{
		...
		if (flag || relations.mugshotCrc == 0 || num3 >= mugshotUpdateInterval)
		{
			...
			if (flag3)
			{
				...
				float num5 = Vector3Ex.Distance2D(player.transform.position, otherPlayer.transform.position);
	...
	public void SendRelationshipsFor(BasePlayer player)
	{
		...
		using ProtoBuf.RelationshipManager.PlayerRelationships arg = GetRelationshipSaveByID(playerID);
		ClientRPC(RpcTarget.Player("CLIENT_RecieveLocalRelationships", player), arg);
	}
	...
	public override void Save(SaveInfo info)
	{
		...
		foreach (KeyValuePair<ulong, PlayerTeam> team in teams)
		{
			...
			playerTeam.joinKey = value.joinKey;
			playerTeam.members = Facepunch.Pool.Get<List<ProtoBuf.PlayerTeam.TeamMember>>();
			playerTeam.invites = Facepunch.Pool.Get<List<ulong>>();
