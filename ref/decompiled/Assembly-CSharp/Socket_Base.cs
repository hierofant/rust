...
public class Socket_Base : PrefabAttribute
{
	...
	public virtual bool CheckSocketMods(ref Construction.Placement placement)
	{
		...
		{
			if (!(socketMod.SocketGrouping != null))
			{
				if (placement.shouldParent && !flag && socketMod is SocketMod_AreaCheck)
				{
					flag = true;
				}
				if (!socketMod.DoCheck(ref placement))
				{
		...
		if (flag && placement.shouldParent && !placement.parentPassed)
		{
			Construction.lastPlacementError = ConstructionErrors.NotStableEnough;
			return false;
		}
		return true;
	}
