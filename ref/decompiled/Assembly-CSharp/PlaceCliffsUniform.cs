				...
				for (float num6 = x; num6 < num; num6 += num4)
				{
					...
					foreach (Prefab prefab in array2)
					{
							...
							for (int k = min_rotation_x; k <= max_rotation_x; k += rotation_delta_x)
							{
								for (int l = -1; l <= 1; l += 2)
								{
									for (int m = -1; m <= 1; m += 2)
									{
										...
										if (prefab.ApplyTerrainFilters(pos2, quaternion, vector2) && prefab.ApplyTerrainAnchors(ref pos2, quaternion, vector2, AnchorMode, ((FilterMode & SpawnFilterMode.TerrainAnchorPoints) != 0) ? Filter : null) && prefab.ApplyTerrainChecks(pos2, quaternion, vector2, ((FilterMode & SpawnFilterMode.TerrainCheckPoints) != 0) ? Filter : null) && prefab.ApplyWaterChecks(pos2, quaternion, vector2) && prefab.ApplyEnvironmentVolumeChecks(pos2, quaternion, vector2) && (!flag || vector.y < pos2.y))
										{
											flag = true;
											vector = pos2;
											rotation = quaternion;
						...
						if (flag)
						{
							...
							if (prefab.CheckTerrainFootprint(vector, rot2, scale2))
							{
								prefab.FillTerrainFootprint(vector, rot2, scale2);
								prefab.SpawnedCount++;
								World.AddPrefab("Decor", prefab, vector, rotation, scale2);
								break;
							}
						}
					}
