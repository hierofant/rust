...
public class ElectricBattery : IOEntity, IInstanceDataReceiver
{
	public class DischargeWorkQueue : PersistentObjectWorkQueue<ElectricBattery>
	{
		...
		{
			if (!((float)entity.lastDischargeCheck < 1f))
			{
				entity.lastDischargeCheck = 0f;
	...
	private Action TickUsageCB;

	private Action AddChargeCB;
	...
	{
		base.ServerInit();
		lastDischargeCheck = 0f;
		DischargeQueue.Add(this);
	}

	internal override void DoServerDestroy()
	{
		base.DoServerDestroy();
		DischargeQueue.Remove(this);
	}
	...
	public override void IOStateChanged(int inputAmount, int inputSlot)
	{
		...
		if (inputSlot == 0 && rechargable)
		{
			if (AddChargeCB == null)
			{
				AddChargeCB = AddCharge;
			}
			if (!IsPowered() && !IsFlickering())
			{
				CancelInvoke(AddChargeCB);
				lastChargeIn = 0f;
			}
			else if (!IsInvoking(AddChargeCB))
			{
				InvokeRandomized(AddChargeCB, 1f, 1f, 0.1f);
			}
		}
	}
	...
	public virtual void ChargeChanged(float oldCharge)
	{
		...
		FlagsUpdateScope setFlags = StartSetFlags(FlagsUpdateMode.SendNetworkUpdate_Flags);
		try
		{
			if (HasFlag(Flags.Reserved5) != flag || HasFlag(Flags.Reserved6) != flag2)
			{
				setFlags.Set(Flags.Reserved5, flag);
				setFlags.Set(Flags.Reserved6, flag2);
			}
			RefreshFullChargeFlag();
		}
		finally
		{
			((IDisposable)setFlags).Dispose();
		}
		void RefreshFullChargeFlag()
		{
			bool flag3 = (float)Mathf.RoundToInt(rustWattSeconds / 60f) >= maxCapactiySeconds / 60f;
			bool flag4 = HasFlag(Flags.Reserved9);
			if (flag3 && !flag4)
			{
	...
	public void SetPassthroughOn(bool wantsOn)
	{
		...
		if (TickUsageCB == null)
		{
			TickUsageCB = TickUsage;
		}
		if (IsOn())
		{
			if (!IsInvoking(TickUsageCB))
			{
				InvokeRandomized(TickUsageCB, 1f, 1f, 0.1f);
			}
		}
		else
		{
			CancelInvoke(TickUsageCB);
		}
		MarkDirty();
	}
	...
	{
		ElectricBattery electricBattery = BaseNetworkable.serverEntities.Find(ArgEx.GetEntityID(arg, 1)) as ElectricBattery;
