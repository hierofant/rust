...
public class BaseMetalDetector : HeldEntity
{
	...
	private void DetectLongRange()
	{
		...
		if (Vector3.SqrMagnitude(ownerPlayer.transform.position - lastDetectPlayerPos) < DetectMinMovementDistance * DetectMinMovementDistance)
		{
	...
	private void DetectSources(BasePlayer player)
	{
		...
		{
			IMetalDetectable metalDetectable = obj[i];
