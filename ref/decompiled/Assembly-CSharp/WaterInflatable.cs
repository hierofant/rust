...
public class WaterInflatable : BaseMountable, IPoolVehicle, INotifyTrigger
{
	...
	{
		using (TimeWarning.New("WaterInflatable.VehicleFixedUpdate"))
		{
	...
	public override void PlayerServerInput(InputState inputState, BasePlayer player)
	{
		...
		else
		{
			...
			{
				isInPool = IsInWaterVolume(base.transform.position, out var natural) && !natural;
				inPoolCheck = 0f;
			}
			if (additiveDownhillVelocity > 0f && !isInPool)
			{
				...
				rigidBody.linearVelocity = Vector3.Lerp(linearVelocity, base.transform.forward * linearVelocity.magnitude, 0.4f);
			}
