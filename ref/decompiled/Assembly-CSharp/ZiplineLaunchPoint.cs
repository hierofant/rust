...
public class ZiplineLaunchPoint : BaseEntity
{
	...
	public void FindZiplineTarget(ref List<Vector3> foundPositions)
	{
		...
		List<ZiplineTarget> obj = Facepunch.Pool.Get<List<ZiplineTarget>>();
		GamePhysics.OverlapSphere(position + base.transform.forward * 185f, 185f, obj, 1084293377);
		ZiplineTarget ziplineTarget = null;
		float num = float.MinValue;
		float num2 = 3f;
		foreach (ZiplineTarget item in obj)
		{
			...
			foreach (ZiplineTarget item2 in obj)
			{
				...
				else
				{
					...
					foreach (ZiplineTarget item3 in obj)
					{
	...
	{
		if (IsBusy() || msg.player == null || !msg.player.CanInteract() || msg.player.Distance(LineDeparturePoint.position) > 3f || !IsPlayerFacingValidDirection(msg.player) || ziplineTargets.Count == 0)
		{
			return;
		}
		Vector3 position = LineDeparturePoint.position;
