...
public class CompleteTrain : IDisposable
{
	...
	private bool isCinematicBraking;

	private float cinematicBrakeForce;

	private bool isCinematicDriving;

	private float cinematicTargetSpeed;

	private float cinematicAccelForce;
	...
	private bool IsCinematicActive
	{
		get
		{
			if (!isCinematicBraking)
			{
				return isCinematicDriving;
			}
			return true;
		}
	}
	...
	{
		List<TrainCar> list = Facepunch.Pool.Get<List<TrainCar>>();
		list.Add(trainCar);
		Init(list);
	}
	...
	private void Cleanup()
	{
		if (!disposed)
		{
			...
			disposed = true;
			Facepunch.Pool.FreeUnmanaged(ref trainCars);
		}
	}
	...
	public void UpdateTick(float dt)
	{
		...
		ranUpdateTick = true;
		if (IsAllAsleep() && !HasAnyEnginesOn() && !HasAnyCollisions() && !isShunting && !IsCinematicActive)
		{
			trackSpeed = 0f;
			return;
		}
		...
		if (!disposed)
		{
			...
			if (!HasAnyEnginesOn() && !HasAnyCollisions() && !IsCinematicActive && UnityEngine.Time.time > lastMovingTime + 10f)
			{
				trackSpeed = 0f;
				SleepAll();
			}
	...
	public bool TryGetAdjacentTrainCar(TrainCar trainCar, bool next, Vector3 forwardDir, out TrainCar result)
	{
		...
		Vector3 lhs = ((!trainCars[num].IsCoupledBackwards()) ? trainCar.transform.forward : (-trainCar.transform.forward));
	...
	private void MovementTick(float dt)
	{
		...
		if (isCinematicBraking)
		{
			float maxDelta = cinematicBrakeForce / TotalMass * dt;
			trackSpeed = Mathf.MoveTowards(trackSpeed, 0f, maxDelta);
			if (trackSpeed == 0f)
			{
				isCinematicBraking = false;
				isCinematicDriving = false;
			}
		}
		else if (isCinematicDriving)
		{
			float maxDelta2 = cinematicAccelForce / TotalMass * dt;
			trackSpeed = Mathf.MoveTowards(trackSpeed, cinematicTargetSpeed, maxDelta2);
			if (cinematicTargetSpeed == 0f && trackSpeed == 0f)
			{
				isCinematicDriving = false;
			}
		}
		...
		else
		{
			...
			else
			{
				...
				float num2 = shuntDistance - Vector3.Distance((Vector3)shuntStartPos2D, (Vector3)shuntTargetPos2D);
				if (num2 < 2f)
				{
		...
		if (!isCinematicDriving && !isCinematicBraking)
		{
			float num3 = trainCars[0].rigidBody.linearDamping;
	...
	public void SetTrackSpeedCinematic(float speed, float accelForce = 50000f)
	{
		isCinematicBraking = false;
		isCinematicDriving = true;
		cinematicTargetSpeed = Mathf.Clamp(speed, 0f - (TrainCar.TRAINCAR_MAX_SPEED - 1f), TrainCar.TRAINCAR_MAX_SPEED - 1f);
		cinematicAccelForce = accelForce;
		lastMovingTime = UnityEngine.Time.time;
		WakeAll();
	}

	public void CinematicBrake(float force = 50000f)
	{
		isCinematicBraking = true;
		cinematicBrakeForce = force;
		lastMovingTime = UnityEngine.Time.time;
		WakeAll();
	}
	...
	private int CheckLinedUpToUnload(out Vector3 unloaderPos)
	{
		...
		for (int i = 0; i < trainCars.Count; i++)
		{
			...
			{
				using (BaseEntity.FlagsUpdateScope flagsUpdateScope = trainCar.StartSetFlags(BaseEntity.FlagsUpdateMode.SendNetworkUpdate))
				{
					flagsUpdateScope.Set(BaseEntity.Flags.Reserved4, isLinedUp);
				}
				if (isLinedUp)
				{
					return i;
	...
	private float ApplyCollisionsToTrackSpeed(float trackSpeed, float totalMass, float deltaTime)
	{
		...
		Vector3 forwardVector = (owner.IsCoupledBackwards() ? (-owner.transform.forward) : owner.transform.forward);
		...
		forwardVector = (owner.IsCoupledBackwards() ? (-owner.transform.forward) : owner.transform.forward);
	...
	private float ApplyCollisions(float trackSpeed, TrainCar ourTrainCar, Vector3 forwardVector, bool atOurFront, TriggerTrainCollisions trigger, float ourTotalMass, ref StaticCollisionState wasStaticColliding, StaticCollisionState otherEndStaticColliding, float deltaTime)
	{
		...
		if (!flag && otherEndStaticColliding == StaticCollisionState.Free)
		{
			...
			{
				trackSpeed = HandleRigidbodyCollision(atOurFront, trackSpeed, forwardVector, ourTotalMass, otherRigidbodyContent, otherRigidbodyContent.linearVelocity, otherRigidbodyContent.mass, deltaTime, calcSecondaryForces: true);
				num += Vector3.Magnitude(otherRigidbodyContent.linearVelocity - vector) * Mathf.Clamp(otherRigidbodyContent.mass, 1f, 13000f);
			}
		}
	...
	private float HandleTrainCollision(bool front, Vector3 forwardVector, float trackSpeed, Transform ourTransform, TrainCar theirTrain, float deltaTime, ref StaticCollisionState wasStaticColliding)
	{
		...
		if ((num > 40f && num < 140f) || Mathf.Abs(f) < 0.95f)
		{
			trackSpeed = (front ? (-0.5f) : 0.5f);
		}
		else
		{
			List<CompleteTrain> prevTrains = Facepunch.Pool.Get<List<CompleteTrain>>();
			...
			float num2 = GetTotalPushingForces(vector, forwardVector, ref prevTrains, out nextTrainTrackSpeed);
			if (!front)
			{
				num2 *= -1f;
			}
			...
			{
				float max = Mathf.Max(trackSpeed + 2f, nextTrainTrackSpeed + 2f);
				float min = Mathf.Min(trackSpeed - 2f, nextTrainTrackSpeed - 2f);
				trackSpeed += num2 / TotalMass * deltaTime;
				trackSpeed = Mathf.Clamp(trackSpeed, min, max);
			}
			Facepunch.Pool.FreeUnmanaged(ref prevTrains);
		}
		return trackSpeed;
	}
	...
	private float GetTotalPushingMass(Vector3 pushDirection, Vector3 ourForward, ref List<CompleteTrain> prevTrains)
	{
		...
		foreach (TrainCar trainContent in triggerTrainCollisions.trainContents)
		{
			...
			{
				Vector3 ourForward2 = (trainContent.IsCoupledBackwards() ? (-trainContent.transform.forward) : trainContent.transform.forward);
