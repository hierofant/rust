...
public class SteamInventory : EntityComponent<BasePlayer>
{
	...
	public bool HasWorkshopSkin(ItemDefinition itemDefinition)
	{
		...
		if (workshopSkinShortNames == null || workshopSkinVersion != workshopSkinCacheVersion)
		{
			...
			{
				IPlayerItemDefinition itemDefinition2 = PlatformService.Instance.GetItemDefinition(playerItem.DefinitionId);
		...
		{
			return workshopSkinShortNames.Contains(itemDefinition.name);
		}
		return true;
	}
	...
	private async Task UpdateSteamInventory(BaseEntity.RPCMessage msg)
	{
		...
		{
			Debug.LogWarning($"UpdateSteamPlayer: inventory belongs to someone else (userID={base.baseEntity.userID.Get()})");
		}
		else if ((bool)base.gameObject)
		{
			SetItems(playerInventory.Items.ToArray());
