...
public class GenerateDungeonBase : ProceduralComponent
{
		...
		for (int i = 0; i < component.Sockets.Count; i++)
		{
			...
			foreach (Quaternion quaternion in rotations)
			{
				...
				if (!IsBlocked(segments, component, vector, quaternion3))
				{
					...
					if (num2 > linkScore && prefab.CheckEnvironmentVolumesOutsideTerrain(vector, quaternion3, Vector3.one, EnvironmentType.UnderwaterLab, (EnvironmentType)0, 1f))
					{
	...
	public static void SetupAI()
	{
		...
		foreach (DungeonBaseInfo dungeonBaseEntrance in TerrainMeta.Path.DungeonBaseEntrances)
		{
			...
			WakeAIZ wakeAIZ = obj.AddComponent<WakeAIZ>();
			wakeAIZ.isBox = true;
	...
	{
		if (AI.useUnityNavmesh || AiManager.nav_disable || !AI.npc_enable || TerrainMeta.Path == null || TerrainMeta.Path.DungeonBaseEntrances == null)
		{
			return;
		}
		...
		foreach (DungeonBaseInfo dungeonBaseEntrance in TerrainMeta.Path.DungeonBaseEntrances)
		{
			if (!(dungeonBaseEntrance == null))
			{
				...
				{
					NavMeshTools.LogWarning("Underwater lab " + dungeonBaseEntrance.name + " has no bakeable colliders, it will have no navmesh at all.");
					continue;
				}
				GameObject obj = new GameObject("LabNavmesh");
				obj.transform.position = bounds.center;
				obj.transform.SetParent(dungeonBaseEntrance.gameObject.transform, worldPositionStays: true);
				IndependantNavmesh independantNavmesh = obj.AddComponent<IndependantNavmesh>();
	...
	private static bool TryGetStructureBounds(DungeonBaseInfo dungeon, out Bounds bounds)
	{
		...
		NavMeshTools.EncapsulateNavmeshColliders(dungeon.gameObject, ref bounds, ref hasFootprint);
