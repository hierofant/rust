...
public class ItemCrafter : EntityComponent<BasePlayer>
{
	...
	public void ServerUpdate(float delta)
	{
		...
		if (value.endTime == 0f)
		{
			...
			float num = GetScaledDuration(value.blueprint, currentCraftLevel, owner.IsInTutorial);
			if (value.workbenchEntity != null && value.workbenchEntity is Workbench workbench)
			{
				num *= workbench.GetCraftSpeedMultiplier(value);
			}
			value.endTime = UnityEngine.Time.realtimeSinceStartup + num;
			if (owner != null)
			{
				owner.Command("note.craft_start", value.taskUID, num, value.amount);
	...
	public bool CraftItem(ItemBlueprint bp, BasePlayer owner, ProtoBuf.Item.InstanceData instanceData = null, int amount = 1, int skinID = 0, Item fromTempBlueprint = null, bool free = false, int attachmentID = 0)
	{
		...
		itemCraftTask.attachmentID = attachmentID;
	...
	public void FinishCrafting(ItemCraftTask task)
	{
		...
		Item item2 = ItemManager.CreateByItemID(task.blueprint.targetItem.itemid, 1, skin, (ulong)task.attachmentID);
	...
	public ProtoBuf.ItemCrafter Save()
	{
		...
		itemCrafter.queue = Facepunch.Pool.Get<List<ProtoBuf.ItemCrafter.Task>>();
		foreach (ItemCraftTask item in queue)
		{
		...
		{
			List<ProtoBuf.Item> list = Facepunch.Pool.Get<List<ProtoBuf.Item>>();
			if (items != null)
			{
