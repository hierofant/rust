...
public class DeployVolumeOBB : DeployVolume
{
	...
	{
		position += rotation * (worldRotation * bounds.center + worldPosition);
		if (DeployVolume.CheckOBB(new OBB(position, bounds.size, rotation * worldRotation), (int)layers & mask, this, types, filterMode, ignoredEntity, ignoreChildrenOfEntity))
		{
			return true;
		}
		return false;
