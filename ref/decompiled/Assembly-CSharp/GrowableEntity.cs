...
public class GrowableEntity : BaseCombatEntity, IInstanceDataReceiver, IHeatSourceListener
{
	...
	public static float CalculateArtificialLightExposure(Transform forTransform)
	{
		...
		CeilingLight.FarmLightGrid.Query(position.x, position.z, ceilingLightGrowableRange, obj);
	...
	public static float SunRaycast(Vector3 checkPosition)
	{
		...
		if (!UnityEngine.Physics.Raycast(checkPosition, normalized, out var _, 100f, 144769025))
		{
			return 1f;
		}
		return 0f;
	...
	public static float CalculateArtificialTemperature(Transform forTransform)
	{
		...
		GrowableHeatSource.FarmHeatSourceGrid.Grid.Query(position2.x, position2.z, artificialTemperatureGrowableRange, obj);
		float num = 0f;
		foreach (GrowableHeatSource item in obj)
		{
			if (!(item.GetBaseEntity() == null) && !(item.GetBaseEntity().transform == null) && !(Vector3.Distance(item.GetBaseEntity().transform.position, position2) > artificialTemperatureGrowableRange))
			{
	...
	public static void GrowAll(ConsoleSystem.Arg arg)
	{
		...
		using PooledList<GrowableEntity> pooledList = Facepunch.Pool.Get<PooledList<GrowableEntity>>();
		Query.Server.GetInSphere(basePlayer.ServerPosition, 6f, pooledList, Query.DistanceCheckType.Bounds);
		foreach (GrowableEntity item in pooledList)
		{
			if (item.isServer)
			{
		...
		using PooledList<PlanterBox> pooledList2 = Facepunch.Pool.Get<PooledList<PlanterBox>>();
		Query.Server.GetInSphere(basePlayer.ServerPosition, 6f, pooledList2, Query.DistanceCheckType.Bounds);
		foreach (PlanterBox item2 in pooledList2)
		{
	...
	{
		if (DeepSeaManager.IsInsideDeepSea(this) && HasParent() && parentEntity.Get(base.isServer) is PlanterBoxStatic)
		{
			if (DeepSeaManager.Get(base.isServer) != null)
			{
	...
	public override void OnDeployed(BaseEntity parent, BasePlayer deployedBy, Item fromItem)
	{
		...
		float num = ((deployedBy.modifiers != null) ? deployedBy.modifiers.GetValue(Modifier.ModifierType.Farming_BetterGenes) : 0f);
