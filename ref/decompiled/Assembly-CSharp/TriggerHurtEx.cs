...
public class TriggerHurtEx : TriggerBase, IServerComponent, IHurtTrigger
{
	...
	public float enterDamageCooldown = 0.5f;

	internal Dictionary<BaseEntity, EntityTriggerInfo> entityInfo;

	private Dictionary<BaseEntity, float> lastEnterDamageTimes;
	...
	internal void DoDamage(BaseEntity ent, HurtType type, List<DamageTypeEntry> damage, GameObjectRef effect, float multiply = 1f)
	{
		...
		using (TimeWarning.New("TriggerHurtEx.DoDamage"))
		{
			...
			{
				if (LOSCheck && GamePhysics.Trace(new Ray(base.transform.position + new Vector3(0f, 0.1f, 0f), (ent.transform.position - base.transform.position).normalized), 0f, out var _, LOSDistance, 1218519297, QueryTriggerInteraction.Ignore, GameObjectEx.ToBaseEntity(base.gameObject)))
				{
					return;
				}
				...
				if ((bool)baseCombatEntity)
				{
					...
					hitInfo2.Initiator = GameObjectEx.ToBaseEntity(base.gameObject);
					hitInfo2.PointStart = base.transform.position;
	...
	private void ProcessQueues()
	{
		...
		foreach (BaseEntity entityLeave in entityLeaveList)
		{
			...
			if (lastEnterDamageTimes != null && lastEnterDamageTimes.TryGetValue(entityLeave, out var value) && Time.time - value >= enterDamageCooldown)
			{
				lastEnterDamageTimes.Remove(entityLeave);
			}
			if (entityInfo != null)
			{
	...
	private bool CanApplyEnterDamage(BaseEntity ent)
	{
		if (enterDamageCooldown <= 0f)
		{
			return true;
		}
		if (lastEnterDamageTimes == null)
		{
			lastEnterDamageTimes = new Dictionary<BaseEntity, float>();
		}
		if (lastEnterDamageTimes.TryGetValue(ent, out var value) && Time.time - value < enterDamageCooldown)
		{
			return false;
		}
		lastEnterDamageTimes[ent] = Time.time;
		return true;
	}
}
