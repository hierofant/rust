...
public class VDDraw : SingletonComponent<VDDraw>, IServerComponent
{
	...
	public static bool isRecording
	{
		get
		{
			if (!(SingletonComponent<VDDraw>.Instance != null))
			{
				return false;
			}
	...
	private void StopRecording()
	{
		if (_isRecording)
		{
			...
			SaveLogs();
			stopwatch.Stop();
			UnityEngine.Debug.Log($"VDDraw: Saved logs in {stopwatch.ElapsedMilliseconds} ms");
			stopwatch.Reset();
			CompressCurrentLog();
			stopwatch.Stop();
			UnityEngine.Debug.Log($"VDDraw: Compressed logs in {stopwatch.ElapsedMilliseconds} ms");
			currentLogPath = null;
			UnityEngine.Application.quitting -= StopRecording;
		}
	}
	...
	private void SaveLogs()
	{
		...
		{
			using FileStream stream = new FileStream(currentLogPath, FileMode.Append, FileAccess.Write, FileShare.None, 4096, useAsync: false);
			VDDrawEntry result;
			while (logEntries.TryDequeue(out result))
			{
				result.WriteToStream(stream, lengthDelimited: true);
				result.Dispose();
			}
		}
	}
	...
	private static void ManageLogFiles()
	{
		...
		if (list.Count > 10)
		{
			...
			for (int i = 0; i < num; i++)
			{
				UnityEngine.Debug.Log("Deleting log file: " + list[i].FullName);
				list[i].Delete();
			}
		}
	}
