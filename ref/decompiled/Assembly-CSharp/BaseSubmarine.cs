...
public class BaseSubmarine : BaseVehicle, IPoolVehicle, IEngineControllerUser, IEntity, IAirSupply
{
	...
	public float UpDownInput
	{
		get
		{
			if (base.isServer)
			{
				return GetUpDownInput(UnityEngine.Time.time);
			}
			return _upDown;
		}
		protected set
	...
	public override bool OnRpcMessage(BasePlayer player, uint rpc, Message msg)
	{
		using (TimeWarning.New("BaseSubmarine.OnRpcMessage"))
		{
			...
			if (rpc == 2181221870u && player != null)
			{
				...
				if (ConVar.Global.developer > 2)
				{
					Debug.Log("SV_RPCMessage: " + player?.ToString() + " - RPC_OpenTorpedoStorage");
				}
	...
	public override void VehicleFixedUpdate()
	{
		using (TimeWarning.New("BaseSubmarine.VehicleFixedUpdate"))
		{
			...
			rigidBody.linearDamping = (HasDriver() ? normalDrag : highDrag);
			float num = 2f;
			if (IsSurfaced)
			{
				float num2 = 20f * num;
			...
			if (IsInWater)
			{
				...
				float num5 = rigidBody.linearVelocity.y - targetClimbSpeed;
			...
			if (IsOn() && IsInWater)
			{
				...
				float num7 = turnPower * rigidBody.mass * rigidBody.angularDamping;
			...
			if (torpedoContainer != null)
			{
				...
				flagsUpdateScope.Set(Flags.Reserved6, b2);
			}
			BasePlayer driver = GetDriver();
			if (driver != null && primaryFireInput)
			{
				bool flag = true;
				if (IsInWater && (float)timeSinceTorpedoFired >= reloadTime)
				{
	...
	public void RPC_OpenFuel(RPCMessage msg)
	{
		...
		if ((!IsSafe() || !(player != creatorEntity)) && CanBeLooted(player))
		{
