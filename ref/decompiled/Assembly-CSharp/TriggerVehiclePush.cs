	...
	public void FixedUpdate()
	{
		...
		foreach (BaseEntity entityContent in entityContents)
		{
			...
			if ((bool)rigidbody && !rigidbody.isKinematic)
			{
				...
				rigidbody.AddForceAtPosition(vector2 * maxPushVelocity * num3 * rigidbody.mass, vector, ForceMode.Force);
				if (num4 > 0f)
				{
					rigidbody.AddForceAtPosition(vector2 * 1f * num4 * rigidbody.mass, vector, ForceMode.Impulse);
				}
			}
		}
	}
