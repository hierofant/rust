...
public class WorldItem : BaseEntity, PlayerInventory.ICanMoveFrom
{
	...
	private static readonly Translate.Phrase NotYourBackPackError = new Translate.Phrase("error.notyourbackpack", "Cannot move item: Not your backpack!");
	...
	public override bool OnRpcMessage(BasePlayer player, uint rpc, Message msg)
	{
		using (TimeWarning.New("WorldItem.OnRpcMessage"))
		{
			...
			if (rpc == 2145528377 && player != null)
			{
				Assert.IsTrue(player.isServer, "SV_RPC Message is using a clientside player!");
				if (Global.developer > 2)
				{
					Debug.Log("SV_RPCMessage: " + player?.ToString() + " - PickupTimer");
				}
				using (TimeWarning.New("PickupTimer"))
				{
					using (TimeWarning.New("Conditions"))
					{
	...
	public void PickupTimer(RPCMessage msg)
	{
		...
		{
			pickupStartTime = UnityEngine.Time.realtimeSinceStartup;
		}
	}
	...
	private void RPC_OpenLoot(RPCMessage rpc)
	{
		...
		if (!(component == null) && component.canLootInWorld)
		{
			...
			if ((bool)player && player.CanInteract() && !player.IsBlockedFromLootingByMountable() && CanOpenInSafeZone(player) && Interface.CallHook("CanLootEntity", player, this) == null && player.inventory.loot.StartLootingEntity(this))
			{
