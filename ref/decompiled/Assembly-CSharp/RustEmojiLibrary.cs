...
public class RustEmojiLibrary : BaseScriptableObject
{
	...
	public struct EmojiSource
	{
		...
		public Translate.Phrase ItemDisplayPhrase;
		...
		public bool CanBeUsedBy(BasePlayer p, ulong userId)
		{
			...
			if (RequiredItem != null)
			{
				...
				if (p != null && p.blueprints.CheckSkinOwnership(RequiredItem.id, p))
				{
					return true;
				}
				return false;
	...
	private void Prewarm()
	{
		...
		{
			if (!(item.iconSprite == null) && (!item.Hidden() || item.vehicleItem))
			{
