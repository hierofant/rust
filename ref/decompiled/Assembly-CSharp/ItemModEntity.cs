...
public class ItemModEntity : ItemMod
{
	...
	public override void OnRemove(Item item)
	{
		...
		if (!(heldEntity == null))
		{
			...
			{
				Debug.LogError($"Held entity for item '{item.info.shortname}' has prefab '{heldEntity.PrefabName}' ({heldEntity.net.ID}) but expected '{entityPrefab.resourcePath}'!!", heldEntity);
			}
			heldEntity.Kill();
