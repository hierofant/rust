...
public class MissionObjective_DeployItem : MissionObjective
{
	...
	private void UpdatePings(BasePlayer playerFor)
	{
		...
		foreach (TutorialBuildTarget item in obj)
		{
			...
			{
				playerFor.AddPingAtLocation(PingType, item.transform.position, 86400f, currentTutorialIsland.net.ID);
			}
			else
			{
				playerFor.RemovePingAtLocation(PingType, item.transform.position, 0.5f, currentTutorialIsland.net.ID);
			}
		}
