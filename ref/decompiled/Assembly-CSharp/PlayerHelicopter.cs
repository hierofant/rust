...
public class PlayerHelicopter : BaseHelicopter, IEngineControllerUser, IEntity, SamSite.ISamSiteTarget
{
	...
	private static readonly Translate.Phrase CantRepairWithEngineOn = new Translate.Phrase("error_cannot_repair_with_engine_on", "Cannot repair while the engine is running.");
	...
	public bool IsGrounded()
	{
		...
		if (wheels.Length == 0)
		{
			flag = UnityEngine.Physics.Raycast(base.transform.position + Vector3.up * 0.1f, Vector3.down, 0.5f);
		}
		else
		{
			float num = 1f;
	...
	protected override void ApplyHandbrake()
	{
		...
		recentVelocities[recentVelIndex] = rigidBody.linearVelocity.sqrMagnitude;
