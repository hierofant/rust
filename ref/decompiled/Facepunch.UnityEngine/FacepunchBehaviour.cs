...
public abstract class FacepunchBehaviour : MonoBehaviour
{
	...
	public void InvokeUnscaledTime(Action action, float time)
	{
		InvokeHandlerUnscaledTime.Invoke(this, action, time);
	}

	public void InvokeUnscaledTime(Action action, float time, float random)
	{
		time += UnityEngine.Random.Range(0f - random, random);
		InvokeHandlerUnscaledTime.Invoke(this, action, time);
	}

	public void InvokeRepeatingUnscaledTime(Action action, float time, float repeat)
	{
		InvokeHandlerUnscaledTime.InvokeRepeating(this, action, time, repeat);
	}

	public void InvokeRandomizedUnscaledTime(Action action, float time, float repeat, float random)
	{
		InvokeHandlerUnscaledTime.InvokeRandomized(this, action, time, repeat, random);
	}

	public void CancelInvokeUnscaledTime(Action action)
	{
		InvokeHandlerUnscaledTime.CancelInvoke(this, action);
	}

	public bool IsInvokingUnscaledTime(Action action)
	{
		return InvokeHandlerUnscaledTime.IsInvoking(this, action);
	}
}
