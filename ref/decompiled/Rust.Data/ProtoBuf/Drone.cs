...
public class Drone : IDisposable, Pool.IPooled, IProto<Drone>, IProto
{
	...
	public static Drone Deserialize(BufferStream stream, Drone instance, bool isDelta)
	{
		...
		while (true)
		{
			...
			stream.ConsumeFieldOperation("ProtoBuf.Drone");
			if (num == 13)
			{
				stream.ValidateFieldOrder(ref lastFieldId, 1u, fieldIsRepeated: false, "ProtoBuf.Drone");
			...
			{
				stream.ValidateFieldOrder(ref lastFieldId, key.Field, fieldIsRepeated: true, "ProtoBuf.Drone");
				ProtocolParser.SkipKey(stream, key);
			}
		}
		return instance;
