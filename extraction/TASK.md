...
## Этап 6. Визуал из клиента: меши, текстуры, иконки → `data/<buildid>/visual/`

Код клиента обфусцирован, но его бандлы (`<Steam>/steamapps/common/Rust/Bundles/`) — обычные Unity AssetBundle.
Их можно читать Python-библиотекой **UnityPy** (`pip install UnityPy Pillow`). Клиент должен быть той же версии, что и сервер
(buildid в отчёте). Скрипт положи в `extraction/visual/`.

Список префабов, для которых нужен визуал: `extraction/visual-prefabs.txt` (1153 пути: всё, что можно поставить,
их скины грейдов и условные модели).

Для каждого префаба из списка:
1. Найди его в бандлах по container path (`assets/prefabs/...prefab`).
2. Обойди иерархию GameObject. Для каждого узла с `MeshFilter`+`MeshRenderer` (и `SkinnedMeshRenderer`, если есть)
   запиши трансформ **относительно корня префаба** (позиция, кватернион, масштаб — числами, как в Unity, без конвертации осей),
   ссылку на меш и материалы по сабмешам.
3. `LODGroup`: бери только **LOD0** (если LOD0 > 20k треугольников — LOD1). Неактивные узлы пропускай. Узлы только с коллайдерами не нужны.
4. Материал: имя шейдера, основная текстура (`_MainTex` / `_BaseColorMap` / `_Albedo` — какая есть), цвет (`_Color`/`_BaseColor`),
   признак прозрачности (render queue ≥ 2450, cutout с `_Cutoff`, или прозрачный шейдер — стекло), `_Cutoff`, если есть.

Формат вывода:
- `visual/meshes/<meshName>_<hash>.obj` — вершины, **UV0**, нормали, группы `g submesh_<N>` по сабмешам. Координаты как в Unity, без флипа.
- `visual/textures/<texName>_<hash>.webp` — не больше **512 px** по большей стороне (скины грейдов стен/фундаментов/потолков можно 1024), качество ~80.
- `visual/prefabs.json` — `{ "<prefab path>": [ { "node": "<путь узла>", "pos":[..], "rot":[x,y,z,w], "scale":[..], "mesh": "<meshId>",
  "materials": [ { "shader": "...", "texture": "<texId|null>", "color": [r,g,b,a], "transparent": bool, "cutoff": number|null } ] } ] }`.
  Префабы, которых нет в бандлах или у которых нет рендереров, тоже перечисли с пустым массивом и причиной в отчёте.
- `visual/icons/<shortname>.webp` — иконки предметов (спрайт `ItemDefinition.iconSprite`), 128 px.

Бюджет: всё `visual/` ≤ **120 МБ** (это пойдёт в APK). Если не влезает — уменьши текстуры до 256 px кроме скинов грейдов и напиши, сколько вышло.
В `REPORT.md` допиши раздел «Этап 6»: сколько префабов с визуалом, сколько без, размер, версия клиента.
